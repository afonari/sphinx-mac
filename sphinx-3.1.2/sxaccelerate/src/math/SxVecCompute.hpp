...
// --- The next 312 lines were generated from snippets/SxVecCompute.hpp snippet Simd_OP
...
SxVecCompute<Simd>::add (const SxVecRef<T, Layout> &x, const SxVecRef<T, Layout> &y)
{
   ...
   int resMemType = SxAllocMem::resMemType (x.allocMem->memType,
                                            y.allocMem->memType);
   SxAllocMem *mem = SxAllocMem::create (sizeof(T) * n, resMemType);

#ifdef USE_GPU
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      addGPU ((T*)mem->mem, n, x.elements, y.elements, gpuId);
   } else
#endif
...
SxVecCompute<Simd>::add (const SxVecRef<T,Layout> &x, SxVecRef<T,Layout> &&y)
{
   ...
   if (x.isGPU ())  {
      SX_CHECK (y.isGPU ());
      SX_CHECK (x.getGPUId () == y.getGPUId (), x.getGPUId (), y.getGPUId ());
      typename SxVecResult<T,Layout>::VecType res(y);
      addInPlaceGPU (res.elements, x.elements, x.getSize (), x.getGPUId ());
      return res;
   }
#endif
   // never reuse shared memory
   SX_CHECK (y.allocMem->memType == SxAllocMem::NormalRAM, y.allocMem->memType);
...
SxVecCompute<Simd>::subtract (const SxVecRef<T, Layout> &x, const SxVecRef<T, Layout> &y)
{
   ...
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      subtractGPU ((T*)mem->mem, n, x.elements, y.elements, gpuId);
   } else
#endif
...
SxVecCompute<Simd>::subtract (const SxVecRef<T,Layout> &x, SxVecRef<T,Layout> &&y)
{
   ...
   if (x.isGPU ())  {
      ...
      SX_CHECK (x.getGPUId () == y.getGPUId (), x.getGPUId (), y.getGPUId ());
      typename SxVecResult<T,Layout>::VecType res(y);
      subtractInPlaceGPU (res.elements, x.elements, x.getSize (), x.getGPUId ());
      return res;
   }
#endif
   // never reuse shared memory
...
SxVecCompute<Simd>::multiply (const SxVecRef<T, Layout> &x, const SxVecRef<T, Layout> &y)
{
   ...
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      multiplyGPU ((T*)mem->mem, n, x.elements, y.elements, gpuId);
   } else
#endif
...
SxVecCompute<Simd>::multiply (const SxVecRef<T,Layout> &x, SxVecRef<T,Layout> &&y)
{
   ...
   if (x.isGPU ())  {
      ...
      SX_CHECK (x.getGPUId () == y.getGPUId (), x.getGPUId (), y.getGPUId ());
      typename SxVecResult<T,Layout>::VecType res(y);
      multiplyInPlaceGPU (res.elements, x.elements, x.getSize (), x.getGPUId ());
      return res;
   }
#endif
   // never reuse shared memory
...
SxVecCompute<Simd>::divide (const SxVecRef<T, Layout> &x, const SxVecRef<T, Layout> &y)
{
   ...
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      divideGPU ((T*)mem->mem, n, x.elements, y.elements, gpuId);
   } else
#endif
...
SxVecCompute<Simd>::divide (const SxVecRef<T,Layout> &x, SxVecRef<T,Layout> &&y)
{
   ...
   if (x.isGPU ())  {
      ...
      SX_CHECK (x.getGPUId () == y.getGPUId (), x.getGPUId (), y.getGPUId ());
      typename SxVecResult<T,Layout>::VecType res(y);
      divideInPlaceGPU (res.elements, x.elements, x.getSize (), x.getGPUId ());
      return res;
   }
#endif
   // never reuse shared memory
...
// --- The next 136 lines were generated from snippets/SxVecCompute.hpp snippet Simd_OPEQ
// vector x+= y (x, y both contiguous, same type)
template<class T, StrideType Layout>
void
SxVecCompute<Simd>::addInPlace (SxVecRef<T, Layout> &x, const SxVecRef<T, Layout> &y)
{
   ...
   if (x.isGPU ())  {
      SX_CHECK (y.isGPU ());
      SX_CHECK (x.getGPUId () == y.getGPUId (), x.getGPUId (), y.getGPUId ());
      addInPlaceGPU (x.elements, y.elements, n, x.getGPUId ());
...
SxVecCompute<Simd>::subtractInPlace (SxVecRef<T, Layout> &x, const SxVecRef<T, Layout> &y)
{
   ...
   if (x.isGPU ())  {
      SX_CHECK (y.isGPU ());
      SX_CHECK (x.getGPUId () == y.getGPUId (), x.getGPUId (), y.getGPUId ());
      subtractInPlaceGPU (x.elements, y.elements, n, x.getGPUId ());
      // SX_VALIDATE_VECTOR (x);
      return;
   }
#endif
...
SxVecCompute<Simd>::multiplyInPlace (SxVecRef<T, Layout> &x, const SxVecRef<T, Layout> &y)
{
   ...
   if (x.isGPU ())  {
      SX_CHECK (y.isGPU ());
      SX_CHECK (x.getGPUId () == y.getGPUId (), x.getGPUId (), y.getGPUId ());
      multiplyInPlaceGPU (x.elements, y.elements, n, x.getGPUId ());
      // SX_VALIDATE_VECTOR (x);
      return;
   }
#endif
...
SxVecCompute<Simd>::divideInPlace (SxVecRef<T, Layout> &x, const SxVecRef<T, Layout> &y)
{
   ...
   if (x.isGPU ())  {
      SX_CHECK (y.isGPU ());
      SX_CHECK (x.getGPUId () == y.getGPUId (), x.getGPUId (), y.getGPUId ());
      divideInPlaceGPU (x.elements, y.elements, n, x.getGPUId ());
      // SX_VALIDATE_VECTOR (x);
      return;
   }
#endif
...
// --- The next 296 lines were generated from snippets/SxVecCompute.hpp snippet SimdCol_OP
...
SxVecCompute<SimdCol>::add (const SxVecRef<T, Layout> &x, const SxVecRef<T, Layout2> &y)
{
   ...
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      addGPU (nRows, x.getNCols (), x.elements, x.getColStride (),
                   y.elements, y.getColStride (),
                   (T*)mem->mem, nRows,  gpuId);
   } else
#endif
#ifdef USE_OPENMP
   if (n > sxChunkSize)
...
SxVecCompute<SimdCol>::subtract (const SxVecRef<T, Layout> &x, const SxVecRef<T, Layout2> &y)
{
   ...
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      subtractGPU (nRows, x.getNCols (), x.elements, x.getColStride (),
                   y.elements, y.getColStride (),
                   (T*)mem->mem, nRows,  gpuId);
   } else
#endif
...
SxVecCompute<SimdCol>::multiply (const SxVecRef<T, Layout> &x, const SxVecRef<T, Layout2> &y)
{
   ...
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      multiplyGPU (nRows, x.getNCols (), x.elements, x.getColStride (),
                   y.elements, y.getColStride (),
                   (T*)mem->mem, nRows,  gpuId);
   } else
#endif
...
SxVecCompute<SimdCol>::divide (const SxVecRef<T, Layout> &x, const SxVecRef<T, Layout2> &y)
{
   ...
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      divideGPU (nRows, x.getNCols (), x.elements, x.getColStride (),
                   y.elements, y.getColStride (),
                   (T*)mem->mem, nRows,  gpuId);
   } else
#endif
...
// --- The next 472 lines were generated from snippets/SxVecCompute.hpp snippet SimdIt_OP
                            ...
                            const SxVecRef<T, Layout>  &y)
{
   ...
   SX_CHECK (x.getNCols () == y.getNCols (),
             x.getNCols (), y.getNCols ());
   SX_CHECK (x.isRAM () && y.isRAM ()); // GPU not yet supported
...
// --- The next 168 lines were generated from snippets/SxVecCompute.hpp snippet UseIterator_OP
...
// --- The next 160 lines were generated from snippets/SxVecCompute.hpp snippet UseIterator_OPEQ
template<class T, StrideType Layout, class T2, StrideType Layout2>
void
...
// --- The next 188 lines were generated from snippets/SxVecCompute.hpp snippet Simd_OP_scalar
...
{
   SX_CHECK (x.isRAM ()); // GPU not yet supported
   typedef decltype(T() + T2()) TRes;
...
// --- The next 50 lines were generated from snippets/SxVecCompute.hpp snippet Simd_scalar_vec
...
// --- The next 448 lines were generated from snippets/SxVecCompute.hpp snippet Simd_vecfunc
// Unary minus
template<class T, StrideType Layout>
typename SxVecResult<T, Layout>::VecType
SxVecCompute<Simd>::minus (const SxVecRef<T, Layout> &vec)
{
   ...
   SX_CHECK (vec.isRAM ()); // GPU not yet supported
...
// --- Simd_vecfunc
// --- The next 54 lines were generated from snippets/SxVecCompute.hpp snippet Simd_abs_complex
template<class T, StrideType Layout>
typename SxVecResult<T, Layout>::VecType
...
// --- The next 268 lines were generated from snippets/SxVecCompute.hpp snippet UseIterator_OP_scalar
// vector x + scalar y
template<class T, StrideType Layout, class T2>
...
// --- The next 68 lines were generated from snippets/SxVecCompute.hpp snippet UseIterator_scalar_vec
// scalar y - vector x
template<class T, StrideType Layout, class T2>
SxVector<decltype(T(0) + T2(0))>
...
// --- The next 280 lines were generated from snippets/SxVecCompute.hpp snippet UseIterator_vecfunc
// Unary minus
template<class T, StrideType Layout>
...
// --- The next 252 lines were generated from snippets/SxVecCompute.hpp snippet UseIterator_abs_real
// Absolute value
template<StrideType Layout>
...
// --- The next 70 lines were generated from snippets/SxVecCompute.hpp snippet UseIterator_abs_complex
template<class T, StrideType Layout>
SxVector<T>
...
// --- The next 54 lines were generated from snippets/SxVecCompute.hpp snippet SimdCol_reduce
/// Add all elements
template<class T>
...
// --- The next 39 lines were generated from snippets/SxVecCompute.hpp snippet UseIterator_reduce
/// Add all elements
template<class T, StrideType Layout>
...
// --- The next 176 lines were generated from snippets/SxVecCompute.hpp snippet Transpose
                             ...
                             const SxVecRef<T,Layout2> &b)
{
   ...
   int resMemType = SxAllocMem::resMemType (a.allocMem->memType,
                                            b.allocMem->memType);
   SxAllocMem *mem = SxAllocMem::create (sizeof(T) * nRow * nCol, resMemType);
#ifdef USE_GPU
   if (SxAllocMem::isGPU (resMemType)) {
      int gpuId = SxAllocMem::getLocationId(resMemType);
      // call BLAS {s,d,c,z}gemm wrapper
      matmultGPU ((T*)mem->mem, 0., a.elements, b.elements,
                  (int)a.getNRows (), (int)a.getNCols (), (int)b.getNCols (),
                  (int)a.getColStride (), (int)b.getColStride (),
                  (int)a.getNRows (), gpuId);
   } else
#endif
   // call BLAS {s,d,c,z}gemm wrapper
                                            ...
                                            const SxVecRef<T,Layout2> &b)
{
   SX_CHECK (a.getNRows () > 0);
   SX_CHECK (b.getNCols () > 0);
   SX_CHECK (a.isRAM () && b.isRAM ()); // GPU not yet supported
   ssize_t nRow = a.getNRows (), nCol = b.getNCols ();
