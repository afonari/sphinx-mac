...
// --- The next 480 lines were generated from snippets/SxMathLib.cpp snippet inPlaceRot
                 ...
                 int nRows, int nCols)
{
   ...
   char noTrans = 'N';
#endif
#if defined USE_INTEL_MKL || defined USE_NETLIB
   if (sxAbsSqr(rotMat[nCols-1]) < 1e-24 * sxAbsSqr (rotMat[0]))  {
      ...
      if (upT)  {
        ...
        cblas_strmm (CblasColMajor, CblasRight, CblasUpper, CblasNoTrans,
                     CblasNonUnit, nRows, nCols,
                     one, rotMat, nCols, mat, nRows);
#else
...
//     * maybe if omp is used inside zgemm, omp can deal with it...
         ...
         cblas_sgemm (CblasColMajor, CblasNoTrans, CblasNoTrans,
                      myBlockSize, nCols, nCols,
                      one, (float*)(mat + r), nRows,
                      (float*)rotMat, nCols, zero,
...
//     * maybe if omp is used inside zgemm, omp can deal with it...
         ...
         cblas_dgemm (CblasColMajor, CblasNoTrans, CblasNoTrans,
                      myBlockSize, nCols, nCols,
                      one, (double*)(mat + r), nRows,
                      (double*)rotMat, nCols, zero,
   ...
   if (sxAbsSqr(rotMat[nCols-1]) < 1e-24 * sxAbsSqr (rotMat[0]))  {
      ...
      if (upT)  {
        ...
        cblas_ctrmm (CblasColMajor, CblasRight, CblasUpper, CblasNoTrans,
                     CblasNonUnit, nRows, nCols,
                     (float*)&one, rotMat, nCols, mat, nRows);
#else
        #error "Missing implementation"
#endif
         return;
   ...
   if (sxAbsSqr(rotMat[nCols-1]) < 1e-24 * sxAbsSqr (rotMat[0]))  {
      ...
      if (upT)  {
        ...
        cblas_ztrmm (CblasColMajor, CblasRight, CblasUpper, CblasNoTrans,
                     CblasNonUnit, nRows, nCols,
                     (double*)&one, rotMat, nCols, mat, nRows);
#else
        #error "Missing implementation"
#endif
         return;
