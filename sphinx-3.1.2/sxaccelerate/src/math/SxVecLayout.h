...
/** \brief Check if a layout describes a contiguous block of memory
    @param the layout to be checked

    A layout may be contiguous by design, or if the strides leave
    no holes in memory usage, i.e., the row stride is 1 and
    the col stride matches the number of rows.
*/
template<StrideType Layout>
inline bool isContiguous(const SxVecLayout<Layout> &layout)
{
   return isContiguous (Layout)
          || (layout.getRowStride () == 1
              && (   layout.getColStride () == layout.nRows
                  || layout.nCols == 1)                     );
}
