...
// --- The next 312 lines were generated from snippets/SxVecCompute.cpp snippet Simd_abs_real
// Absolute value
SxVector<int> SxVecCompute<Simd>::abs (const SxVecRef<int,Compact> &vec)
{
   SX_CHECK (vec.getSize () > 0);
   SX_CHECK (vec.isRAM ()); // no GPU support yet
...
// --- The next 134 lines were generated from snippets/SxVecCompute.cpp snippet isHermitian
bool SxVecCompute<Simd>::isHermitian (const SxVector<float> &H)
{
   SX_CHECK (H.isRAM ()); // no GPU support (yet?)
