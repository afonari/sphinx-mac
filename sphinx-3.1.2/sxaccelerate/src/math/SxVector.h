...
class SX_EXPORT_MATH SxAuxData
{
   public:
      ...
      /// Set species and atom
      inline void setAtom (int iSpecies, int iAtom)
      {
         is = iSpecies;
         ia = iAtom;
      }

      /// Copy species and atom
      inline void setAtom (const SxAuxData &in)
      {
         is = in.is;
         ia = in.ia;
      }
      ...
      /// Set n and l and m
      inline void setOrb (int nIn, int lIn, int mIn)
      {
         n = char(nIn); l = char(lIn); m = char (mIn);
      }
};
...
class SxAllocMem
{
   public:
      ...
      /// Memory types
      enum MemType {
         /// normal CPU RAM allocated via malloc & friends
         NormalRAM = 0x0,
         /// CPU shared RAM (currently unused)
         SharedRAM = 0x1,
         /// MPI shared RAM (currently unused)
         MPISharedRAM = 0x3,
         /// GPU device memory. Device id can be set by adding 256 * id
         GPUMemory = 0x4,
         /// GPU device memory. Device will current id
         CurrentGPUMemory = 0x6
      };

      /// Check if data is in RAM (CPU memory space)
      static inline bool isRAM (int memType)
      {
#        ifdef USE_GPU
            return !(memType & GPUMemory);
#        else
            SX_UNUSED (memType);
            return true;
#        endif
      }
      inline bool isRAM () const { return isRAM (memType); }

      /// Check if data is in GPU memory space
      static inline bool isGPU (int memType)
      {
#        ifdef USE_GPU
            return !(memType & GPUMemory);
#        else
...
#        endif
      ...
      inline bool isGPU () const { return isGPU (memType); }

      /// Get memory location (GPU device) id
      inline int getLocationId () const { return memType >> 8; }
      static inline int getLocationId (int memType) { return memType >> 8; }

      /** \brief Determine the resulting memory type for automatic
                 allocation.

          If the vectors live in RAM, allocate standard RAM (not shared).
          If the vectors live on GPU, allocate on same device.
      */
      static inline int resMemType (int memType)
      {
         return isRAM(memType) ? NormalRAM : (GPUMemory + (memType & ~0xff));
      }
      static inline int resMemType (int memType1, int memType2)
      {
         // check that memType1 and memType2 are either both in RAM
         // or both on the same GPU => agree in bits 3 and 9-32
         SX_CHECK ((memType1 & 0xfffff04) == (memType2 & 0xffffff04),
                   memType1 & 0xff, memType2 & 0xff,
                   getLocationId (memType1), getLocationId(memType2));
         return resMemType(memType1);
      }
      ...
      static void destroy (SxAllocMem *alloc)
      {
         ...
         if (alloc->memType == NormalRAM) {
            SxAllocation::retain (alloc, roundCL(sizeof(SxAllocMem)));
         } else {
            // call destructor to deallocate
            alloc->~SxAllocMem ();
         }
      }
   protected:
...
std::ostream& operator<< (std::ostream& out, const SxAllocMem &allocMem)
{
   ...
   if (allocMem.memType & SxAllocMem::GPUMemory)  {
      out << " on GPU " << allocMem.getLocationId ();
   } else if (allocMem.memType & SxAllocMem::SharedRAM)  {
      out << " [shared]";
   }
   return out;
}
...
class SxVecRef : protected SxVecLayout<Layout>
{
   ...
   public:
      ...
      /// Check if data is in RAM (CPU memory space)
      inline bool isRAM () const { return allocMem && allocMem->isRAM (); }
      /// Check if data is in GPU memory space
      inline bool isGPU () const { return allocMem && allocMem->isGPU (); }
      /// Get GPU id
      inline int getGPUId () const
      {
         SX_CHECK (isGPU ());
         return allocMem->getLocationId ();
      }
   ...
   public:
      ...
      Iterator      begin ()
      {
         SX_CHECK (isRAM ()); // no iterators on GPU!
         return SxVecIt<T,Layout> (elements, *this);
      }
      ConstIterator begin () const
      {
         SX_CHECK (isRAM ()); // no iterators on GPU!
         return SxVecIt<T,Layout> (const_cast<T*>(elements), *this);
      }
      /// Get end iterator
      ...
      {
#        ifdef USE_GPU
         SX_CHECK (isRAM ()); // no direct access on GPU!
#        endif
...
{
   if (in.isGPU ())  {
      cout << "Cannot validate GPU vectors yet" << endl;
      return;
   }
...
class SX_EXPORT_MATH SxVector : public SxVecRef<T, Compact>
{
   public:
      ...
      void allocOnGPU (ssize_t nRowsIn, ssize_t nColsIn = 1, int gpuId = -1);
#endif
   ...
   public:
      ...
      /// Set from a vecref with a different type
      template <class T2, StrideType Layout>
      ...
      {
         SX_CHECK (elements);
         SX_CHECK (this->isRAM ()); // GPU not yet supported
   ...
   : auxData (in.auxData)
{
   ...
   SX_CHECK (in.isRAM ()); // GPU to be implemented
   alloc (in.getSize ());
...
SxVecRef<T,Layout>::operator<<= (const SxVecRef<T,Layout2>& in)
{
   ...
   if (isGPU () || in.isGPU ()) {
      SX_CHECK (getSize () > 0);
      SX_CHECK (getRowStride () == 1, getRowStride ()); // not yet implemented
      SX_CHECK (in.getRowStride () == 1, in.getRowStride ()); // not yet impl.
      if (isContiguous (*this) && isContiguous(in))  {
         memCopyGPU (   elements,    allocMem->memType,
                     in.elements, in.allocMem->memType, sizeof(T) * getSize ());
      } else {
         SX_CHECK (!isPacked(Layout)); // to be implemented
         SX_CHECK (!isPacked(Layout2)); // to be implemented
         SX_CHECK (in.getNRows () == getNRows (), in.getNRows (), getNRows ());
         SX_CHECK (in.getNCols () == getNCols (), in.getNCols (), getNCols ());
         ssize_t stride1 = sizeof(T) * this->getColStride (),
                 stride2 = sizeof(T) *    in.getColStride ();
         memCopyGPU (   elements, stride1,    allocMem->memType,
                     in.elements, stride2, in.allocMem->memType,
                     getNRows () * sizeof(T), getNCols ());
      }
      return *this;
   }
#endif
...
{
   SX_CHECK (isRAM ()); // GPU to be implemented
...
{
   SX_CHECK (vec.getSize () == 0 || vec.isRAM ()); // otherwise: copy to RAM first
...
void SxVecRef<T,Layout>::rotate(const SxVecRef<T,Compact> &rotMat)
{
   ...
   SX_CHECK (isRAM () && rotMat.isRAM ()); // no GPU support yet
...
SxVector<T> SxVecRef<T, Layout>::overlap (const SxVecRef<T, Layout2> &y, ssize_t nSum) const
{
   ...
   SX_CHECK (isRAM () && y.isRAM ()); // no GPU support yet
...
{
   SX_CHECK (getSize () > 0);
   SX_CHECK (in.isRAM ()); // no GPU support yet
...
#ifdef USE_GPU
template<class T>
void SxVector<T>::allocOnGPU (ssize_t nRowsIn, ssize_t nColsIn, int gpuId)
{
   SX_CHECK (nRowsIn > 0, nRowsIn);
   SX_CHECK (nColsIn > 0, nColsIn);
   int memType = (gpuId == -1) ?  SxAllocMem::CurrentGPUMemory
                               : (SxAllocMem::GPUMemory + (gpuId<<8));
   if (allocMem)  {
      if (allocMem->memType == memType
          && getSize () == nRowsIn * nColsIn
          && isOnlyRef ())
      {
         this->reshape (nRowsIn, nColsIn);
         return;
   ...
   allocMem = SxAllocMem::create (sizeof(T) * nRowsIn * nColsIn, memType);
   elements = (T*)allocMem->mem;
   SxVecLayout<Compact>::set (nRowsIn, nColsIn);
}
#endif
...
void SxVector<T>::resize (ssize_t n, bool keep, const T& fillVal)
{
   ...
   SX_CHECK (!this->isGPU ()); // to be implemented
   if (size == n && isOnlyRef ())
   {
...
#ifdef USE_GPU
   if (in.isGPU ())  {
      allocOnGPU (in.getNRows (), in.getNCols (), in.getGPUId ());
      this->operator<<= (in);
      return;
   }
#endif
...
#ifdef USE_GPU
         if (in.isGPU ()) {
            allocOnGPU (in.getSize (), 1, in.getGPUId ());
         } else
#endif
         {
            alloc (in.getSize ());
...
{
   SX_CHECK (getSize () > 0);
   SX_CHECK (this->isRAM ()); // GPU to be implemented?
   T res = elements[0];
...
{
   SX_CHECK (this->isRAM ()); // no GPU support
...
SxVector<T> SxVector<T>::inverse () const&
{
   ...
   SX_CHECK (this->isRAM ()); // no GPU support yet
