...
// SxCubicSpline-class
//------------------------------------------------------------------------------
SxCubicSpline::SxCubicSpline (const SxVecRef<double> &x,
...
SxCubicSpline::SxCubicSpline (const SxVecRef<double> &x,
                              ...
                              double slope, double slope2)
{
   SX_CHECK(x.getSize () == y.getSize ());
   SX_VALIDATE_VECTOR(x);
   SX_VALIDATE_VECTOR(y);
   xVals = x;
   yVals = y;
...
SxCubicSpline::SxCubicSpline (const SxVecRef<double> &xData,
                              ...
                              double slope, double slope2)
{
   SxArray<int> pointsPerSpline = getPointsPerSpline (basis, xData);
   ...
   SX_VALIDATE_VECTOR(xFit);
   SX_VALIDATE_VECTOR(yFit);
   SX_VALIDATE_VECTOR(xVals);
   splineType = splineTypeIn;
   fitType = fitTypeIn;
   setH();
   leftHermite = slope;
   ...
   if (xVals.getSize() != basis.getSize()) {
      SxCubicSpline spline(xVals, yVals, splineType, leftHermite, rightHermite);
      xVals = basis;
                              ...
                              double slope, double slope2)
{
   ...
   SX_VALIDATE_VECTOR(xVals);
   SX_VALIDATE_VECTOR(hVals);
   SX_VALIDATE_VECTOR(yVals);
   splineType = splineTypeIn;
   fitType = None;
   leftHermite = slope;
   rightHermite = slope2;
...
SxVector<double> SxCubicSpline::getYFit () const
{
...
SxVector<double> SxCubicSpline::getY (const SxVecRef<double> &x) const
{
   int dim = (int)x.getSize ();
   SX_CHECK(dim > 0);
   SX_VALIDATE_VECTOR(x);
   SxVector<double> result (dim);
   for(int i = 0; i < dim; i++)   {
      result(i) = getY(x(i));
   }
   return result;
...
SxVector<double> SxCubicSpline::getdYdX (const SxVecRef<double> &x) const
{
   int dim = (int)x.getSize ();
   SX_CHECK(dim > 0);
   SX_VALIDATE_VECTOR(x);
...
void SxCubicSpline::errorIdx (double x, const SxVecRef<double> &basis, int idx)
{
   cout << "Here is SxCubicSpline!" << endl;
   cout << "I get no spline Idx for x, is: " << x << endl;
   cout << "current idx : " << idx << ", but x is ";
   if (idx + 1 >= basis.getSize ())  {
         cout << "larger than " << basis(basis.getSize ()-1) << endl;
         cout << "Somehow ran to upper bound!" << endl;
   } else {
      cout << "not between " << basis(idx) << " and " << basis(idx+1) << endl;
   }
   cout << basis << endl;
   SX_EXIT;
}
...
void SxCubicSpline::computeSpline ()
{
   ...
   //SxVector<double> polyCoeff2 = (TMat.inverse () ^ betaVals);

   // Use trigonal Matrix decomposition
   SxVector<double> polyCoeff2 = symmetricTridiagonalGauss (TMat, betaVals);
...
SxVector<double> SxCubicSpline::calcDRDataDRGrid ()
{
   ...
   SxArray<SxVector<double> > dPolyCoeff (4);
   // da/dy
   dPolyCoeff(0) = dady();
   // dc/dy
...
// least square fit
void SxCubicSpline::computeLeastSquareFit ()
{
   ...
   SxArray<SxVector<double> > q (7);
   SxArray<SxVector<double> > fq (4);
   for (int i = 0; i < 7; i++)   {
      q(i).resize(nSpline);
      q(i).set(0.0);
      if (i < 4)   {
   ...
   for (ssize_t iVal = 0; iVal < nData; iVal++) {
      ...
      double diff = xFit(iVal)-xVals(iSpline);
      for (int i = 0; i < 7; i++)   {
         double powDiff = 1.0;
         for (int j = 1; j <= i; j++)   {
            powDiff *= diff;
   ...
   for (ssize_t iVal = 0; iVal < nData; iVal++) {
      ...
      for(it = splineExtend(iSpline).begin(); it != splineExtend(iSpline).end(); it++)  {
         double diff = xFit(iVal)-xVals(*it);
         for (int i = 0; i < 7; i++)   {
            double powDiff = 1.0;
            for (int j = 1; j <= i; j++)   {
               powDiff *= diff;
   ...
   SxArray<SxArray<SxVector<double> > > R (4);
   ...
   SxVector<double> Mat (nSpline, nSpline);
   Mat.set(0.0);
   for(int iCoeff = 0; iCoeff < 4; iCoeff++)   {
      SxVector<double> dPCDagger = dPolyCoeff(iCoeff).transpose ();
      SxVector<double> RSum = R(iCoeff)(0);
   ...
   // beta = da_k/df_m * fq
   SxVector<double> beta (nSpline);
   beta.set(0.0);
   for(int iCoeff = 0; iCoeff < 4; iCoeff++)   {
      SX_VALIDATE_VECTOR(fq(iCoeff));
      SX_VALIDATE_VECTOR(dPolyCoeff(iCoeff));
      beta += dPolyCoeff(iCoeff).transpose () ^ fq(iCoeff);
   }
...
void SxCubicSpline::setT (const enum SplineType sType)
{
   int nSpline = (int)xVals.getSize();
   // layout:
...
void SxCubicSpline::setBeta (const enum SplineType sType)
{
   int nSpline = (int)xVals.getSize ();
...
SxVector<double> SxCubicSpline::dady ()
{
   ssize_t nSpline = xVals.getSize ();
   SxVector<double> result (nSpline,nSpline);
   result.set(0.0);
   // da_i / dy_j = d_{i,j}
...
SxVector<double> SxCubicSpline::dbdy (const SxVecRef<double> &dA,
                                      const SxVecRef<double> &dC)
{
   ssize_t nSpline = xVals.getSize ();
   SX_CHECK(nSpline == dA.getNRows (), nSpline, dA.getNRows ());
   SX_CHECK(nSpline == dC.getNRows (), nSpline, dC.getNRows ());
   SX_CHECK(nSpline == dA.getNCols (), nSpline, dA.getNCols ());
   SX_CHECK(nSpline == dC.getNCols (), nSpline, dC.getNCols ());
   SxVector<double> result (nSpline, nSpline);
   result.set(0.0);
...
SxVector<double> SxCubicSpline::dcdy (const enum SplineType sType)
{
   SX_CHECK(TMat.getSize() > 0);
   SxVector<double> dBeta = dbetady (sType);
   SxVector<double> result = symmetricTridiagonalGauss (TMat, dBeta);
   return result;
}

SxVector<double> SxCubicSpline::dbetady (const enum SplineType sType)
{
   ssize_t nSpline = xVals.getSize();
   SxVector<double> result (nSpline, nSpline);
   result.set(0.0);
...
SxVector<double> SxCubicSpline::dddy (const SxVecRef<double> &dC)
{
   ssize_t nSpline = xVals.getSize();
   SX_CHECK(nSpline == dC.getNRows (), nSpline, dC.getNRows ());
   SX_CHECK(nSpline == dC.getNCols (), nSpline, dC.getNCols ());
...
void SxCubicSpline::setSpline (const SxVecRef<double> &spline)
{
   SX_CHECK(spline.getSize() > 0);
   SX_CHECK(spline.getSize() % 4 == 0);
   SX_VALIDATE_VECTOR(spline);
...
SxVector<double> SxCubicSpline::getSpline () const
{
   SX_CHECK(polyCoeff.getSize () > 0);
   SX_CHECK(polyCoeff.getSize () % 4 == 0);
   SX_VALIDATE_VECTOR(polyCoeff);
   return polyCoeff;
}

void SxCubicSpline::mirror ()
{
   ssize_t nSpline = xVals.getSize();
   ssize_t nData   = xFit.getSize();
   ...
   if (nData > 0)  {
      ...
      if (fabs(xFit(0)) < 1e-10)   {
         SxVector<double> xMirror(2*nData-1);
         SxVector<double> yMirror(2*nData-1);
         ...
         SxVector<double> xMirror(2*nData);
         SxVector<double> yMirror(2*nData);
   ...
   if (nSpline > 0)  {
      if (fabs(xVals(0)) < 1e-10) {
         SxVector<double> xMirror(2*nSpline-1);
         ...
         SxVector<double> xMirror(2*nSpline);
         // Create symmetric spline mesh
         for (ssize_t i = 0; i < nSpline; i++)  {
...
void SxCubicSpline::deMirror ()
{
   // DeMirror Vals
   // Backup
   ssize_t nSpline = xVals.getSize();
   SxVector<double> xValsBackup = xVals;
   SxVector<double> yValsBackup = yVals;
   if ((nSpline & 1) == 1)   {
      nSpline = (nSpline + 1) / 2;
   ...
   //DeMirror Datapoints
   ssize_t nData = xFit.getSize();
   SxVector<double> xFitBackup = xFit;
   SxVector<double> yFitBackup = yFit;
   nData = nData / 2;
   xFit.resize(nData);
   yFit.resize(nData);
   for (ssize_t i = 0; i < nData; i++)   {
...
SxArray<SxList<int> >
SxCubicSpline::getSplineExtend (SxArray<int> &pointsPerSpline)
{
   ssize_t nSpline = xVals.getSize();
   ssize_t nData = xFit.getSize();
   ssize_t minPoints = 1;
...
SxArray<int> SxCubicSpline::getPointsPerSpline (const SxVecRef<double>& basis,
                                                const SxVecRef<double>& xData)
{

   ssize_t nSpline = basis.getSize();
   ssize_t nData = xData.getSize ();
   SxArray<int> result (nSpline);
   result.set(0);
   for (ssize_t iVal = 0; iVal < nData; iVal++) {
...
SxVector<double>
SxCubicSpline::cleanZeroDepths(const SxVecRef<double>& xData,
                               const SxArray<int> &pointsPerSpline)
{
   ...
   if (fullSplines == nSplines) return xData;
      ...
      SxVector<double> result(fullSplines);
...
SxVector<double>
SxCubicSpline::symmetricTridiagonalGauss (const SxVecRef<double> &Mat,
                                          const SxVecRef<double> &rhs)
{
   SX_CHECK (Mat.getNCols () == 3, Mat.getNCols ());
   SX_CHECK (Mat.getNRows () == rhs.getNRows (), Mat.getNRows (), rhs.getNRows ());
   int dim = (int)Mat.getNRows ();
   SxVector<double> diag2, b;
   diag2.copy (Mat.colRef (1));
   b = rhs.transpose (); // improve data locality...
   b.reshape (rhs.getNCols (), dim);
   SxVector<double> diag1 (dim-1), diag3 (dim - 1);
  ...
  for (int i = 0; i < dim - 1; ++i)  {
     ...
     diag2(i + 1) -= diag1 (i) * x;
     for (int j = 0; j < b.getNRows (); j++) // b is rhs transposed
        b(j,i + 1) -= b(j,i) * x;
     diag3(i) = 0;
  }
...
void SxCubicSpline::print (const SxVecRef<double> &diag1,
                           const SxVecRef<double> &diag2,
                           const SxVecRef<double> &diag3,
                           const SxVecRef<double> &b)
{
  int dim = int(diag2.getSize ());
