...
int main (int argc, char** argv)
{
   ...
   */
   ...
   {
      SxVector<double> sym = exp(-mat.overlap (mat)/sqr(mat(99)));
      ...
      sym.diag () += 1.;
      // temporary version
      if (!sym.inverse ().isHermitian ())  {
         cout << "inverse sym is not Hermitian?" << endl;
         SxVector<double> symI = sym.inverse ();
         cout << (symI - symI.adjoint ()) << endl;
      }
