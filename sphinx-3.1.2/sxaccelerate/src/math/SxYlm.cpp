...
void SxYlm::getPlmArray(int lmax, int m, double x, SxVecRef<double> *out)
{
   SX_CHECK (out);
...
inline void getPlmArray(int lmax, double cos_theta, SxVecRef<double> *out)
{
   SX_CHECK (out);
   SX_CHECK (lmax >= 0, lmax);
...
void SxYlm::getYlmArray(int lmax, double x, double y, double z,
                 SxVecRef<double> *resultPtr)
{
   SX_CHECK (resultPtr);
   SX_CHECK (lmax >= 0, lmax);
   SxVecRef<double> &result = *resultPtr;
#ifndef NDEBUG
   int nl = lmax + 1;
#endif
      ...
      const SxClebschTable &complexClebschGordan)
{
   ...
   SxVector<SxComplex16> D1 = res(1), Dold, D;
