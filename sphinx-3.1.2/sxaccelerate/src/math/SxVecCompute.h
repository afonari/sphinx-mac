...
class SxVecCompute<Simd>
{
   public:
      ...
      */
      ...
      {
         SX_CHECK (x.isRAM () && y.isRAM ()); // no GPU support yet
      ...
      //     For integer types, the division must be done for every element
      ...
      // --- The next 26 lines were generated from snippets/SxVecCompute.h snippet Simd_reduce
      /// Add elements
      template<class T>
      ...
      {
         SX_CHECK (!x.isGPU ()); // no GPU support yet
      ...
      {
         SX_CHECK (x.isRAM ()); // no GPU support yet
      ...
      static inline typename SxVecResult<float, Layout>::VecType
      adjoint (const SxVecRef<float, Layout> &vec)
      {
      ...
      /// Matrix "adjoint" for real-type matrix = transpose
      template<StrideType Layout>
      static inline typename SxVecResult<double,Layout>::VecType
      adjoint (const SxVecRef<double, Layout> &vec)
      {
         return transpose (vec);
      }
