...
case "$cxxname" in
   g++)  # GNU C/C++ Compiler
      ...
      # -----------------------------------------------------------------------
      CXX_REL="-Wall -O3 $GCC_M_ARCH -fpermissive"
      #
      ...
      CXX_REL="$CXX_REL -pipe"
      CXX_REL="$CXX_REL -Wno-variadic-macros"
      # -----------------------------------------------------------------------
      CXX_DBG="-g -O2 -Wall -Wcast-align"
...
#      fi
   ...
   aCC)  # HP-UX aCC Compiler
      ...
      CXX_REL="$CXX_REL +O2 +DA2.0W"
      CXX_REL="$CXX_REL -DUSE_VECLIB -DUSE_VECLIB_FFT"
...
if test $found_compiler = no -a "(" "$ac_test_CXXFLAGS" != "set" -o "CXXFLAGS" = "auto" ")" ; then
   cat << ERRMSG
$SEPARATOR
...
if test  "$ac_test_CXXFLAGS" != "set" -o "$CXXFLAGS" = "auto" ; then
   ...
   AM_CXXFLAGS="$CXXFLAGS $CXX_M_ARCH"
fi
...
# set NDEBUG and _REENTRANT flags
if test "$ac_cv_enable_debug" = "yes"; then
   AM_CXXFLAGS="$AM_CXXFLAGS -D_REENTRANT"
else
   AM_CXXFLAGS="$AM_CXXFLAGS -DNDEBUG -D_REENTRANT"
fi
