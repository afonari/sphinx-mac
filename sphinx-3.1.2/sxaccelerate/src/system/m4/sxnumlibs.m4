...
SX_ARG_ENABLE([flame],    [USE_FLAME], [$SX_PARAM_FLAME],
              [compile package with FLAME/BLIS support])
...
if test x"${ac_cv_enable_netlib}"  = x"yes"; then
   SXCOPY="${SXCOPY}|    - BLAS/LAPACK     http://www.netlib.org\n"
fi
if test x"${ac_cv_enable_flame}"  = x"yes"; then
   SXCOPY="${SXCOPY}|    - FLAME/BLIS      https://github.com/flame\n"
fi
...
for dir in `echo "$ac_cv_with_numlibs" | sed -e's/&.*$//;s/:/ /g'` ; do
   ...
   if test -d $dir/lib; then
      ...
      if test x"${enable_shared}" = x"yes" -a x"$(cd $dir/lib && ls *.la 2>/dev/null)" == x"" ; then
         # if no .la files are present, add library path to rpath
         LDFLAGS="$LDFLAGS -Wl,-rpath,${dir}/lib"
         AC_MSG_NOTICE([Adding dynamic library path $dir/lib])
      fi
   else
...
# add extra libraries
AC_ARG_VAR([NUMLIBS_LIBS],
           AS_HELP_STRING([Additional libraries to link (from numlibs)]))
if test x"$NUMLIBS_LIBS" == x"" ; then :; else
   LIBS="$NUMLIBS_LIBS $LIBS"
   AC_MSG_NOTICE([Adding extra libs: $numlibs_extra_libs])
fi
...
if test x"${ac_cv_enable_mkl}"  = x"yes"; then
   ...
   if echo "${ac_cv_with_mklpath}" | grep -q : ; then
      mklincludepath=`echo "${ac_cv_with_mklpath}" | sed -e's/:.*//'`
      if test -d "$mklincludepath" ; then
        mklincludepath=`cd $mklincludepath; pwd`
      fi
      mkllibpath=`echo "${ac_cv_with_mklpath}" | sed -e's/.*://'`
      if test -d "$mkllibpath" ; then
        mkllibpath=`cd $mkllibpath; pwd`
      fi
      AC_CHECK_FILE([$mklincludepath/mkl.h], [],
                    [AC_MSG_ERROR([Cannot find Intel MKL.  Use the flag --with-mklpath to point to an MKL installation. Use --with-mklpath=<mkl include path>:<mkl lib path> for full flexibility.])])
      AC_CHECK_FILE([$mkllibpath/libmkl_core.so], [],
                    [AC_MSG_ERROR([Cannot find Intel MKL.  Use the flag --with-mklpath to point to an MKL installation. Use --with-mklpath=<mkl include path>:<mkl lib path> for full flexibility.])])
      CPPFLAGS="$CPPFLAGS -I$mklincludepath"
   else
...
if test x"$ac_cv_enable_flame" = x"yes"; then
   AC_DEFINE([USE_NETLIB],[],[use netlib compatible CBLAS/LAPACKE interface])
   AC_CHECK_HEADER([cblas.h],,[AC_MSG_ERROR([Cannot find cblas.h])] )
   AC_CHECK_HEADER([lapacke.h],,[AC_MSG_ERROR([Cannot find lapacke.h])] )
   # --- check if suitable blas library is available
   AC_SEARCH_LIBS([cblas_dnrm2], [blis blas])
   case "$ac_cv_search_cblas_dnrm2" in
     no*) : ;;
     *)   SX_BLAS_LIBS="$ac_cv_search_cblas_dnrm2 $SX_BLAS_LIBS"
   esac
   # --- check for lapack library
   AC_SEARCH_LIBS([LAPACKE_zhpev_work], [flame lapacke], [],
                  AC_MSG_ERROR([Cannot link against LAPACKE library]),
                  [$SX_BLAS_LIBS])
   case "$ac_cv_search_LAPACKE_zhpev_work" in
     no*) : ;;
     *)   SX_BLAS_LIBS="$ac_cv_search_LAPACKE_zhpev_work $SX_BLAS_LIBS"
   esac
   AC_MSG_NOTICE([Added libraries for CBLAS/LAPACKE: $SX_BLAS_LIBS])
fi
...
if test x"$ac_cv_enable_netlib" = x"yes"; then
   ...
   AC_CHECK_HEADER([lapacke.h],,[AC_MSG_ERROR([Cannot find lapacke.h])] )
   # --- check if suitable blas library is available
   AC_SEARCH_LIBS([dnrm2_], [blas])
   case "$ac_cv_search_dnrm2_" in
     no*) : ;;
     *)   SX_BLAS_LIBS="$ac_cv_search_dnrm2_ $SX_BLAS_LIBS"
   esac
   # --- check for cblas library
   AC_SEARCH_LIBS([cblas_dnrm2], [cblas], [],
   ...
   # --- check for lapack library
   AC_SEARCH_LIBS([zhpev_], [lapack], [], [], [$SX_BLAS_LIBS])
   case "$ac_cv_search_zhpev_" in
     no*) : ;;
     *)   SX_BLAS_LIBS="$ac_cv_search_zhpev_ $SX_BLAS_LIBS"
   esac
   # --- check for LAPACKE library
   AC_SEARCH_LIBS([LAPACKE_zhpev_work], [lapacke], [],
                  AC_MSG_ERROR([Cannot link against LAPACKE library]),
                  [$SX_BLAS_LIBS])
   case "$ac_cv_search_LAPACKE_zhpev_work" in
     no*) : ;;
