...
void SxLocpot::write (const SxString &filename,
                      const SxMeshR &pot,
                      const SxAtomicStructure &structure)
{
   SxTextIO out (filename);
   SX_CHECK (pot.getBasisPtr ());
   const SxRBasis &R = pot.getBasis<SxRBasis> ();
   const SxMesh3D &mesh = R.fft3d.mesh;
   SxPoscar::write (out.getFp (), structure, structure.getElements ());
   out.printf ("%d %d %d\n", mesh(0), mesh(1), mesh(2));
   int iMesh = 0;
   SxVector3<int> x;
   for (x(2) = 0; x(2) < mesh(2); x(2)++)
      for (x(1) = 0; x(1) < mesh(1); x(1)++)
         for (x(0) = 0; x(0) < mesh(0); x(0)++, iMesh++) {
            ssize_t ir = R.fft3d.mesh.getMeshIdx(x, SxMesh3D::Positive);
            out.printf ("% 12.8e%c", pot(ir), iMesh % 5 == 4 ? '\n' : ' ');
         }

   if (iMesh % 5 != 0) out.printf ("\n");
}
