...
int main (int argc, char **argv)
{
   ...
   int locpotGroup = cli.newGroup ("vasp LOCPOT files");
   cli.excludeGroup (sxGroup);
   cli.excludeGroup (pdbGroup);
   cli.excludeGroup (binGroup);
   cli.excludeGroup (vaspGroup);
   bool locpotFile = cli.option ("--locpot","input file is VASP LOCPOT file")
      .toBool ();
   ...
   // standard output options (-o, --outsxb, --printsym)
   ...
   if (sxbFile)  {
      structure = structureFromInput (inFile, true);
   } else if (pdbFile) {
...
#  endif
      ...
      structure = structureFromInput (inFile, false, keepMovable);
   }
