...
SxVector<SxComplex16> nice (const SxVector<SxComplex16> &in)
{
   SxVector<SxComplex16> res = in; // copy
...
rotateAoMatrix (const SxVector<SxComplex16> &S,
                const SymMat &sym,
                const SxAOBasis &aoBasis,
                const SxAtomicStructure &structure,
                const SxGrid &grid,
                const SxArray<SxVector<double> > &ylmRot,
                const Coord &k)
{
   Coord rotK = sym ^ k;
   int nOrb = aoBasis.getNOrb ();

   SxVector<SxComplex16> rotS(nOrb, nOrb);
   ...
   for (int iOrb = 0; iOrb < nOrb; /* done inside */)  {
      ...
      // --- collect the rows
      SxVector<SxComplex16> rowsT = S.colsRef(iOrb, nl).conj ();

      // rotate the rows
      if (l > 0)
         rowsT = rowsT ^ SxVector<SxComplex16> (ylmRot(l).transpose ());
      if (l & 1)
      ...
      rotS.colsRef (iOrbRot, nl) <<= rowsT;

      iOrb += nl;
   }
   // exchange rows and columns
   SxVector<SxComplex16> rotS2 = rotS.transpose ();
   // --- rotate the columns
   for (int iOrb = 0; iOrb < nOrb; /* done inside */)  {
      int is = aoBasis.orbitalMap(iOrb).is;
      int ia = aoBasis.orbitalMap(iOrb).ia;
      ...
      // --- collect the columns
      SxVecRef<SxComplex16> cols;
      cols = rotS2.colsRef (iOrb, nl);

      // rotate the columns
      SxVector<SxComplex16> colsRot;
      if (l > 0)
         colsRot = cols ^ SxVector<SxComplex16>(ylmRot(l).transpose ());
      else
         colsRot = cols; // copy
      if (l & 1)
      ...
      rotS.colsRef (iOrbRot, nl) <<= colsRot;
...
SxVector<SxComplex16>
aoMatrixRtoK (const SxArray<SxVector<SMatType> > AinR,
              const SxAtomicStructure &structure,
...
void switchOffInteraction (const SxArray<SxArray<int> > &blockInteraction, 
                           ...
                           SxVector<SMatType> &SinR,
                           SxVector<SMatType> &HinR,
         ...
         for (int io = offsetI; io < offsetI + noI;)  {
            ...
            for (int jo = offsetJ; jo < offsetJ + noJ;)  {
               ...
               if ( (iSpecies == blockInteraction(iBlock)(0) 
                  ...
                  // Switchoff Overlap S 
                  SinR.blockRef(io, jo, nOrbsI, nOrbsJ).set (0.);
                  // Switchoff Hamiltoncontribution H
                  HinR.blockRef(io, jo, nOrbsI, nOrbsJ).set (0.);
               }
...
void writeMatElems (SxBinIO &io, SxArray<SxVector<SMatType> > &SinR,
                    SxArray<SxVector<SMatType> > &HinR,
                    SxAtomicStructure &structure,
                    SxArray<Coord> &vecR, SxAOBasis &aoBasis,
                    SxMesh3D &meshR, int iSpecies)
{
   ...
   SxVector<double> dist (nNeighbors,3);
   SxVector<int> neighborSpecies (nNeighbors);
   SxVector<SMatType> SElems (nNOrbitals, nOrbs), HElems(nNOrbitals, nOrbs);
   SxVector<double> RElems(3 * nNeighbors,3);
      ...
      for (int iR = 0; iR < vecR.getSize(); iR++)  {
         ...
         for (int js = 0; js < structure.getNSpecies (); js++)  {
            ...
            for (int ja = 0; ja < structure.getNAtoms (js); 
               ...
               SX_LOOP2(j(3), i(3))
                  RElems(3 * iNeighbor + i, j) = RotMat(i,j);

               // --- collect transpose(!) of blocks
               for (ssize_t j = 0; j < noJ; ++j)  {
                  for (ssize_t i = 0; i < noI; ++i)  {
                     SElems(writeOffset + i, j) = SinR(iR)(offsetJ + j, offsetI + i);
                     HElems(writeOffset + i, j) = HinR(iR)(offsetJ + j, offsetI + i);
                  }
               }
            }
         }
...
void printNice (const SxVecRef<SMatType,Layout> &block)
{
   sxprintf("[");
   for (int iRow = 0; iRow < block.getNRows (); iRow++)  {
      if (iRow > 0) sxprintf(",\n");
      sxprintf("[%.8f",block(iRow,0));
      for (int iCol = 1; iCol < block.getNCols (); iCol++)  {
...
int main (int argc, char **argv)
{
   ...
   SxKPoints kPoints (hamSolver.structure.cell,
                      SxList<SxVector3<PrecG> > () << Coord (0,0,0),
   ...
   if (aoFile.getSize () == 0)  {
      aoPtr = aoPtr.create (gkBasis, pseudoPsi);
   } else {
      try {
         ...
         lmax = orbs.getLMax ();
         aoPtr = aoPtr.create (gkBasis, orbs.getMuSet ());
      } catch (SxException e) {
         e.print ();
         SX_EXIT;
      }
   ...
   if (interactionFile.getSize () != 0)  {
      ...
      SxParser::Table blockerTable
         = parser.read (interactionFile, "std/interactionBlocker.std");
      try   {
   ...
   SxArray<SxVector<SMatType> > SinR(nR);
   SxArray<SxVector<SMatType> > HinR(nR);
   SxArray<SxVector<SMatType> > UinR(nR);
   SxClebschTable complexClebschTable;
   SxArray<SxVector<double> > Dmm, Dmm2;
   ...
   for (int ik = 0; ik < nk; ++ik)  {
      ...
      SxVector<SxComplex16> Sk(nOrb, nOrb);
      SxVector<SxComplex16> Hk(nOrb, nOrb);
      SxVector<SxComplex16> U(nOrb, nOrb);
      U.set(0.0);
      {
         SX_CLOCK (Ham);
      ...
      if (printS)  { 
         if (iSpin == 0)  {
            cout << "S: " << symEigenvalues(Sk) << endl;
            cout << "S=" << nice(Sk) << endl;
         }
         cout << "H: " << symEigenvalues(Hk) << endl;
         cout << "H=" << nice(Hk) << endl;
      }
      ...
      if (scissor)  {
         cout << "Perform scissorshift with U = " << scissorU 
              ...
              << "." << endl;
         SxVector<SxComplex16> L;
         L = Sk.choleskyDecomposition (UpperRight).inverse ();
         SxSymEigensystem<SxComplex16> eig (L.adjoint () ^ Hk ^ L);
         cout << "E: " << HA2EV * eig.vals << endl;
         for(int iRow = 0; iRow < nOrb; iRow++)  {
            if ((eig.vals(iRow) * HA2EV >= scissorRangeLower) &&
                (eig.vals(iRow) * HA2EV<= scissorRangeUpper))
      ...
      for (int iSym = 0; iSym < nSym; ++iSym)  {
         ...
         SxVector<SxComplex16> rotH;
         SxVector<SxComplex16> rotU;
               ...
               for (int js = 0; js < structure.getNSpecies (); ++js)  {
                  ...
                  for (int ja = 0; ja < structure.getNAtoms (js); ++ja)  {
                     ...
                     if ( (d.norm() > neighborCut) && (neighborCut > 0.0) )  {
                        SinR(iR).blockRef (offsetI, offsetJ, noI, noJ).set (0.);
                        HinR(iR).blockRef (offsetI, offsetJ, noI, noJ).set (0.);
                     }

                     if (scissor && d.norm() < dCutoff) {
                        cout << "Apply Correction!" << endl;
                        HinR(iR).blockRef (offsetI, offsetJ, noI, noJ)
                           += UinR(iR).blockRef (offsetI,offsetJ,noI,noJ);
                     }
                     ...
                     if (write)  {
                        ...
                        cout << "S = ";
                        printNice (SinR(iR).blockRef(offsetI, offsetJ, noI, noJ));
                        cout << "H = ";
                        printNice (HinR(iR).blockRef(offsetI, offsetJ, noI, noJ));

                        //Perform rotation
                        for (int io = offsetI; io < offsetI + noI;)  {
                           int iL = aoPtr->refOrbMap(is)(io-offsetI).l;
                           int nOrbsI = 2*iL+1;
                           SxVecRef<SMatType> rotLeft = Dmm(iL);
                           for (int jo = offsetJ; jo < offsetJ + noJ;)  {
                              int jL = aoPtr->refOrbMap(js)(jo-offsetJ).l;
                              int nOrbsJ = 2*jL+1;
                              SxVector<SMatType> rotRight = Dmm(jL).transpose ();
                              // Rotate S 
                              SxVecRef<SMatType, SubMatrix> block
                                 = SinR(iR).blockRef(io, jo, nOrbsI, nOrbsJ);
                              block <<= (rotLeft ^ block ^ rotRight);
                              block.unref ();
                              //Rotate H
                              block = HinR(iR).blockRef(io, jo, nOrbsI, nOrbsJ);
                              block <<= (rotLeft ^ block ^ rotRight);
                              jo += nOrbsJ;
                           }
                           io += nOrbsI;
   ...
   */
   ...
   for (int ik = 0; ik < kPoints.getNk (); ++ik)  {
      ...
      L = Sk.choleskyDecomposition (UpperRight).inverse ();
      cout << "E = "
           << symEigenvalues (L.adjoint () ^ Hk ^ L) * HA2EV
           << endl;
   }
   */
   if ((kFile.getSize () > 0) && !write)  {
      ...
      for (int ik = 0; ik < bs.getNk (); ++ik)  {
         ...
         Sk = aoMatrixRtoK (SinR, structure, meshR, aoBasis, kVec);
         Hk = aoMatrixRtoK (HinR, structure, meshR, aoBasis, kVec);
         int nr = int(Sk.getNRows ());
         if (printS)  {
            cout << SX_SEPARATOR;
            cout << "ik=" << ik << endl;
            bs.getK(ik).print();
            SxSymEigensystem<SxComplex16> seig (Sk);
            cout << "S: " << endl; 
            seig.vals.print(true);
            cout << SX_SEPARATOR;
            if (seig.vals (0) < 1e-12)  {
               int i = 0;
               while (seig.vals(i) < 1e-12) i++;
               SxVecRef<SxComplex16> nonSing = seig.vecs.colsRef (i, nr - i);
         ...
         L = Sk.choleskyDecomposition (UpperRight).inverse ();
         Hk = L.adjoint () ^ Hk ^ L;
         SxVector<double> eps = symEigenvalues (Hk);
         SX_STOP_TIMER (Diag);
         epsFile << (ik+1);
