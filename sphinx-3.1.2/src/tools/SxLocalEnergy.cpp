...
typedef SxArray<SxArray<SxVector<SxComplex16> > > SxExpCoeffs;
...
void matPrint (const SxVector<double> &mat)
{
   ssize_t nRows = mat.getNRows ();
   ssize_t nCols = mat.getNCols ();
...
SxVector<double> createMask (SxPAWRho &projRho,
                               SxPAWRho &fullRho,
                               ...
                               const SxVector<int> &map)
{
   ...
   SxVector<double> result (projRho.pwRho(0).getSize ());
   result.set(0.0);

   SxVector<double> rhoProjR = (nSpin == 1) ? 1.0 * projRho.pwRho.rhoR(0)
                               : projRho.pwRho.rhoR(0) + projRho.pwRho.rhoR(1);
   SxVector<double> rhoFullR = (nSpin == 1) ? 1.0 * fullRho.pwRho.rhoR(0)
...
SxArray<SxVector<int> >
atomicSpaceRegion (const SxRBasis *rPtr, const SxAtomicStructure &structure)
{
...
double getSpaceNorm (const SxPW &proj, const SxPW &ref, SxPtr<SxPAWOverlap> SPtr)
{
   ...
   for (int ik = 0; ik < nk; ik++)  {
      ...
      for (int iSpin = 0; iSpin < nSpin; iSpin++)  {
         SxVector<SxComplex16> SRefSet = SPtr->apply(ref(iSpin, ik));
...
SxPW wavesDiff(const SxPW &waves1,const SxPW &waves2)
{
   ...
   SxPW result(nStates, nSpin, gkBasisPtr, /* nComp */ 1, tmpDir);
...
SX_CLOCK(Timer::ExpansionCoeffs);
   ...
   for (int ik = 0; ik < nk; ik++)  {
      if (!SxLoopMPI::myWork(ik)) continue;
      const SxVector<SxComplex16> &Sinv = aoBasis.getInverseOverlap(ik);
      for (int iSpin = 0; iSpin < nSpin; iSpin++)  {
         const PsiRef &psiG = waves(iSpin,ik);
         SxVector<SxComplex16> muPsiG = (aoBasis | psiG);
                   ...
                   const SxVector<int> &map)
{
   ...
   SxPW projWaves (nStates, nSpin, gkBasisPtr, /* nComp */ 1, tmpDir);
   for (int ik = 0; ik < nk; ik++)  {
      if (!SxLoopMPI::myWork(ik)) continue;
      for (int iSpin = 0; iSpin < nSpin; iSpin++)  {
                     ...
                     const SxVector<int> &map)
{
   ...
   for (int ik = 0; ik < nk; ik++)  {
      ...
      for (int iSpin = 0; iSpin < nSpin; iSpin++)  {
         for (int iState = 0; iState < nStates; iState++)  {
            ...
            if (f > 1e-12)  {
               PsiR psiFullR = R | fullPsi(iState,iSpin,ik);
               PsiR psiProjR = R | projPsi(iState,iSpin,ik);
               result.pwRho(iSpin)
   ...
   SxArray<SxMeshG> rhoCorePSG(nSpecies);
                 ...
                 const SxVector<int> &map)
{
   ...
   SxVector<double> UMat(nAtoms, maxNeighbors);
   UMat.set(0.0);
   for (int iAtomI = 0; iAtomI < nAtoms; iAtomI++)  {
      int ia;
      int is = structure.getISpecies(iAtomI, &ia);
      ...
      for (int iAtomJ = 0; iAtomJ < nNeighbors; iAtomJ++)  { 
         ...
         // Ref. 1 Eq. (28): kernel
         SxVector<double> delta(sqr(lmaxI+1),sqr(lmaxJ+1));
                    ...
                    double charge)
{
   ...
   SxVector<double> YlmGl(gBasis.ng, sqr(lMaxAll + 1));
   ...
   // get gRl and YlmGl
   SxArray<SxArray<SxVector<double> > > gRl (nSpecies);
   for (int is = 0; is < nSpecies; is++)  {
   ...
   */
   ...
   vDipole.set (0.);
   if (dipoleCorr)  {
      SxMeshR rhoHartreeRTot = rBasis | rhoHartreeSoftTot;
      SxMeshR rhoHartreeRAtom = rBasis | rhoHartreeSoftAtom;
      dipoleCorr->update (rBasis | rhoTotG, rhoHartreeRTot);
   ...
   if ((map.getSize() == structure.getNAtoms()))  {
      ...
      vHartreeHard(noZero) = FOUR_PI * rhoHartreeHard(noZero)
         / gBasis.g2(noZero);
      RhoR vElStat(1);
      vElStat(0) = (rBasis | vHartreeHard) + vDipole;
                    ...
                    const SxVector<int> &map)
{
   ...
   vBarTotal.set(0.0);
   for (int is = 0; is < nSpecies; is++)
      vBarTotal += (gBasis | pawPot.vBar(is)) * gBasis.getStructureFactor (is);
                  ...
                  const SxVector<int> &map)
{
   ...
   if (xcPtr->xcBasisPtr 
      ...
      for (int iSpin=0; iSpin < nSpin; iSpin++)  {
         ...
         rhoXcRAtom(iSpin) = *xcPtr->xcBasisPtr | (G | rhoXcRAtom(iSpin));
         rhoXcRTot(iSpin)  = *xcPtr->xcBasisPtr | (G | rhoXcRTot(iSpin) );
      }
   }
   ...
   for (size_t i = 0; i < dim; i++)  {
      if (xcPtr->xcFunctional == SxXC::LDA)  {
         ...
         SxVector3<PrecRhoR> gradRhoUp, gradRhoDown;
         SxVector3<PrecRhoR> gradRhoTl;
...
int main (int argc, char **argv)
{
   ...
   SxPtr<SxPAWOverlap> SPtr = SxPtr<SxPAWOverlap>::create (pBasis);
   ...
   try {
      ...
      orbs.setup(aoGroup);
      aoPtr = SxPtr<SxAOBasis>::create (*gkBasisPtr, orbs.getMuSet (), SPtr);
   } catch (SxException e) {
      e.print ();
      SX_EXIT;
   }
   ...
   for (int ik = 0; ik < nk; ik++)  {
      if (!SxLoopMPI::myWork(ik)) continue;
      for (int iSpin = 0; iSpin < nSpin; iSpin++)  {
         const SxVecRef<SxComplex16> &PsiPWSet = waves(iSpin, ik);
         const SxVecRef<SxComplex16> &PsiQuamolSet = fullWavesQ(iSpin, ik);
         SxVector<SxComplex16> SPsiPWSet = SPtr->apply(waves(iSpin, ik));
         for (int iState = 0; iState < nStates; iState++)  {
   ...
   for (int iAtom = -1; iAtom < nAtoms; iAtom++)  {
      ...
      if (compute)  {
         ...
         SxVector<double> filter = createMask (projRhoQ, fullRhoQ, atomicSpace, map);
         ...
         for (int iSpecies = 0; iSpecies < nSpecies; iSpecies++)  {
            const SxVector<double> &r = rad.radFunc(iSpecies);
