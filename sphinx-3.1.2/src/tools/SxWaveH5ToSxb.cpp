...
int main (int argc, char **argv)
{
   ...
   SxMatrix3<double> cellMat = waves.getMatrix3<double>("primitive_vectors");
   SxVector3<int> meshDims = waves.getVector3<int>("grid_size");
   ...
   SxVector<double> weightsVec = waves.getVector<double>("weights",nk);
   ...
   SxArray<SxArray<SxVector<SxComplex16> > > coeffs (nk);
   ...
   SxVector<SxComplex16> waveStore (mesh.getSize ());
   for (int ik = 0; ik < nk; ik++)   {
   ...
   planeWaves.writeWavesFile (wavesOut,fermi,structure,chemNames);
   ...
   for (int ik = 0; ik < nk; ik++)  {
      for (int iSpin = 0; iSpin < nSpins; iSpin++)  {
         for (int iState = 0; iState < nStates; iState++)  {
            ...
            const SxVecRef<PrecCoeffG> &pw = planeWaves(iState,iSpin,ik);
