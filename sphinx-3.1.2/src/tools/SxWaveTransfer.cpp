   ...
   try  {
      ...
      for (ik=0; ik < nk; ik++)  {
         ...
         if (io.ncMode == SxBinIO::WRITE_DATA)  {
            mesh.set (-1);
            ng = oldGk(ik).ng;
            SxVector<PrecFFTIdx>::Iterator n123It = oldGk(ik).n123(0).begin ();
         ...
         for (iSpin=0; iSpin < nSpin; iSpin++)  {
            for (i=0; i < nPerK (ik); i++)  {
               if (io.ncMode == SxBinIO::WRITE_DATA)  {
                  ...
                  in.readVec (psiVarNameRead, &psiOld, oldGk(ik).ng, inOffset);
                  // --- remap it
               ...
               // write psi
               io.writeVec (psiVarNameWrite, psi, coeffDimNameWrite, offset);
...
int main (int argc, char** argv)
{
   ...
   if (command == "check" || command == "reorder")  {
      ...
      for (int ik = 0; ik < nk; ++ik)  {
         ...
         if (inG->ng != waveG->ng)  {
            ...
            SxVector<PrecFFTIdx>::Iterator a = inG->n123(0).begin (),
   ...
   if (command == "reorder" || command == "change")  {
      ...
      reorder.writeWavesFile (wavesOut, fermi, structure);
   }
   ...
   if (command == "resize")  {
      ...
      // output
      waves.writeWavesFile (wavesOut, fermi, structure);
   ...
   if (command == "select")  {
      ...
      fermi.resize (nStatesNew);
      wavesNew.writeWavesFile (wavesOut, fermi, structure);
   }
   ...
   if (command == "propagate")  {
      ...
      for (int ik = 0; ik < waves.getNk (); ik++)  {
         ...
         SxVector3<double> oldKRel = oldRecCell.carToRel(gkOrigPtr->kVec(ik));
         SxVector3<double> newKRel = newRecCell.carToRel(gkPtr->kVec(ik));
         ...
         for (int iSpin = 0; iSpin < waves.getNSpin (); iSpin++)  {
            for (int iState = 0; iState < waves.getNStates (ik); iState++)  {
               PsiRef psiOld = waves(iState,iSpin,ik);
               PsiRef psiNew = wavesNew(iState,iSpin,ik);
               psiNew <<= (*gkPtr)(ik).transferWaves (psiOld);
            }
            SPtr->orthonormalize(&wavesNew(iSpin,ik));
         }
      }
      wavesNew.writeWavesFile (wavesOut, fermi, newStruct);
   }
   return 0;
