...
SxVector<double> computeRhoCore (SxPAWPot &pawPot, SxPAWRho &pawRho,
      SxAtomicStructure &structure)
{
   ...
   SxVector<SxComplex16> rhoCoreG(0.0, G);
   for (int is = 0; is < nSpecies; is++)  {
      const SxVector<double> &rhoPseudoCore = pawPot.rhoCorePS(is);
      //rhoPseudoCore.auxData.is = char(is);
      SX_CHECK (rhoPseudoCore.auxData.l == 0, (int)rhoPseudoCore.auxData.l);
      SX_CHECK (rhoPseudoCore.auxData.m == 0, (int)rhoPseudoCore.auxData.m);
      SxVector<SxComplex16> rhoPseudoCoreG = (G | rhoPseudoCore);
      rhoCoreG += G.getStructureFactor (is) * rhoPseudoCoreG;
   }
   
   SxVector<double> result = R.symmetrize(R | rhoCoreG);
...
SxVector<double> computeNPseudoCore (SxPAWPot &pawPot,SxAtomicStructure &structure)
{
   ...
   for (int is = 0; is < nSpecies; is++)  {
      const SxVector<double> &r = rad.radFunc(is);
...
int main (int argc, char **argv)
{
   ...
   SxArray<SxCubicSpline> coreAE(nSpecies);
   SxVector<double> rhoPseudoTot (R);
   SxVector<double> rhoPseudoCore (R);
   SxVector<double> rhoCompensation (R);
   SxVector<double> rhoPseudoValence (R);
   ...
   if (table->containsGroup("pseudoPot"))   {
      ...
      for (int iSpecies = 0; iSpecies < nSpecies; iSpecies++)  {
         separator.PAWDist(iSpecies) = potPtr->rPAW(iSpecies);
   ...
   SxArray<SxVector<double> > voronoi = separator.voronoi (R);
