      ...
      SxConstPtr<SxOverlapBase> SPtrIn)
{
   ...
   radials = SxAtomicOrbitals (muSet,radGBasisPtr,0);
...
void SxQuamol::computeKineticEnergy (const SxAtomicOrbitals &functions, bool calcGrad)
{
      ...
      bool calcGrad)
{
   ...
   for (int iSpecies = 0; iSpecies < nSpecies; iSpecies++)  {
      ...
      for (int iot = 0; iot < nOrbTypes; iot++)  {
         ...
         if (calcGrad)  {
            ...
            muSetG(iSpecies)(iot).auxData.setAtom (muG.auxData);
   ...
   if (calcGrad)  {
      locGrad = SxAtomicOrbitals (muSetG,functions.getRadGBasisPtr (),0);
   } // calcGrad
}
      ...
      bool calcGrad)
{
   ...
   SxAtomicOrbitals radR = getOrbitals (functions, radRBasisPtr);

   int nSpecies = radials.getNSpecies ();
   
   SxAtomicOrbitals locGradR = 0.0 * radR;
   ...
   if (calcGrad)  {
      locGrad = SxAtomicOrbitals (muSetG,radGBasisPtr,0);
   } // calcGrad
}
*/

SxAtomicOrbitals SxQuamol::calcNumericGrad (const SxAtomicOrbitals &functions,
      double h)
{
   SX_CLOCK(Timer::calcNumericGrad);
   SxAtomicOrbitals trial = 1.0 * functions;
   SxAtomicOrbitals result = 0.0 * functions;
...
void SxQuamol::compute ()
{
   if (checkGrad)  {
      ...
      SxAtomicOrbitals numericGrad = calcNumericGrad (radials);
   ...
   SxAtomicOrbitals dir = 1.0 * grad;
   SxAtomicOrbitals oldGrad = 1.0 * grad;
   ...
   while (((step <= maxSteps) && !converged) || doNotStop) {
      ...
      SxAtomicOrbitals dirNorm = 1.0 * dir;
...
double SxQuamol::lineMin (const SxAtomicOrbitals &dir,
                          ...
                          SxVector3<double> &y)
{
   ...
   for (int i = 0; i < 3; i++)  {
      if (y(i) < 0)  {
         SxAtomicOrbitals trial = cos(x(i)) * radials + sin(x(i)) * dir;
         computeFunctional(trial, false, false);
         y(i) = functionalValue;
      }
   }
   ...
   if (printLine)  {
      ...
      for (int i = 0; i < dim; i++)  {
         nX(i) = i * delta;
         SxAtomicOrbitals trial = cos(nX(i)) * radials + sin(nX(i)) * dir;
         computeFunctional(trial, false, false);
...
SxOrbitals SxQuamol::expandRadialsG (const SxAtomicOrbitals &functions, int ik)
{
...
void SxQuamol::checkTrafo (const SxAtomicOrbitals &functionsIn)
{
   ...
   cout << SX_SEPARATOR;
   SxAtomicOrbitals functions = 1.0 * functionsIn;
   int nk = wavesPtr->getNk ();
   int nSpecies = functions.getNSpecies ();
   ...
   for (int iSpecies = 0; iSpecies < nSpecies; iSpecies++ )  {
      ...
      for (int iot = 0; iot < nOrbTypes; iot++)  {
         ...
         function.auxData.setAtom (iSpecies, -1);
         function.auxData.setOrb (iot, l, NONE_M);
         double norm = radGBasis.integrate(function * function);
         cout << "iSpecies = " << iSpecies
              << ", iot = " << iot
              << ", norm = " << norm << endl;
   ...
   SxAtomicOrbitals GkTrafo(muSet,radGBasisPtr,0);
   ...
   SxAtomicOrbitals RadRTrafo(muSet,radGBasisPtr,0);
      ...
      SxConstPtr<SxRadialBasis> radRBasisPtr)
{
   ...
   SxAtomicOrbitals result (muSet,radRBasisPtr,0);
   
   return result;
}

SxAtomicOrbitals SxQuamol::setOrthogonal (
      const SxAtomicOrbitals &functions,
      const SxAtomicOrbitals &referenz)
{
   SxAtomicOrbitals result = 1.0 * functions;
...
void SxQuamol::completenessProfile (const SxAtomicOrbitals &functions)
{
   SxConstPtr<SxRadialBasis> radGBasisPtr = functions.getRadGBasisPtr ();
   int nGPoints = (int)radGBasisPtr->getRadFunc().getSize();
   double gMax =  radGBasisPtr->getRadFunc()(nGPoints-1);
   ...
   for (int is = 0; is < radials.getNSpecies(); is++)  {
      ...
      for (int l = 0; l <= radR.getLMax(is); l++)  {
         ...
         for (int i = 0; i < xDim; i++)  {
            ...
            probeGauss.auxData.setAtom (is, -1);
            probeGauss.auxData.setOrb (-1, l, NONE_M);
            for (int iot = 0; iot < nOrbTypes; iot++)  {
...
void SxQuamol::getNormContribution (const SxAtomicOrbitals &functions,
      SxConstPtr<SxRadBasis> radBasisPtr)
{
   ...
   for (int is = 0; is < nSpecies; is++)  {
      ...
      for(int l = 0; l <= lMax; l++)  {
         ...
         for (int iot = 0; iot < eig.vals.getSize(); iot++)  {
            ...
            newRadial.auxData.setAtom (is, -1);
            newRadial.auxData.setOrb (iot, l, NONE_M);
   ...
   SxAtomicOrbitals resultG (muSet,functions.getRadGBasisPtr (),0);
...
void SxQuamol::printSpillagePerState (const SxAtomicOrbitals &functions)
{
...
void SxQuamol::calcResidues (const SxAtomicOrbitals &functions)
{
...
int main (int argc, char** argv)
{
   ...
   if (quamolGroup->containsGroup("basis"))  {
      ...
      radBasisPtr = initGuess.getRadBasisPtr ();
