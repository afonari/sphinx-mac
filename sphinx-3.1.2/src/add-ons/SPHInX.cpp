...
int SPHInX::compute ()
{
   ...
   try  {
      if (table->containsGroup ("basis"))  {
         ...
         if (SxExxSolver::isExx(cmd))
            potential = new SxExxSolver (str, &*table);
         else  {
            SxHamSolver* hamSolver = new SxHamSolver (str, &*table);
            potential = hamSolver;
            // --- register input add-ons
            hamSolver->addCommand("pDos", computePDOS);
         }
...
bool setupParallelHierarchy (SxParallelHierarchy &ph)
{
   if (SxFile ("parallelHierarchy.sx").exists ())
   {
      buildHierarchy("parallelHierarchy.sx", ph);
      return true;
   }
   return false;
}
...
int main (int argc, char **argv)
{
   ...
   SxParallelHierarchy::setupRoutine = &setupParallelHierarchy;
   getMPIHierarchy ();
...
#ifdef USE_LOOPMPI
   writeParallelHierarchy ("parallelHierarchy.sx.actual");
