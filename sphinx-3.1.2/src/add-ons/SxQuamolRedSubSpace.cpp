...
void SxQuamolRedSubSpace::set (
      SxString basisFile,
      const SxAtomicOrbitals &initGuess,
      SxConstPtr<SxRadBasis> radBasisPtrIn,
      SxPW &waveIn,
      SxGkBasis *GkPtrIn,
      SxConstPtr<SxRadialBasis> radGBasisPtrIn,
...
SxArray<SxVector<SxComplex16> > SxQuamolRedSubSpace::getOverlapB ()
{
   SX_CLOCK(Timer::overlapB);
   int nk = waves.getNk ();
   SxArray<SxVector<SxComplex16> > result(nk);
   for(int ik = 0; ik < nk; ik++)   {
      SxVector<SxComplex16> Snu = SPtr->apply(basisGk(ik));
      result(ik) = basisGk(ik).overlap (Snu);
   }

   return result;
}

SxArray<SxVector<SxComplex16> > SxQuamolRedSubSpace::getCoefficients ()
{
   int nk = waves.getNk ();
   SxArray<SxVector<SxComplex16> > result(nk);
   ...
   for(int ik = 0; ik < nk; ik++)   {
      SxVector<SxComplex16> Snu = SPtr->apply(orbitalsC(ik));
      SxVector<SxComplex16> Pbc = basisGk(ik).overlap (Snu);
      result(ik) = Sb(ik).inverse () ^ Pbc;
   }

   return result;
}
SxArray<SxVector<SxComplex16> > SxQuamolRedSubSpace::getOverlapC ()
{
   int nk = waves.getNk ();
   SxArray<SxVector<SxComplex16> > result(nk);
...
SxArray<SxArray<SxVector<SxComplex16> > > SxQuamolRedSubSpace::getProjections ()
{
   SX_CLOCK(Timer::projections);
   int nk = waves.getNk ();
   int nSpin = waves.getNSpin ();
   SxArray<SxArray<SxVector<SxComplex16> > > result(nk);

   for(int ik = 0; ik < nk; ik++)   {
      result(ik).resize(nSpin);
         SxVector<SxComplex16> Snu = SPtr->apply(basisGk(ik));
      for(int iSpin = 0; iSpin < nSpin; iSpin++)   {
         result(ik)(iSpin) = waves(iSpin,ik).overlap (Snu);
      }
   }
   return result;
}
...
double SxQuamolRedSubSpace::calcNormB () 
{
   ...
   double result = 0.0;
   for (int ik = 0; ik < nk; ik++)   {
      SxVector<SxComplex16> invSb = Sb(ik).inverse();
      for (int iSpin = 0; iSpin < nSpin; iSpin++)   {
...
double SxQuamolRedSubSpace::calcNormC () 
{
   ...
   double result = 0.0;
   SxArray<SxVector<SxComplex16> > Sc = getOverlapC ();
   for (int ik = 0; ik < nk; ik++)   {
      SxVector<SxComplex16> invSc = Sc(ik).inverse();
      SxVector<SxComplex16> betaDagger = beta(ik).adjoint();
      for (int iSpin = 0; iSpin < nSpin; iSpin++)   {
...
// Calculate Gradient
SxArray<SxVector<SxComplex16> > SxQuamolRedSubSpace::calcGradient ()
{
   ...
   for (int ik = 0; ik < nk; ik++)   {
      result(ik).reformat (beta(ik));
      result(ik).set(0.0);
      SxVector<SxComplex16> invSc = Sc(ik).inverse();
      SxVector<SxComplex16> betaDagger = beta(ik).adjoint ();
      for (int iSpin = 0; iSpin < nSpin; iSpin++)   {
         SxVector<SxComplex16> PDagger = P(ik)(iSpin).adjoint ();
         SxVector<SxComplex16> gradk = (PDagger ^ P(ik)(iSpin) ^ beta(ik) ^ invSc)
                                   - (Sb(ik) ^ beta(ik) ^ invSc ^ betaDagger 
                                   ^ PDagger ^ P(ik)(iSpin) ^ beta(ik) ^ invSc);
         result(ik) += 1.0 / double(nSpin) * gradk;
      }
      keepSymmetry (&result(ik));
   }

   SxVector<SxComplex16> redResult = reduce (result);
   result = expand (redResult);
   
   return result;
}

void SxQuamolRedSubSpace::keepSymmetry (SxVector<SxComplex16> *MatIN)
{
   SxVector<SxComplex16> &result = *MatIN;
   ...
   SX_CHECK (MatIN->getNRows () == nOrbitalsB, MatIN->getNRows (), nOrbitalsB);
   SX_CHECK (MatIN->getNCols () == nOrbitalsC, MatIN->getNCols (), nOrbitalsC);
...
SxOrbitals SxQuamolRedSubSpace::radialsToOrbitals (const SxAtomicOrbitals &radialsIn)
{
   ...
   SxAtomicOrbitals radialsG (muSet,radGBasisPtr,0);
   radialsG.toSpline ();
...
SxVector<SxComplex16> SxQuamolRedSubSpace::reduce (SxArray<SxVector<SxComplex16> > &matIn) const
{
   SxArray<SxQuantumNumbers> mapB = basis.getOrbitalMap (structure);
   SxArray<SxQuantumNumbers> mapC = improvedBasis.getOrbitalMap (structure);
   ...
   SxVector<SxComplex16> result (dimB, dimC);
   result.set(0.0);

   int nk = waves.getNk ();
   const SxVector<double> &kWeight = GkBasisPtr->weights;
   for(int ik = 0; ik < nk; ik++)   {
      SX_CHECK (mapB.getSize  () ==  matIn(ik).getNRows (),
                mapB.getSize (), matIn(ik).getNRows ());
      SX_CHECK (mapC.getSize () ==  matIn(ik).getNCols (),
                mapC.getSize (), matIn(ik).getNCols ());
      for(int iRow = 0; iRow < matIn(ik).getNRows (); iRow++)   {
         for(int iCol = 0; iCol < matIn(ik).getNCols (); iCol++)   {
...
SxArray<SxVector<SxComplex16> > SxQuamolRedSubSpace::expand (SxVector<SxComplex16> &matIn) const
{
   SxArray<SxQuantumNumbers> mapB = basis.getOrbitalMap (structure);
   SxArray<SxQuantumNumbers> mapC = improvedBasis.getOrbitalMap (structure);
   SxArray<SxQuantumNumbers> redMapB = basis.getReducedOrbitalMap ();
   ...
   SX_CHECK (redMapB.getSize () ==  matIn.getNRows (),
             redMapB.getSize (), matIn.getNRows ());
   SX_CHECK (redMapC.getSize () ==  matIn.getNCols (),
             redMapC.getSize (), matIn.getNCols ());
...
{
   SxVector<SxComplex16> redBeta = reduce (beta);
...
double SxQuamolRedSubSpace::dotproductRad (const SxVecRef<double> &vec1, const SxVecRef<double> &vec2)
{
   // Check ingredients 
   SX_CHECK (vec1.auxData.is == vec2.auxData.is,
             vec1.auxData.is, vec2.auxData.is);
   SX_CHECK (vec1.getSize() ==  vec2.getSize(),
             vec1.getSize(), vec2.getSize());
   int is = vec1.auxData.is;
   const SxRadBasis &radBasis = *radBasisPtr;
   int l1 = vec1.auxData.l;
   int l2 = vec2.auxData.l;
...
void SxQuamolRedSubSpace::compute ()
{
   ...
   SxArray<SxVector<SxComplex16> > grad = calcGradient ();
   SxArray<SxVector<SxComplex16> > dir = grad; // copy
   ...
   do   {
      ...
      if(step == 1 || restartCG)   {
         ...
         SxArray<SxVector<SxComplex16> > oldGrad = std::move(grad);
         
         // Calculate new Gradient
         grad = calcGradient ();

         SxArray<SxVector<SxComplex16> > oldDir = std::move(dir);
      ...
      sw = control.re;
      SxArray<SxVector<SxComplex16> > toBeta = skalarMultBeta(sw, dir);
      beta = addBeta(beta, toBeta);
      norm = calcNormC (); 
...
SxArray<SxVector<SxComplex16> > SxQuamolRedSubSpace::addBeta (
...
SxArray<SxVector<SxComplex16> > SxQuamolRedSubSpace::skalarMultBeta (
      const double skalar,
...
SxComplex<double> SxQuamolRedSubSpace::lineMin (
      const SxArray<SxVector<SxComplex16> > &dir,
      double sw)
{
   // backup actual beta
   SxArray<SxVector<SxComplex16> > backup = std::move(beta);
   ...
   double x1 = sw;
   SxArray<SxVector<SxComplex16> > toBeta = skalarMultBeta (x1, dir);
   beta = addBeta (backup, toBeta);
   double N1 = calcNormC ();
   ...
   if (lineMinSteps >= 10)   {
      ...
      SxVector<double> swArray(101), N(101);
      ...
      SxTextIO("LineMin.dat").writeXYPlot(swArray,N);
      cout << SX_SEPARATOR;
      SX_EXIT;
   }

   beta = std::move (backup);
   ...
   for (int iSpecies = 0; iSpecies < nSpecies; iSpecies++)   {
      ...
      for (int l = 0; l < nL; l++)   {
         ...
         for (int iZeros = 0; iZeros < nZeros; iZeros++)   {
            ...
            muSet(iSpecies)(iot).auxData.setOrb (iot, l, NONE_M);
            iot++;
         }
      }
   }
...
SxVector<double> SxQuamolRedSubSpace::jsb (int l, const SxVecRef<double> &z) const
{
...
int main (int argc, char** argv)
{
   ...
   if (table->containsGroup("pseudoPot"))   {
      ...
      SxConstPtr<SxRadBasis> radBasisPotPtr 
         = SxConstPtr<SxRadBasis>::create (psPotPtr->rad, psPotPtr->logDr);
      ...
      SxConstPtr<SxRadBasis> radBasisPotPtr
         = SxConstPtr<SxRadBasis>::create (pawPotPtr->rad, pawPotPtr->logDr);
   ...
   for(int is = 0; is < ns; is++)   {
      ...
      for(int iot = 0; iot < nOrbTypes; iot++)   {
         ...
         file += iot;
         file += ".dat";
         SxTextIO (file).writeXYPlot(redQuamol.radBasisPtr->radFunc(is),
                                     redQuamol.improvedBasis(is,iot));
      }
   }
