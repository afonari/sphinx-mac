...
CellMat SxCell::getRegularLattice (const CellMat &cell, double eps)
{
   ...
   PrecTauR proj;
   bool done;
   SxVector3<PrecTauR> basis[3], redVec;
   basis[0] = cell.col(0);
   basis[1] = cell.col(1);
   basis[2] = cell.col(2);
...
SxList<SymMat> SxCell::getLatticeSymmetries (bool regular) const
{
   ...
   */
...

   {
      ...
      SxMatrix3<double> S2 = symCart.transpose () ^ symCart;
      SX_LOOP (d) S2(d,d) -= 1.;
      SxSymEigensystem<double> eig ((SxVector<double> (S2)));
      SxVector<double> diag(3,3);
      diag.set (0.);
      SX_LOOP (d) {
         double x = eig.vals(d), sev = sqrt(1. + x);
         //diag(d,d) = 1./sqrt(eig.vals(d)) - 1.;
         diag(d,d) = -x / (sev * (1. + sev));
      }
      SxMatrix3<double> U3(eig.vecs ^ diag ^ eig.vecs.transpose ());
      symCart += symCart ^ U3;
   }
