...
void SxPoscar::read ()
{
   ...
   res = fgets (buffer, BUFLEN, fp); iLine++;
                      ...
                      const SxArray<SxString> &chemName)
{
   ...
   if (structure_.getNSpecies() == 0)  {
      // --- abortion, the structure is empty
      return;
   }

   // --- open file and write
   write (SxTextIO(filename).getFp (), structure_, chemName);
}


// Write the data to a POSCAR file.
void SxPoscar::write (FILE *fp,
                      const SxAtomicStructure &structure_,
                      const SxArray<SxString> &chemName,
                      const char *header)
{
   // --- write header
   ...
   int nSpecies = structure_.getNSpecies ();
   ...
   for (int iSpecies=0; iSpecies < nSpecies; ++iSpecies)  {
      ...
      for (int iAtom=0; iAtom < structure_.getNAtoms(iSpecies); ++iAtom)  {
