     ...
     epsEqual(in.epsEqual)
{
   ...
   if (nTlAtoms > 0)  {
      if (handling == Reference)  {
         ...
         coords = SxVector<PrecTauR> (in.coords);
      } else {
         SX_CHECK (handling == NoCoords);
      }
   }
}
     ...
     epsEqual(SX_EPS_STRUCT_DEFAULT)
{
   ...
   // allocate memory
   coords = SxVector<PrecTauR>(3, nTlAtoms);
   // copy coordinates
     ...
     epsEqual(SX_EPS_STRUCT_DEFAULT)
{
   ...
   if (nAtoms > 0)  {
      coords = SxVector<PrecTauR> (3, nAtoms);
      creationStatus = Finished;
   }
}
...
SxAtomicStructure::SxAtomicStructure (const SxVecRef<PrecTauR> &coord,
                                      enum DataHandling handling)
   ...
   epsEqual(SX_EPS_STRUCT_DEFAULT)
{
   ...
   if (nTlAtoms > 0) {
      if (handling == Reference)
         coords = const_cast<SxVecRef<PrecTauR>&>(coord);
      else
         coords = SxVector<PrecTauR>(coord);
      coords.reshape (3, nTlAtoms);
   }
   ...
   epsEqual(SX_EPS_STRUCT_DEFAULT)
{
   ...
   coords = SxVector<PrecTauR>(3, nTlAtoms);
   SxVecRef<int>::Iterator it = atomInfo->parentMap.begin ();
...
void SxAtomicStructure::operator<<= (const SxAtomicStructure &in)
{
       ...
       || (!atomInfo || !in.atomInfo))  // or atomInfo is lacking
   {
      ...
      const SxVecRef<int> &idx = in.atomInfo->getIdxMap (atomInfo);
      SxVecRef<int>::Iterator it = idx.begin ();
      int na = in.nTlAtoms;
      for (int ia = 0; ia < na; ++ia, ++it)
         ref(*it) = in.constRef (ia);
   } else if (atomInfo->isChild (in.atomInfo))  {
      // --- this is derived from in
      const SxVecRef<int> &idx = atomInfo->getIdxMap (in.atomInfo);
      SxVecRef<int>::Iterator it = idx.begin ();
      for (int ia = 0; ia < nTlAtoms; ++ia, ++it)
...
SxAtomicStructure::operator+ (const SxAtomicStructure &in) const
{
       ...
       || (!atomInfo || !in.atomInfo))  // or atomInfo is lacking
   {
      ...
      SX_CHECK (in.coords.getSize() == coords.getSize(),
                in.coords.getSize(),   coords.getSize());
      SxAtomicStructure res ((atomInfo ? *this : in), NoCoords);
...
SxAtomicStructure& SxAtomicStructure::set (const Coord &fillVal)
{
   SX_CHECK (creationStatus == Finished);
   PrecTauR *pos = coords.elements;
   for (int i=0; i < nTlAtoms; i++, pos += 3)
      Coord::toVec3Ref(pos) = fillVal;
   return *this;
}
...
void SxAtomicStructure::set (const SxVecRef<double> &pos,
                             enum DataHandling handling)
{
   SX_CHECK (&pos != &coords);
   SX_CHECK(!isCreationMode ());
   ...
   SX_CHECK (handling == Reference || handling == Copy);
   SX_CHECK(pos.getNRows () == 3 || handling == Copy, pos.getNRows ());
   nTlAtoms = (int)pos.getNCols ();
   ...
   if (handling == Reference)  {
      ...
      coords.unref ();
      coords = SxVector<double> (3, nTlAtoms);
      coords <<= pos;
   }
...
SxVector<PrecTauR> SxAtomicStructure::absSqr () const
{
   SX_CHECK (creationStatus == Finished);
   SX_CHECK (nTlAtoms > 0, nTlAtoms);
   SxVector<PrecTauR> r2(nTlAtoms);
   SxVector<PrecTauR>::Iterator r2It = r2.begin ();
   for (int i = 0; i < nTlAtoms; i++)
      *r2It++ = constRef(i).normSqr();

   SX_VALIDATE_VECTOR(r2);
   return r2;
}
...
void SxAtomicStructure::printCell (const SxCell &cell)
{
...
void SxAtomicStructure::write (SxBinIO &io) const
{
   ...
   try  {
      ...
      if ((io.ncMode == SxBinIO::WRITE_HEADER) || (SxLoopMPI::me() == 0))
      {
         ...
         io.write ("tau", coords.transpose (), "nAllAtoms", "xyz");
...
void SxAtomicStructure::read (const SxBinIO &io)
{
   ...
   try  {
      ...
      coords.unref ();
      coords = SxVector<PrecTauR>(nTlAtoms, 3);
...
SxAtomicStructure
SxAtomicStructure::operator* (const SxVecRef<PrecTauR> &v) const
{
   SX_CHECK (creationStatus == Finished);
   if (v.getSize () == nTlAtoms)  {
      // --- atom-specific factors
      SxAtomicStructure res (getNewStr ());
      SxVecRef<PrecTauR>::Iterator vIt = v.begin ();
      for (int ia = 0; ia < nTlAtoms; ++ia)
      ...
      SxAtomicStructure res (getNewStr ());
      for (int is = 0, iTl = 0; is < getNSpecies (); is++)
         for (int ia = 0; ia < getNAtoms (is); ia++, iTl++)
            res.ref(iTl) = constRef(iTl) * v(is);
      return res;
   }
}
...
SxAtomicStructure::operator/ (const SxVecRef<PrecTauR> &v) const
{
   ...
   if (v.getSize () == nTlAtoms)  {
      ...
      for (int is = 0, iTl = 0; is < getNSpecies (); is++)  {
         SX_CHECK_DIV(v(is));
         PrecTauR vInv = 1. / v(is);
         for (int ia = 0; ia < getNAtoms (is); ia++, iTl++)
            res.ref(iTl) = constRef(iTl) * vInv;
      }
      return res;
   }
}
...
SxAtomicStructure &
SxAtomicStructure::operator*= (const SxVecRef<PrecTauR> &v)
{
   SX_CHECK (creationStatus == Finished);
   if (v.getSize () == nTlAtoms)  {
      // --- atom-specific factors
...
SxVector<PrecTauR> SxAtomicStructure::operator^(const Coord &x) const
{
   if (nTlAtoms == 0) return SxVector<PrecTauR> ();
   SxVector<PrecTauR> res(nTlAtoms);
   SxVector<PrecTauR>::Iterator it = res.begin ();
   for (int ia = 0; ia < nTlAtoms; ++ia)
      *it++ = x ^ constRef(ia);
...
{
   if (nTlAtoms == 0 || x.nTlAtoms) return SxVector<PrecTauR> ();
   SX_CHECK (this != &x); // use absSqr () instead
   SxVector<PrecTauR> res(nTlAtoms, x.nTlAtoms);
   SxVector<PrecTauR>::Iterator it = res.begin ();
