...
SxExxSolver::SxExxSolver (const SxAtomicStructure &structureIn,
         ...
         : SxHamSolver(structureIn, table), nValStates(-1)
{
   SX_CHECK(wavesPtr);
   const SxSymbolTable *top = table->topLevel();
   // --- initialize n123inv table of |G+k> basises
   SxXC::XCFunctional xc = SxXC::getXCFunctional (top);
   if (   xc == SxXC::EXX
       || xc == SxXC::EXX_LDA
       || xc == SxXC::EXX_PBE
       || xc == SxXC::EXX_PBE_WB
...
void SxExxSolver::relaxEXXPotential (const SxSymbolTable *cmd, int step)
{
   ...
   // --- get input parameters
   SxSymbolTable          *relax = NULL;
   int                     maxRelaxXSteps = -1;  // Don't confuse with 'maxSteps'!
   int                     minRelaxXSteps = -1;
   double                  epsX = -1.;
   double                  alpha = -1.;
...
void SxExxSolver::gsEXX (const SxSymbolTable *cmd, bool calc)
{
   ...
   int             maxStepsEXX = -1;  // These parameters belong to the outer loop.
   PrecEnergy      dEnergyEXX = -1.;   // "maxSteps" and "dEnergy" are controlled by
                                 // the inner loop (group "relaxRho").
   SxSymbolTable  *relaxRho = NULL, *stern = NULL, *direct = NULL;
   bool            withInterimResults = false;
