...
class SX_EXPORT_DFT SxHamSolver : public SxPotential
{
   public:
      ...
      /// Base class for registered commands
      class Command {
         public:
            virtual ~Command () {}
            /** \brief Execute the command
              @param solver the SxHamSolver object
              @param cmd    the symbol table with parameters
              @param calc   whether to really execute
              @return whether to save rho & waves to disk
              */
            virtual bool execute (SxHamSolver *solver,
                                  const SxSymbolTable *cmd,
                                  bool calc) = 0;
      };
      /// Class for registering internal commands
      class InternalCommand; // defined in cpp file
      /** \brief Wrapper class for registering external (add-on) commands */
      class ExternalCommand : public Command {
         public:
            /// Function pointer
            typedef void (*Func)(SxHamSolver*, const SxSymbolTable *, bool);
         protected:
         ...
         public:
            /// Constructor
            ExternalCommand (Func funcIn, bool saveDataIn = false)
               : func(funcIn), saveData(saveDataIn) {}

            /// Execute the wrapped external command function
            virtual bool execute (SxHamSolver *solver,
                                  const SxSymbolTable *cmd,
                                  bool calc) override
            {
               func (solver, cmd, calc);
               return saveData;
            }
            /// Destructor
      ...
      SxMap<SxString, SxPtr<Command> > cmdTable;


      /// Add command for the "execute" parser
      inline void addCommand (const SxString &cmdName,
                              ExternalCommand::Func func,
                              bool saveData = false)
      {
         SX_CHECK (!cmdTable.containsKey(cmdName));
         cmdTable(cmdName) = SxPtr<ExternalCommand>::create (func, saveData);
      }

   protected:
      /// Set up the internal table of available commands
   ...
   public:
      ...
      */
      ...
      /** \brief Perform the initial guess */
      void initialGuess (const SxSymbolTable *table, bool calc=true) override;
      ...
      */
      ...
      /** \brief Solve the non-linear DIIS problem
        ...
        */
      static PsiG
      solveRmmDiis (const SxVecRef<PrecCoeffG> &h2In,
      ...
      void subspaceDiagonalization (const SxSymbolTable *cmd, bool calc);
        ...
        */
      SxVector<PrecEps> subspaceDiagonalization (SxVecRef<PrecCoeffG> *wavesPtrIN,
                                                 int subDiagSize = -1,
                                                 int subDiagOvlp = -1);
