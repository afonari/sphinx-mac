...
void updateHubbard (SxHamSolver *solver, const SxSymbolTable *table, bool)
{
   SX_CHECK (solver);
   SxPtr<SxPAWHamiltonian> pawHam = solver->hamPtr;
   SX_CHECK (pawHam->hubbardU);
   pawHam->hubbardU->readUpdate (table);
   ...
   : potPtr (potPtrIn)
{
   ...
   if (pawHam->hubbardU)  {
      pawHam->setupForHubbard (*gkPtr, nSpin);
      hamSolver->addCommand("updateHubbard", updateHubbard);
   }
...
void SxInitPAW::randomRho ()
{
   ...
   SxVector<PrecCoeffG> coresG(0., G);
   for (int is = 0; is < structure.getNSpecies (); ++is)
      coresG += G.getStructureFactor (is)
                           ...
                           const SxAtomicStructure &structure)
{
   ...
   } else if (rhoGroup && rhoGroup->containsGroup("aspherical"))  {
      pawHam->pawRho.asphericalAtomCharge (rhoGroup, structure, potPtr, R, nSpin);
...
#ifndef NDEBUG
   SX_LOOP3(iSpin, is, ia)  {
      SX_CHECK((int)pawHam->pawRho.Dij(iSpin,is,ia).auxData.iSpin == int(iSpin),
               (int)pawHam->pawRho.Dij(iSpin,is,ia).auxData.iSpin, int(iSpin));
   }
#endif
}
                                      ...
                                      int nStates, int nSpin)
{
   ...
   return SxPtr<SxPW>::create (nStates, nSpin, gkPtr, /*nComp*/ 1, tmpDir);
}
