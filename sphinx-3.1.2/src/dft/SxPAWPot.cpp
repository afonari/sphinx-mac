...
SxVector<double> lowPass (const SxVecRef<double> &f, double eps = 1e-4)
{
   if (eps <= 0.) return f;
   ...
   if (igMin > 0) fSmooth.getRef(igMin) <<= f.getRef(igMin);
...
SxVector<double> toRSpace (const SxVecRef<double> &f, double gMax,
...
SxVector<double> toRSpace (const SxVecRef<double> &f, double gMax,
                           const SxVecRef<double> &r, int l = 0)
{
     ...
     verbose(false)
{
   ...
   SYMBOLPARSE(table->getGroup ("pawPot"))
   {
      SxString potFile, potType;
      nSpecies = SYMBOL_COUNT("species");
      ...
      if (SYMBOLGET("kjxc").toBool ())
         kjXcShape.resize (nSpecies);

      FOREACH_SYMBOLGROUP ("species")
      {
         potFile = SYMBOLGET("potential");
         potType = SYMBOLGET("potType");
         ...
         if ( potType == "AbInit" ) {
            ...
            readVasp (potFile, is, SYMBOLGROUP_TABLE);
         } else if ( potType == "xml" ) {
            int nr = SYMBOLGET("nRadGrid") || -1;
            readXml (potFile, is, nr);
         } else {
            cout << "Unknown potType=" << potType << endl;
            cout << "Possible types are AbInit, AtomPAW, CPPAW, VASP, xml"
                 << endl;
            SX_QUIT;
         }

         lMaxRho(is) = SYMBOLGET("lMaxRho") || (2 * lMax(is));
         if (verbose)
            cout << "lMaxRho(" << prettyName(is) << ")= " << lMaxRho(is)<< endl;

         aGridType(is) = SxSphereGrid::GridType (SYMBOLGET("angularGrid") || int(SxSphereGrid::Grid_110));

         if (HAVE_SYMBOL("atomicRhoOcc"))  {
            foccInit(is) = SYMBOLGET("atomicRhoOcc")->toList ();
         ...
         if (HAVE_SYMBOL("nRadGrid"))  {
            ...
            SxVector<double> newPhiAE(nrNew, npt),
                             newPhiPS(nrNew, npt),
                             newProj(nrNew, npt);
            newPhiAE.auxData = phiAE(is).auxData;
            newPhiPS.auxData = phiPS(is).auxData;
            newProj.auxData = pPS(is).auxData;
            for (int ipt = 0; ipt < npt; ++ipt)  {
               newPhiAE.colRef(ipt) <<= interpolateRad (phiAE(is).colRef(ipt),
                                                        r0, logDr(is), newRad);
               newPhiPS.colRef(ipt) <<= interpolateRad (phiPS(is).colRef(ipt),
                                                        r0, logDr(is), newRad);
               if (!dynamic_cast<const SxRadialBasis *>(pPS(is).getBasisPtr ()))
         ...
         phiAE(is).auxData.is = is;
         phiPS(is).auxData.is = is;
         pPS(is).auxData.is   = is;

         rPAW(is) << SYMBOLGET("rPAW");
         double rMax = rad(is)(rad(is).getSize () - 1);
         if (rPAW(is) < 0.  || rPAW(is) > rMax * exp(logDr(is))) {
            cout << "Inconsistency! rPAW(is=" << is + 1
                 << "; " << prettyName(is) << ")="
                 << rPAW(is) << " is beyond radial grid" << endl;
            cout << "rMax = " << rMax << endl;
            SX_EXIT;
         }
         ...
         if (verbose && SxLoopMPI::me() == 0)  {
            for (int i = 0; i < lPhi(is).getSize (); ++i)  {
               ...
               SxTextIO ("paw-ae-"+id+".dat")
                  .writeXYPlot (rad(is), phiAE(is).colRef(i));
               SxTextIO ("paw-ps-"+id+".dat")
                  .writeXYPlot (rad(is), phiPS(is).colRef(i));
               if (phiLCAO.getSize () > 0 && phiLCAO(is).getSize () > 0)
                  SxTextIO ("paw-lcao-"+id+".dat")
                     .writeXYPlot (radBasisLCAO->radFunc(is), getPhiPS(is, i));
               if (!dynamic_cast<const SxRadialBasis *>(pPS(is).getBasisPtr ())) {
                  SxTextIO ("paw-proj-"+id+".dat")
                     .writeXYPlot (rad(is), pPS(is).colRef(i));
               }
            }
         }

         if (HAVE_SYMBOL("coreX"))  {
            coreX(is) = SxVector<double> (SYMBOLGET("coreX")->toList ());
         ...
         coreWaveFile(is) = SYMBOLGET("coreWaves") || "";

         checkOverlap(is, !SYMBOLGET("checkOverlap").toBool ());
   ...
   // complete LCAO rad basis where needed
   if (radBasisLCAO)  {
      SX_LOOP (js)  {
         if (radBasisLCAO->radFunc(js).getSize () == 0)  {
            radBasisLCAO->logDr(js) = logDr(js);
            radBasisLCAO->radFunc(js) = radBasisPtr->radFunc(js);
         }
      }
   }
   ...
   for (is = 0; is < nSpecies; ++is)  {
      ...
      if (coreWaveFile(is).getSize () > 0)  {
         ...
         // --- check core
         SxVector<double> rhoCore (rad(is).getSize ());
         rhoCore.set (0.);
      ...
      if (hybridFunctional && psiCoreAE(is).getSize () == 0)  {
         int npt = (int)lPhi(is).getSize ();
         coreX(is) = SxVector<double> (npt, npt);
...
namespace {
double tfunc(const SxVector<double> &V,
                   double E,
                   int l, double logdr,
                   int irMatch)
{
   SxVector<double> u = SxRadialAtom::getTail (V, E, l, logdr, irMatch-1);
   int nNode = 0;
   for (ssize_t ir = irMatch; ir < u.getSize (); ++ir)   {
      if (u(ir-1) * u(ir) < 0.) nNode++;
      if (u(ir-1) == 0.) nNode++;
   }
   return SxRadialAtom::matchTarget(u(irMatch-1), u(irMatch), logdr, nNode);
}
double findEnergy (const SxVector<double> &V,
                   int l, double logdr,
                   int irMatch,
                   double t0)
{
   double eL = -10.,
          tL = tfunc(V, eL, l, logdr, irMatch),
          eR = 0.,
          tR = tfunc(V, eR, l, logdr, irMatch);
   while (tL > t0 && eL > -100.)  {
      eL -= 1.;
      tL = tfunc(V, eL, l, logdr, irMatch);
   }
   ...
   if (tL > t0 || tR < t0)  {
      cout << "Failed to find energy bracket: " << endl;
      cout << "t0 = " << t0 << endl;
      cout << "eL = " << eL << " tL= " << tL << endl;
      cout << "eR = " << eR << " tR= " << tR << endl;
      SX_QUIT;
   }
   while (eR - eL > 1e-8)  {
      double e = 0.5 * (eL + eR);
      double t = tfunc (V, e, l, logdr, irMatch);
      if (fabs(t - t0) < 1e-8) return e;
      if (t > t0)  {
         tR = t; eR = e;
      } else {
         tL = t; eL = e;
...
void SxPAWPot::recomputeTail (int is, const SxVecRef<double> &V, int  irMatch)
{
   if (phiLCAO.getSize () == 0)  {
      phiLCAO.resize (getNSpecies ());
      radBasisLCAO = SxPtr<SxRadBasis>::create ();
      radBasisLCAO->logDr = logDr;
      radBasisLCAO->radFunc.resize (getNSpecies ());
   }

   cout << "Recomputing tails for ipt=";
   ssize_t nrNew = V.getSize ();
   radBasisLCAO->radFunc(is) = V.getBasis<SxRadialBasis> ().getRadFunc ();
   const SxVector<double> &radR = radBasisLCAO->radFunc(is);
   phiLCAO(is).reformat (nrNew, getNProjType (is));
   phiLCAO(is).set (0.);
   SX_LOOP(ipt)  {
      int l = lPhi(is)(ipt);
      phiLCAO(is).getRef(phiPS(is).getNRows (), ipt * nrNew) <<= phiPS(is).colRef(ipt);
      if (fabs(foccInit(is)(ipt) < 1e-6)) continue;

      double u1 = phiPS(is)(irMatch-1,ipt) * sqrt(radR(irMatch-1)),
             u0 = phiPS(is)(irMatch,ipt) * sqrt(radR(irMatch));
      double t0 = SxRadialAtom::matchTarget(u1, u0, logDr(is), u1 * u0 < 0. ? 1 : 0);
      double E = findEnergy(V, l, logDr(is), irMatch, t0);
      cout << ipt << " (E=" << E <<" H), ";
      SxVector<double> u = SxRadialAtom::getTail (V, E, l, logDr(is), irMatch);
      double scale = u0 / u(irMatch);
      for (int ir = irMatch + 1; ir < nrNew; ++ir)
         phiLCAO(is)(ir, ipt) = u(ir) / sqrt(radR(ir)) * scale;
   }
   cout << "done." << endl;
}
...
SxVector<double> getJsbShape (double rCut, int l,
                                const SxVecRef<double> rad, double logdr)
{
   double x0[2], start = 0.;
   ...
   SxVector<double> A(2,2);
   int nr = (int)rad.getSize ();
   SxArray<SxVector<double> > jsbRad(2);
   for (int iq = 0; iq < 2; ++iq)  {
      double q = x0[iq] / rCut;
      jsbRad(iq).resize (nr);
      for (int ir = 0; ir < nr; ++ir)
...
SxVector<double> readWaves (SxList<SxString>::Iterator &it,
                              ...
                              bool expCont = false)
{
   SxVector<double> res(nPtsMax, nWaves);
...
//TODO: simplify regexpr by a more pragmatic approach (see readAtomPAW)
{
   ...
   cout << "EXITING: only Gaussian shape functions are supported (lambda=2)"
        << endl;
            ...
            case 2:
               ...
               if (rShape > rPAW(iSpecies))  {
                  cout << "Updating rPAW = rShape = " << rShape << endl;
                  rPAW(iSpecies) = rShape;
               }
               break;
             case 3:            
   ...
   rhoCoreAE(iSpecies) = getFunction (*it, verbose, meshPts(meshIdx-1),
                                      nPtsMax, logdr);
   rhoCoreAE(iSpecies).auxData.is = iSpecies;
   rhoCoreAE(iSpecies).auxData.l = 0;
   rhoCoreAE(iSpecies).auxData.m = 0;
   ...
   rhoCorePS(iSpecies) = getFunction (*it, verbose, meshPts(meshIdx-1),
                                      nPtsMax, logdr);
   rhoCorePS(iSpecies).auxData.is = iSpecies;
   rhoCorePS(iSpecies).auxData.l = 0;
   rhoCorePS(iSpecies).auxData.m = 0;
   ...
   SxVector<double> Dij(matSize,matSize);
   ...
   SxVector<double> rhoij0(matSize,matSize);
   for (int a = 0;a < matSize; a++) {
      cLine = matList(a).tokenize (' ');
      for (int b = 0; b < cLine.getSize (); b++) {
   ...
   vBar(iSpecies).auxData.is = iSpecies;
   vBar(iSpecies).auxData.l = 0;
   vBar(iSpecies).auxData.m = 0;

   // --- compute kinetic energy matrix elements
   {
      SxVector<double> rhoijPS, rhoijAE;
      rhoCoreAE(iSpecies).setBasis (&radB);
      SxVector<double> vZcAE = (-nuclearCharge(iSpecies))/rad(iSpecies)
      ...
      rhoCoreAE(iSpecies).auxData.basisPtr = NULL;
      double Xij, Sij, Qij;
      ...
      deltaKin(iSpecies) = SxVector<double> (nWaves, nWaves);
      deltaS(iSpecies)   = SxVector<double> (nWaves, nWaves);
   ...
   it++; itMeshIdx++;
   meshIdx = meshIdxs(itMeshIdx);
   SxVector<double> rhoValPS =
      getFunction (*it, verbose, meshPts(meshIdx-1), nPtsMax, logdr);
   
   rhoInit(iSpecies) = (rhoValPS + rhoCorePS(iSpecies)) * sqrt(FOUR_PI);
   rhoInit(iSpecies).auxData.is = iSpecies;
   rhoInit(iSpecies).auxData.l  = 0;
   rhoInit(iSpecies).auxData.m  = 0;
   SxVector<double> rhoValAE(nPtsMax), rhoValPS2(nPtsMax);
   rhoValAE.set (0.);
   rhoValPS2.set (0.);
   ...
   for (int ia = 0; ia < rhoij0.getNRows (); ia++) {
      for (int ja = 0; ja< rhoij0.getNCols (); ja++) {
   ...
   double SQRT_4PI = sqrt(FOUR_PI);
   {
      ...
      SxVector<double> g0 = getGrl(iSpecies, 0) / FOUR_PI;
      //SxVector<double> g0 = g0File; // DEBUG only
      g0File.setBasis (&radB);
      g0File.auxData.is = iSpecies;
      g0File.auxData.l = 0;
      g0File.auxData.m = 0;
      g0.setBasis (&radB);
      g0.auxData.is = iSpecies;
      g0.auxData.l = 0;
      g0.auxData.m = 0;
      if (verbose)  {
         cout << "g0 moment: "
      ...
      // pseudo-density
      SxVector<double> rhoPS = rhoCorePS(iSpecies) + rhoValPS;
      // xc and Hartree potentials
      SxVector<double> vXcKJ, vXcPS, vHdelta;
...
void errorInFile (const SxString &filename)
{
   cout << "In file '" << filename << "': ";
}

using namespace rapidxml;
SxVector<double> readWaves (xml_node<char> *top,
                            const char *name,
                            int nWaves,
                            const SxRadBasis &rad,
                            const SxArray<SxString> &waveId,
                            SxMap<SxString, SxRadialBasis> &radGrids,
                            const SxArray<int> &lPhi,
                            const SxString &filename)
{
   if (count (top, name) != nWaves)  {
      errorInFile (filename);
      cout << "Corrupted file: expected " << nWaves << " <" << name
           << "> nodes, but found " << count (top, name) << endl;
      SX_QUIT;
   }
   int is = (int)rad.getNSpecies () - 1; // always last species
   SxVector<double> allWaves (rad.radFunc(is).getSize (), nWaves);
   allWaves.setBasis (rad);
   allWaves.auxData.is = is;
   for (xml_node<char> *node = get(top, name, filename);
        node; node = node->next_sibling (name))
   {
      SxString id = toString (getAttr(node, "state"));
      int iWave = 0;
      for (; iWave < nWaves; ++iWave)
         if (id == waveId(iWave)) break;
      if (iWave >= nWaves)  {
         errorInFile (filename);
         cout << "Unexpected " << name << " state=" << id << endl;
         SX_QUIT;
      }
      SxString grid = toString(getAttr(node, "grid"));
      const SxRadialBasis &rGrid = radGrids (grid);
      SxVector<double> wave = toVector(node);
      if (wave.getSize () != rGrid.getNElements ())  {
         errorInFile (filename);
         cout << "Error while reading " << name << " state=" << id
              << " grid= " << grid << endl;
         cout << "grid has " << rGrid.getNElements () << " points." << endl;
         cout << "vector has " << wave.getSize () << " entries" << endl;
         SX_QUIT;
      }
      wave.setBasis (rGrid);
      wave.auxData.l = (char)lPhi(iWave);
      wave.auxData.m = 0;
      wave.auxData.is = is;
      allWaves.colRef(iWave) = rad | wave;
   }
   SX_VALIDATE_VECTOR (allWaves);
   return allWaves;
}

SxVector<double> getFunc(xml_node<char> *node,
                         const SxRadBasis &rad,
                         SxMap<SxString, SxRadialBasis> &radGrids,
                         const SxString &filename)
{
   SxVector<double> func = toVector (node);
   SxString grid = toString(getAttr(node, "grid"));
   const SxRadialBasis &rGrid = radGrids (grid);
   if (func.getSize () != rGrid.getNElements ())  {
      errorInFile (filename);
      cout << "Error while reading " << node << ": grid= " << grid << endl;
      cout << "grid has " << rGrid.getNElements () << " points." << endl;
      cout << "vector has " << func.getSize () << " entries." << endl;
      SX_QUIT;
   }
   func.setBasis (rGrid);
   func.auxData.l = 0;
   func.auxData.m = 0;
   func.auxData.is = (int)rad.getNSpecies () - 1; // always last species
   return rad | func;
}

void SxPAWPot::readXml (const SxString &filename, int iSpecies, int nPts)
{

   SxString potFile;
   SxXC::XCFunctional xcFunctional = SxXC::LDA;
   ...
   doc.parse<parse_non_destructive>(const_cast<char*>(potFile.ascii ()));

   // check input format
   xml_node<char> *top = get (&doc, "paw_dataset", verbose);
   xml_attribute<char> *attr = top->first_attribute ("version");
   if (!attr || toDouble (attr) < 0.699) {
      errorInFile (filename);
      cout << "Incompatible paw_dataset version !" << endl;
      cout << "Must be >0.7" << endl;
      SX_QUIT;
   } else if (toDouble(attr)>0.701) {
      errorInFile (filename);
      cout << "Warning: reader tested only for paw_dataset version=0.7" << endl;
   }

   // --- fetch core charge
   xml_node<char> *node = get (top, "atom", verbose);
   double coreCharge       = toDouble (getAttr (node, "core"));
   nuclearCharge(iSpecies) = toDouble (getAttr (node, "Z"));
   valenceCharge(iSpecies) = toDouble (getAttr (node, "valence"));
   if (verbose)  {
      cout << "Z=" << nuclearCharge(iSpecies) << endl;
      cout << "nv=" << valenceCharge(iSpecies) << endl;
   }
   ...
   // --- XC-Functional
   node = get (top, "xc_functional", verbose);
   SxString xcType = toString (getAttr (node, "type"));
   SxString xcName = toString (getAttr (node, "name"));
   if (xcType == "LDA")  {
      //if (verbose) cout << "LDA-PW" << endl;
      if (xcName == "PW" || xcName == "LDA_X+LDA_C_PW")
         xcFunctional = SxXC::LDA_PW;
      else if (xcName == "LDA_X+LDA_C_PZ")
         xcFunctional = SxXC::LDA;
      else {
         errorInFile (filename);
         cout << "Unknown LDA name=" << xcName << endl;
         cout << "Using LDA-PW instead" << endl;
         xcFunctional = SxXC::LDA_PW;
      }
   } else if ( xcType == "GGA")  {
      if (xcName == "PBE" || xcName == "GGA_X_PBE+GGA_C_PBE")
         xcFunctional = SxXC::PBE;
      else {
         errorInFile (filename);
         cout << "Unknown GGA name=" << xcName << endl;
         SX_QUIT;
      }
   } else {
      errorInFile (filename);
      cout << "Unknown xc functional type=" << xcType
           << " name=" << xcName << endl;
      SX_QUIT;
   }

   // --- read info on available partial wave types
   node = get (top, "valence_states", verbose);
   // number of AE/PS/Projectors
   int nWaves = count (node, "state");

   // collect l values, initial occupations, ids
   lMax(iSpecies) = -1;
   lPhi(iSpecies).resize (nWaves);
   SxArray<SxString> waveId(nWaves);
   foccInit(iSpecies).resize (nWaves);
   SxList<int> valState;
   SxArray<double> rcPhi(nWaves);
   {
      int iWave = 0;
      for (xml_node<char> *state = get(node, "state", filename); state ;
           state = state->next_sibling ("state"), ++iWave)
      {
         int l = lPhi(iSpecies)(iWave) = toInt(getAttr(state, "l"));
         if (l > lMax(iSpecies)) lMax(iSpecies) = l;
         waveId(iWave) = toString(getAttr(state, "id"));
         rcPhi(iWave) = toDouble (getAttr(state, "rc"));
         attr = state->first_attribute ("f");
         foccInit(iSpecies)(iWave) = attr ? toDouble (attr) : 0.;
         if (state->first_attribute ("n")) valState << iWave;
      }
   }
   ...
   double logdr = -1.;
   SxMap<SxString, SxRadialBasis> radGrids;
   for (node = get (top, "radial_grid", filename, verbose); node;
        node = node->next_sibling ("radial_grid"))
   {
      SxString eq = toString (getAttr (node, "eq"));
      SxString id = toString (getAttr (node, "id"));
      int iStart = toInt (getAttr (node, "istart")),
          iEnd   = toInt (getAttr (node, "iend"));
      if (eq == "r=d*i")  {
         double d = toDouble (getAttr(node, "d"));
         radGrids(id) = SxRadialBasis (d * iStart, d * iEnd, iEnd - iStart + 1,
                                       true, SxRadialBasis::Linear);
         if (r0 < 0. || d < r0) r0 = d;
         if (d * iEnd > rEnd) rEnd = d * iEnd;
      } else if (eq == "r=a*exp(d*i)")  {
         double a = toDouble (getAttr(node, "a"));
         double d = toDouble (getAttr(node, "d"));
         double rMin = a * exp (d * iStart),
                rMax = a * exp(d * iEnd);
         radGrids(id) = SxRadialBasis (rMin, rMax, iEnd - iStart + 1,
                                       true, SxRadialBasis::Logarithmic);
         if (r0 < 0. || rMin < r0) r0 = rMin;
         if (logdr < 0.) logdr = d;
         else if (fabs(logdr - d) > 1e-6)  {
            cout << "Different logarithmic grids used!" << endl;
            cout << "Choosing the finer one!" << endl;
            if (d < logdr) logdr = d;
         }
         if (rMax > rEnd) rEnd = rMax;
      } else {
         SxVector<double> rVals = toVector(get(node, "values", filename));
         if (rVals.getSize () != iEnd - iStart + 1)  {
            errorInFile (filename);
            cout << "Problem when reading radial grid id=" << id;
            cout << "number of points from istart..iEnd = " << iEnd-iStart+1
                 << endl << "but read only " << rVals.getSize ()
                 << " from values." << endl;
            SX_QUIT;
         }
         radGrids(id) = SxRadialBasis (rVals, true);
         if (eq == "r=a*(exp(d*i)-1)")  {
            double a = toDouble (getAttr(node, "a"));
            double d = toDouble (getAttr(node, "d"));
            if (verbose)
               cout << "grid id=" << id
                    << " is a shifted exponential grid (abinit type)"
                    << " with a=" << a << " and d=" << d << endl;
            if (r0 < 0. || a < r0) r0 = a;
            if (logdr < 0.) logdr = d;
            else if (fabs(logdr - d) > 1e-6)  {
               cout << "Different logarithmic grids used!" << endl;
            ...
            double rMax = a * (exp (d * iEnd) - 1);
            if (rMax > rEnd) rEnd = rMax;
         } else if (r0 < 0.) {
            r0 = (rVals(0) > 0.) ? rVals(0) : rVals(1);
            if (rEnd < rVals(rVals.getSize () - 1))
               rEnd = rVals(rVals.getSize () - 1);
         }
      }
   }
   // --- setup rad basis
   if (rad(iSpecies).getSize () == 0)  {
      if (logdr < 0.) {
         if (nPts < 0) nPts = 500;
         logdr = log(rEnd/r0) / double(nPts - 1);
      } else {
         if (verbose)
            cout << "r0=" << r0 << " rEnd=" << rEnd << endl;
         nPts = (int)lround(log(rEnd/r0) / logdr);
      }
      logDr(iSpecies) = logdr;
      SxVector<double> rMesh(nPts);
      for (int j = 0; j < nPts; j++)
         rMesh(j) = r0*(exp (j*logdr));
      rad(iSpecies) = rMesh;
      if (verbose)  {
         cout << "meshPts:" << nPts << endl;
         cout << "logDr: " << logDr(iSpecies) << endl;
      }
   }
   ...
   // --- fetch shape function parameters
   if (count (top, "shape_function") > 1)  {
      errorInFile (filename);
      cout << "SPHInX does not support multiple shape functions." << endl;
      SX_QUIT;
   }
   node = get (top, "shape_function", filename, verbose);
   SxVector<double> g0File;
   {
      SxString type = toString (getAttr (node, "type"));
      double rcShape = toDouble (getAttr (node, "rc"));
      if (type == "gauss")  {
         if (verbose)
            cout << "Gaussian shape function [exp(-(arg/sigma)^2)]" << endl;
         // sigma corresponds to rshape
         rc(iSpecies) = rcShape;
         g0File = exp(-(rad(iSpecies)/rcShape).sqr ());
      } else if (type == "sinc")  {
         if (verbose)  {
            cout << "Sinc shape function" <<
                    "[(sin(pi*arg/rc_hat)/(pi*arg/rc_hat))**2]\n" <<
         ...
         rc(iSpecies) = rcShape/sqrt (-log(1e-4));
         g0File.resize (nr);
         SX_LOOP (ir) {
            double r = rad(iSpecies)(ir);
            if (r < rcShape)  {
               double x = PI * r / rcShape;
               g0File(ir) = sqr(sin(x)/x);
            } else {
               g0File(ir) = 0.;
            }
         ...
         double lambda = toDouble (getAttr (node, "lambda"));
         if (fabs(lambda-2.) > 1e-6)  {
            errorInFile (filename);
            cout << "WARNING: exp (-(r/r_c)^lambda) shape is not tested"
                 << endl;
         }
         rc(iSpecies) = rcShape;
         g0File = exp(- exp(log(rad(iSpecies)/rcShape)*lambda));
      } else {
         errorInFile (filename);
         cout << "shape_function type=" << type << " is not supported."
              << endl;
         SX_QUIT;
      }
   }

   // --- get PAW cutoff radius
   if (count(top, "paw_radius") > 0)  {
      node = get (top, "paw_radius", filename, verbose);
      rPAW(iSpecies) = toDouble (getAttr (node, "rc"));
   } else {
      rPAW(iSpecies) = rc(iSpecies) * sqrt(-log(1e-4));
   }
   if (verbose) cout << "rPAW = " << rPAW(iSpecies) << endl;
   ...
   // --- fetch AE partial waves
   phiAE(iSpecies) = readWaves (top, "ae_partial_wave", nWaves, radB,
                                waveId, radGrids, lPhi(iSpecies), filename);
   ...
   // --- fetch PS partial waves
   phiPS(iSpecies) = readWaves (top, "pseudo_partial_wave", nWaves, radB,
                                waveId, radGrids, lPhi(iSpecies), filename);
   {
      // --- enforce phiAE=phiPS outside cutoff region
      SX_LOOP (iWave)  {
         ssize_t ir;
         for (ir = 0 ; ir < nr; ++ir)
            if (rad(iSpecies)(ir) > rcPhi(iWave)) break;
         for ( ; ir < nr; ++ir)
           phiAE(iSpecies)(ir, iWave) = phiPS(iSpecies)(ir, iWave);
      }
   }
   ...
   // --- fetch projector functions
   pPS(iSpecies) = readWaves (top, "projector_function", nWaves, radB,
                              waveId, radGrids, lPhi(iSpecies), filename);

   // --- check orthonormality condition
   if (verbose)  {
      cout << "Orthonormality conditions <phiPS|pPS>" << endl;
      SX_CHECK (phiPS(iSpecies).getBasisPtr ());
      SX_LOOP2(ipt, jpt)  {
         if (lPhi(iSpecies)(ipt) == lPhi(iSpecies)(jpt))  {
            cout << " i=" << ipt << "/j=" << jpt << ": "
                 << tr(phiPS(iSpecies).colRef(ipt)*pPS(iSpecies).colRef(jpt))
                 << endl;
         }
      }
   }
   ...
   // --- setup deltaS
   deltaS(iSpecies).reformat (nWaves, nWaves);
   SX_LOOP2(ipt,jpt)  {
      if (lPhi(iSpecies)(ipt) == lPhi(iSpecies)(jpt))  {
         deltaS(iSpecies)(ipt, jpt) = tr (  nij (phiAE(iSpecies), ipt, jpt)
                                          - nij (phiPS(iSpecies), ipt, jpt));
      } else {
         deltaS(iSpecies)(ipt, jpt) = 0.;
      }
   }
   if (verbose) cout << "deltaS=" << deltaS(iSpecies) << endl;

   double SQRT_4PI = sqrt(FOUR_PI);
   // --- fetch AE core density
   node = get (top, "ae_core_density", filename, verbose);
   rhoCoreAE(iSpecies) = getFunc (node, radB, radGrids, filename) / SQRT_4PI;

   if (verbose)  {
      double coreIntegral = FOUR_PI* tr (rhoCoreAE(iSpecies));
      cout << "core charge: "
           << coreIntegral << endl;

      cout << "N (4*Pi*N*rhoCoreAE=corecharge): \n"
           << coreCharge / coreIntegral << endl;
   }

   // fetch PS core density
   node = get (top, "pseudo_core_density", filename, verbose);
   rhoCorePS(iSpecies) = getFunc (node, radB, radGrids, filename) / SQRT_4PI;

   // --- vbar
   node = get (top, "zero_potential", filename, verbose);
   vBar(iSpecies) = getFunc (node, radB, radGrids, filename) / SQRT_4PI;

   // --- kinetic energy matrix elements
   node = get (top, "kinetic_energy_differences", filename, verbose);
   deltaKin(iSpecies) = toVector(node);
   if (deltaKin(iSpecies).getSize () != sqr(nWaves))  {
      errorInFile (filename);
      cout << "Incorrect number of kinetic energy matrix elements" << endl;
      cout << "Expected " << sqr(nWaves) << ", but found "
           << deltaKin(iSpecies).getSize () << endl;
      SX_QUIT;
   }
   deltaKin(iSpecies).reshape (nWaves, nWaves);
   if (verbose)
      cout << "deltaKin:" << deltaKin(iSpecies) << endl;

   // --- PS Valence density
   node = get (top, "pseudo_valence_density", filename, verbose);
   SxVector<double> rhoValPS = getFunc (node, radB, radGrids, filename)
                             / SQRT_4PI;

   rhoInit(iSpecies) = (rhoValPS + rhoCorePS(iSpecies)) * SQRT_4PI;

   SxVector<double> rhoValAE(nPts), rhoValPS2(nPts);
   rhoValAE.set (0.);
   rhoValPS2.set (0.);

   // --- Maxmal difference for pseudo valence densities
   SX_LOOP(iWave)  {
      double focc = foccInit(iSpecies)(iWave);
      rhoValPS2.plus_assign_ax (focc, phiPS(iSpecies).colRef(iWave).sqr ());
      rhoValAE.plus_assign_ax (focc, phiAE(iSpecies).colRef(iWave).sqr ());
   }
   // factor from spherical harmonics
   rhoValAE /= FOUR_PI;
   rhoValPS2 /= FOUR_PI;
...

   {
      ...
      double QZc = - nuclearCharge(iSpecies)
                 + FOUR_PI * tr (rhoCoreAE(iSpecies) - rhoCorePS(iSpecies));
      ...
      SxVector<double> rhoPS = rhoCorePS(iSpecies) + rhoValPS;
      // xc and Hartree potentials
      SxVector<double> vHdelta;
      // --- cf. Ref 3, Eq. (35) (Bloechl) vs Eq. 89 (Kresse-Joubert)
      vHdelta = SxRadialAtom::getHartreeSmooth (-(QZc + Qv) * g0File);
      vHdelta += SxRadialAtom::getHartreePotential ((QZc + Qv) * g0);
      vBar(iSpecies) -= vHdelta;

      if (kjXcShape.getSize () > 0)  {
         SxVector<double> vXcKJ, vXcPS;
         vXcKJ = SxRadialAtom::computeXC(rad(iSpecies), logdr,
                                             rhoPS + Qv * g0File,
                                             xcFunctional);
         vXcPS = SxRadialAtom::computeXC(rad(iSpecies), logdr,
...
SxVector<double> getMatAtomPAW (const SxList<SxString> &potFileTk,
                                  int nWaves, int matSize,
                                  const SxArray<int> &lPhi,
                                  bool verbose)
{

   SxVector<double> matEl(matSize);
   SxList<SxString> cLine;
   ...
   SxVector<double> res(nWaves,nWaves);
   res.set(0);
   i = 0;
   for (int ipt = 0; ipt < nWaves; ++ipt)  {
      // TODO: l-check!
...
void SxPAWPot::readAtomPAW (const SxString &filename, int iSpecies)
{
...

   {
      ...
      SxVector<double> g0 = getGrl(iSpecies, 0) / FOUR_PI, vScr;
      //SxVector<double> g0 = g0File; // DEBUG only
      g0File.setBasis (&radB);
      g0File.auxData.is = iSpecies;
      g0File.auxData.l = 0;
      ...
      // --- atomic volume correction
      SxVector<double> rhoValAE_PS(nRad);
      rhoValAE_PS.set (0.);
      SX_LOOP(ipt)  {
...
void SxPAWPot::readVasp (const SxString &filename, int is,
                         const SxSymbolTable *table)
{
   ...
   */
   ...
   SxPtr<SxRadialBasis> radGBasis
      = SxPtr<SxRadialBasis>::create (0., gMax * double(nG-1)/double(nG), nG, false);
   rhoInitBasis(is) = radGBasis;
   // prefactor: sqrt(4pi) / sqrt(2pi)^3
   rhoInit(is) = (psValG + psCoreG) * (sqrt(0.5) / PI);
   rhoInit(is).setBasis (*radGBasis);
   rhoInit(is).auxData.is = is;
   rhoInit(is).auxData.l = 0;
   rhoInit(is).auxData.m = 0;
   /*
   if (verbose)  {
      double dr = 0.01;
   ...
   */
   ...
   deltaKin(is) = SxVector<double> (npt, npt);
   deltaKin(is).set (0.);
   ...
   if (projG.getSize () != npt)  {
      ...
      // --- resize deltaKin
      SxVector<double> deltaKinNew(npt, npt);
      for (int ipt = 0; ipt < npt; ++ipt)
         for (int jpt = 0; jpt < npt; ++jpt)
   ...
   fp.find ("uccopancies");
   fp.nextLine ();
   SxVector<double> foccMat (npt, npt);
   ...
   fp.read ("aepotential");
   SxVector<double> vAE = fp.getVector (nRad);
   ...
   rhoCoreAE(is) /= A2B;
   rhoCoreAE(is) /= rad(is).sqr ();
   rhoCoreAE(is).auxData.is = is;
   rhoCoreAE(is).auxData.l = 0;
   rhoCoreAE(is).auxData.m = 0;

   fp.topic ("pspotential");
   fp.find ("pspotential");
   fp.reads (" valence only"); // appears in some recent PAW potentials
   SxVector<double> vPS = fp.getVector (nRad);
   ...
   rhoCorePS(is).auxData.is = is;
   rhoCorePS(is).auxData.l = 0;
   rhoCorePS(is).auxData.m = 0;
   ...
   // --- read wave functions
   phiPS(is) = SxVector<double> (nRad, npt);
   phiAE(is) = SxVector<double> (nRad, npt);
   ...
   int nRadOld = nRad;
   {
      ...
      if (rad(is)(nRad-1) < rMinProj)  {
         ...
         SxVector<double> newPS (nRad + nExtra, npt),
                            newAE (nRad + nExtra, npt);
         for (int ipt = 0; ipt < npt; ++ipt)  {
            ssize_t colOffset = ipt * (nRad + nExtra);
            newPS.getRef(nRad, colOffset) <<= phiPS(is).colRef(ipt);
            newAE.getRef(nRad, colOffset) <<= phiAE(is).colRef(ipt);
            if (newPS(nRad -1, ipt) * newPS(nRad - 2, ipt) > 1e-16 )  {
               ...
               if (a < 0.)  {
                  ...
                  const SxVector<double> &r = rad(is);
         ...
         phiPS(is) = std::move(newPS);
         phiAE(is) = std::move(newAE);
   ...
   pPS(is)     = SxVector<double> (nRad, npt);
   for (int ipt = 0; ipt < npt; ++ipt)  {
   ...
   bool useProjG = table->contains("useProjG")
                 ? table->get("useProjG")->toAttribute ()
                 : false;
   if (useProjG)  {
      cout << "Using VASP G spline projectors" << endl;
      double gLastProj = gMaxProj * double(2*nPtsProjG-1)/double(nPtsProjG);
      projBasis(is)
         = SxPtr<SxRadialBasis>::create (0., gLastProj, 2*nPtsProjG, false);
      pPS(is).reformat (nPtsProjG * 2, npt);
      pPS(is).setBasis (*projBasis(is));
      SX_LOOP(ipt)
         pPS(is).colRef (ipt) <<= projG(ipt);
   ...
   // --- kinetic energy matrix elements 
   SxVector<double> kinNum(npt, npt);
   kinNum.set (0.);
   ...
   SxVector<double> rhoValPsBare(radB(is)), rhoValAE(radB(is));
   rhoValPsBare.set (0.);
   rhoValAE.set (0.);
   for (int ipt = 0; ipt < npt; ++ipt)  {
      for (int jpt = 0; jpt < npt; ++jpt)  {
   ...
   SxVector<double> rhoValPsAug = FOUR_PI * toRSpace (psValG, gMax, rad(is));
   for (int ir = nRadOld; ir < nRad; ++ir)  {
      rhoValPsBare(ir) = rhoValAE(ir) = rhoValPsAug(ir);
   }
   SxVector<double> g0File2 = (rhoValPsAug-rhoValPsBare);
   ...
   */
   ...
   rhoValAE.auxData.is = is;
   rhoValAE.auxData.l = 0;
   rhoValAE.auxData.m = 0;
   rhoValPsAug.auxData.is = is;
   rhoValPsAug.auxData.l = 0;
   rhoValPsAug.auxData.m = 0;
   rhoValPsAug.setBasis (&radB);
   rhoValPsBare.auxData.is = is;
   rhoValPsBare.auxData.l = 0;
   rhoValPsBare.auxData.m = 0;

   {
      SxVector<double> nHat = rhoValPsAug - rhoValPsBare;
      nHat *= sqrt(FOUR_PI);
      nHat.auxData = rhoValPsAug.auxData;
      rhoInit(is) -= (*radGBasis) | nHat;
   }

   if (table->contains ("recomputeTail")
       && table->get("recomputeTail")->toAttribute ())
   {
      double rMin = rad(is)(0);
      double rMax = 20.;
      int nR = (int)lrint(log(rMax/rMin)/logdr) + 1;
      rMax = rMin * exp(logdr * (nR-1));
      const SxRadialBasis radTail(rMin, rMax, nR, true, SxRadialBasis::Logarithmic);
      const SxVector<double> &radR = radTail.getRadFunc ();

      SxVector<double> rho = toRSpace(psValG, gMax, radR, 0),
                       rhoCore = toRSpace(psCoreG, gMax, radR, 0);
      
      rho.setBasis (radTail);
      rho.auxData.is = is;
      rho.auxData.l = 0;
      rho.auxData.m = char(0);
      cout << "Q(rho)=" << FOUR_PI * radTail.tr (rho) << endl;
      //cout << "Q(rhoH)=" << FOUR_PI * radTail.tr (rhoH) << endl;
      SxVector<double> V = SxRadialAtom::getHartreeSmooth (rho)
                         + SxRadialAtom::computeXC(radR, logdr, rho + rhoCore, xcf)
                         + toRSpace(vLoc, gMax, radR)
                         - valenceCharge(is) * erf(radR / betaLoc) / radR;
      V.setBasis (radTail);
      writePlot ("V-tail.dat", radTail.getRadFunc (), V);
      recomputeTail (is, V, nRadOld - 1);
   }
   ...
   SxVector<double> vZcPS = toRSpace (vLoc, gMax, rad(is));
   // add long-range part to vLoc subtracted above
   vZcPS -= valenceCharge(is) * erf(rad(is) / betaLoc) / rad(is);

   {
      /*
      double r0 = rad(is)(0);
      rShape = r0 * exp(logdr * int(log(rShape/r0)/logdr));
      SxVector<double> g0File = getJsbShape (rShape, 0, rad(is), logdr);
      g0File.auxData.is = is;
      g0File.auxData.l = 0;
      g0File.auxData.m = 0;
      ...
      */
      ...
      SxVector<double> vZcAE = (-nuclearCharge(is))/rad(is) +
      ...
      SxVector<double> vPsFile = vPS, vAeFile = vAE;
      ...
      deltaS(is)   = SxVector<double> (npt, npt);
   ...
   if (kjxc)  {
      ...
      kjXcShape(is).auxData.is = is;
      kjXcShape(is).auxData.l = 0;
      kjXcShape(is).auxData.m = 0;
   }
   ...
   SxVector<double> grl = getGrl (is, 0);
   grl.setBasis (&radB);
   grl /= FOUR_PI;
   g0File2 /= FOUR_PI;
   ...
   rhoCoreAE(is).auxData.basisPtr = NULL;
   rhoCorePS(is).auxData.basisPtr = NULL;
...
void SxPAWPot::readCPPAW (const SxString &filename, int iSpecies)
{
   ...
   rPAW(iSpecies) = rc(iSpecies) * sqrt(-log(1e-4)); // best guess of rPAW
   ...
   // --- read vBar
   fp.topic ("vBar");
   SxVector<double> &vBarRef = vBar(iSpecies);
   vBarRef.resize (nr);
   for (i=0; i < nr; /*empty*/)  {
      line = fp.getLine ();
   ...
   fp.topic ("delta kin");
   SxVector<double> dtKin(nwave,nwave);
   ...
   fp.topic ("dover");
   SxVector<double> dover(nwave,nwave);
   ...
   SxVector<double>  proj (nr,nwave);   // :i,:ir
   SxVector<double>  phiAE_(nr,nwave);   // :i,:ir
   SxVector<double>  phiPS_(nr,nwave);   // :i,:ir
   int r;
   for (i=0; i < nwave; i++)  {
      fp.topic ("proj " + (i+1));
      // --- projector(lPhi(i))
   ...
   // --- atomic volume correction
   SxVector<double> rhoValAE_PS(nr);
   rhoValAE_PS.set (0.);
   SX_LOOP(ipt)  {
      rhoValAE_PS.plus_assign_ax(foccInit(iSpecies)(ipt)/FOUR_PI,
                                   nij(phiAE(iSpecies), ipt, ipt)
...
SxRadialMesh SxPAWPot::computeRhoPS (const SxVecRef<double> &Dij, 
...
SxRadialMesh SxPAWPot::computeRhoAE (const SxVecRef<double> &Dij,
                                     int is, int nSpin) const
{
   SX_CLOCK (Timer::ComputeRhoRadial);
...
SxRadialMesh SxPAWPot::computeRho (const SxVecRef<double> &Dij, int is,
                                   const SxVecRef<double> &phi) const
{
   SX_CHECK (phi.getNCols () == lPhi(is).getSize (),
             phi.getNCols (), lPhi(is).getSize ());

   int nr = (int)phi.getNRows ();
   ...
   for (int ipt = 0; ipt < getNProjType(is); ++ipt)  {
      ...
      for (int jpt = 0; jpt < getNProjType(is); ++jpt)  {
         ...
         SxVector<double> phiIphiJ = nij(phi, ipt, jpt);
...
SxPAWPot::getAtomRhoG(const SxGBasis &G, int iSpecies) const
{
   ...
   SxVector<double> rhoRad (getRadBasis ()((int)iSpecies));
   rhoRad <<= rhoCorePS (iSpecies);
   double Y00 = SQRT_1_4PI;
   SxVector<double> r3 = rad(iSpecies).cub ();
   for (int ipt = 0; ipt <lPhi(iSpecies).getSize (); ++ipt)  {
      double focc    = foccInit(iSpecies)(ipt);
      SxVector<double> psRho = phiPS(iSpecies).colRef(ipt).sqr ();
   ...
   rhoRad.auxData.is = iSpecies;
   rhoRad.auxData.l  = 0;
   rhoRad.auxData.m  = 0;
   return ( G | rhoRad);
}

SxVector<double> SxPAWPot::getGrl (int is, int l) const
{
   ...
   // Ref. 1, Eq. 23
   SxVector<double> res = pow (rad(is), double(l))
   ...
   res.auxData.basisPtr = radBasisPtr.ptr; // may be NULL
   return res;
}

SxVector<double> SxPAWPot::getVMatEl (const SxRadialMesh &V,
                                      const SxVecRef<double> &phi) const
{
   SX_CLOCK (Timer::RadVMatEl);
   int is = V.meshData.auxData.is;
   SX_CHECK (is >= 0 && is < getNSpecies (), is, getNSpecies ());
   /// Only phiPS/phiAE is allowed
   SX_CHECK (&phi == &phiPS(is) || &phi == &phiAE(is));
   int np = getNProj(is);
   SxVector<double> res(np, np);
   res.set (0.);
   SxVector<double> r3 = rad(is).cub ();
   ...
   for (int ipt = 0; ipt < getNProjType(is); ++ipt)  {
      ...
      for (int jpt = 0; jpt < getNProjType(is); ++jpt)  {
         ...
         SxVector<double> phiIphiJr3 = phi.colRef(ipt) * phi.colRef(jpt) * r3;
...
void SxPAWPot::recomputeOverlap ()
{
   double ovlp;
   for (int is = 0; is < getNSpecies (); ++is)  {
      deltaS(is) = SxVector<double> (getNProjType(is), getNProjType(is));
      for (int ipt = 0; ipt < getNProjType(is); ++ipt)  {
...
{
   if (dynamic_cast<const SxRadialBasis *>(pPS(iSpecies).getBasisPtr ()))
      return;
   // ---
   ...
   SxVector<double> SP(nProjType,nProjType);
   SxVector<double> rCube = rad(iSpecies).cub ();
   ...
   SxVector<SxComplex16> SSP = deltaS(iSpecies) ^ SP;
   SxEigensystem<SxComplex16> eig (SSP, All, true);
   // add the 1 to get the overlap eigenvalues
   eig.vals += 1.;
   cout << SX_SEPARATOR;
   cout << "| PAWPotential Overlap Check. Eigenvalues are:" << endl << '|';
   SX_LOOP (ipt) cout << ' ' << eig.vals(ipt).re;
   cout << endl;
   bool problem = false;
   ...
   if (problem) {
      double dg = 0.01;
      int ng = int(10./dg);
      SxVector<double> SPfilt(ng,nProjType);
      SX_LOOP2(ipt,ig)  {
         double g = double(ig) * dg;
         SxVector<double> bessel = SxRadialBasis::jsb(lPhi(iSpecies)(ipt),
                                                      rad(iSpecies) * g);
      ...
      for (double G2 = 95.; G2 > 9.; G2 -= 5.)  {
         ...
         eig.compute (std::move (SSP), All, true);
         eig.vals += 1.;
...
void SxPAWPot::extendRad(double newRMax)
{
   ...
   for(int is = 0; is < nSpecies; is++)
   {
      ...
      SxVector<double> oldRad       = rad(is),
                       oldRhoCorePS = rhoCorePS(is),
                       oldRhoCoreAE = rhoCoreAE(is),
                       oldVBar      = vBar(is);
      rad(is) = SxVector<double>(newSize);
      ...
      int nOrbTypes = (int)phiPS(is).getNCols ();
      // Build new phiPS, phiAE, pPS
      SxVector<double> oldPhiPS = phiPS(is),
                       oldPhiAE = phiAE(is),
                       oldpPS = pPS(is);
      ...
      for (int iot = 0; iot < nOrbTypes; iot++)
      {
         ...
         file += iot;
         file += ".org";
         SxTextIO (file).writeXYPlot(rad(is),phiPS(is).colRef(iot));
      }
   }
   // Set new Basis
   setBasis(SxPtr<SxRadBasis>::create(rad, logDr));
...
void SxPAWPot::setupQijL ()
{
   ...
   for (int is = 0; is < getNSpecies (); ++is)  {
      ...
      if (fabs(rPAW(is) / rPaw - 1.) > 0.001)  {
         cout << SX_SEPARATOR;
         cout << "| WARNING! The SPHInX estimate of rPAW for "
              << prettyName(is) << " is changed!" << endl;
         cout << "| Value prior to SPHInX 3.1: rPAW = " << rPaw << endl;
         cout << "| Current value rPAW = " << rPAW(is) << endl;
         cout << SX_SEPARATOR;
      }
      rPaw = rPAW(is);
      omegaPAW(is).reformat (npt, npt);
      SxVector<double> rW (rad(is).getSize ());
      SX_LOOP(ir)  {
         double r = rad(is)(ir);
...
void SxPAWPot::computeXKernel ()
{
   ...
   SxArray<SxArray2<SxVector<double> > > nijL;
   ...
   for (int is = 0; is < getNSpecies (); ++is)  {
      ...
      SxArray<SxVector<double> > gL (lMaxRho(is) + 1);
      for (int l = 0; l <= lMaxRho(is); ++l)
         gL(l) = getGrl (is, l);
      ...
      for (int ipt = 0; ipt < npt; ++ipt)  {
         ...
         for (int jpt = ipt; jpt < npt; ++jpt)  {
            ...
            SxVector<double> nijAE = nij(phiAE(is), ipt, jpt);
            SxVector<double> nijPS = nij(phiPS(is), ipt, jpt);
            nijAE.auxData.is = is;
            nijAE.auxData.m = 0;
            nijPS.auxData.is = is;
            nijPS.auxData.m = 0;
            for (int kpt = ipt; kpt < npt; ++kpt)  {
               ...
               for (int lpt = (ipt == kpt) ? jpt : kpt; lpt < npt; ++lpt)  {
                  ...
                  SxVector<double> nklAE = nij(phiAE(is), kpt, lpt);
                  SxVector<double> nklPS = nij(phiPS(is), kpt, lpt);
                  nklAE.auxData.is = is;
                  nklAE.auxData.m = 0;
                  nklPS.auxData.is = is;
                  nklPS.auxData.m = 0;
                  SxVector<double> nijSoft, nklSoft;
                  for (int l = 0; l <= lMaxRho(is); ++l)  {
                         ...
                         || (l < lmin) || (l > lmax))
                     {
                        ...
                        nijAE.auxData.l = char(l);
                        nklAE.auxData.l = char(l);
                        nijSoft = nijPS + QijL(is)(ipt, jpt)(l) * gL(l);
                        nklSoft = nklPS + QijL(is)(kpt, lpt)(l) * gL(l);
                        nijSoft.auxData.l = char(l);
                        nklSoft.auxData.l = char(l);
      ...
      - SxRadialAtom::computeScreenedHartree (nijSoft, nklSoft, 0., dg);
                        ...
                        if (ij == kl)  {
                           SxVector<double> deltaNijR3 = nijAE - nijSoft;
        ...
        F(0,l+2*m+2) * (deltaNijR3 * pow(rad(is),l+2*m+2)).integrate(logDr(is));
                           ...
                           if (verbose)  {
                              ...
                              for (int ig = 0; ig < nG; ++ig)  {
                                 SxVector<double> jsb
                                    = SxRadialBasis::jsb (l, (ig * dg) * rad(is));
...
void SxPAWPot::computeCoreX (int is)
{
   ...
   for (int jpt = 0; jpt < npt; ++jpt)  {
      ...
      for (int iCore = 0; iCore < nCore; ++iCore)  {
         ...
         SxVecRef<double> psiC = psiCoreAE(is).colRef (iCore);
         SxVector<double> rhoPhiCore = phiAE(is).colRef (jpt) * psiC;
         for (int l = abs(lc - lv); l <= lc + lv; l+=2)  {
            ...
            rhoPhiCore.auxData.l = char(l);
            SxVector<double> psiVl;
   ...
   if (SxXCFunctional::omegaHSE > 1e-10)  {
      ...
      SxArray2<SxVector<double> > rhoPhiCore(npt,nCore);
      for (int iCore = 0; iCore < nCore; ++iCore)  {
         SxVecRef<double> psiC = psiCoreAE(is).colRef (iCore);
      ...
      for (int l = 0; l <= lCoreMax + lMax(is); ++l)  {
         for (int ig = 0; ig < ng; ++ig)  {
            ...
            SxVector<double> jsbR3 = SxRadialBasis::jsb(l, g * rad(is));
            jsbR3 *= r3;
...
SxArray<SxArray<SxVector<double> > > SxPAWPot::getPhiPS ()
{
   SX_CHECK (radBasisPtr);
   int nSpecies = getNSpecies();
   SxArray<SxArray<SxVector<double> > > result(nSpecies);
   for(int is = 0; is < nSpecies; is++)   {
      int nProj = (int)phiPS(is).getNCols ();
      result(is).resize(nProj);
      for(int ip = 0; ip < nProj; ip++)
         result(is)(ip) = getPhiPS(is, ip);
   }

   return result;
}

SxVector<double> SxPAWPot::getPhiPS (int is, int ip)
{

   SxVecRef<double> result;
   if (phiLCAO.getSize () > 0 && phiLCAO(is).getSize () > 0)  {
      result = phiLCAO(is).colRef(ip);
      result.setBasis(*radBasisLCAO);
   } else {
      result = phiPS(is).colRef(ip);
      result.setBasis(*radBasisPtr);
   }
   result.auxData.setAtom (is, -1);
   result.auxData.setOrb (ip, lPhi(is)(ip), NONE_M);
   return const_cast<const SxVecRef<double>&>(result);
}

void writePlot (const SxString &fileName,
                const SxVecRef<double> &x,
                const SxVecRef<double> &y1,
                const SxVecRef<double> &y2,
                const SxVecRef<double> &y3)
{ 
