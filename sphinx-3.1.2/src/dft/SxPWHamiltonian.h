...
class SX_EXPORT_DFT SxPWHamiltonian : public SxHamiltonian
{
   public:
      ...
      */
      ...
      PsiG operator* (const PsiRef &) const;
      PsiG H_PsiG  (const PsiRef &) const;
      PsiG H_PsiGI (const PsiRef &) const;
      ...
      SxVector<double>
      preconditioner (const PsiRef &, Preconditioner type) const;
      ...
      SxArray<SxVector<double> >  phiGaussG;   //:is,:ig
      ...
      SxMeshR                         vLocR;
      SxArray<SxVector<double> >  phiLocPseudo;  //:is,:ig
      ...
      getMatrix (const SxGkBasis &gkBasis, int iSpin, int ik, int nG=-1);
      ...
      SxVector<PrecCoeffG> getNlMatrix (const SxMuPW &mu, int ik,
