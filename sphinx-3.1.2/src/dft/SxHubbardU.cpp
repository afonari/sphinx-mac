...
{
   cout << "Hubbard U in " << table->parserFilename
   ...
   SYMBOLPARSE(table)  {
      SYMBOLGROUP("HubbardU")  {
         ...
         FOREACH_SYMBOLGROUP("AO")  {
            ...
            ssize_t iHubMO = moSite.getSize ();
            moSite.resize (iHubMO + 1, true);
            SxPtr<SxHubbardMO> hubMO = SxPtr<SxHubbardMO>::create ((int)nSites);
            moSite(iHubMO) = hubMO;
            // read
            hubMO->readAO (SYMBOLGROUP_TABLE, structure, pawPotPtr);
         ...
         FOREACH_SYMBOLGROUP("fragment")  {
            // create new MO site
            ssize_t iHubMO = moSite.getSize ();
            moSite.resize (iHubMO + 1, true);
            moSite(iHubMO) = SxPtr<SxHubbardFO>::create ((int)nSites);
            // read
            moSite(iHubMO)->read (SYMBOLGROUP_TABLE, structure, pawPotPtr);
...
void SxHubbardU::readUpdate (const SxSymbolTable *table)
{
   SX_CHECK (table->name == "updateHubbard", table->name);
   SYMBOLPARSE(table)  {
      SxArray<int> idList = SYMBOLGET("siteIds");
      bool setAlpha = HAVE_SYMBOL("shift"),
           setU     = HAVE_SYMBOL("U");
      double alpha = (SYMBOLGET("shift") || 0.) / HA2EV,
             U     = (SYMBOLGET("U")     || 0.) / HA2EV;

      if (setAlpha && alphaBias.getSize () == 0)  {
         alphaBias.resize (Ueff.getSize ());
         alphaBias.set (0.);
      }
      SX_LOOP(iSite)  {
         int siteId = idList(iSite);
         if (siteId < 1 || siteId > Ueff.getSize ())  {
            cout << "Illegal site " << siteId << endl;
            cout << "Allowed range is 1.." << Ueff.getSize () << endl;
            SX_QUIT;
         }
         siteId--; // switch to 0..nSite-1
         // now update
         if (setU)     Ueff     (siteId) = U;
         if (setAlpha) alphaBias(siteId) = alpha;
      }
      if (verbose)  {
         cout << "Setting";
         if (setU)     cout << " U=" << (U * HA2EV) << " eV";
         if (setAlpha) cout << " shift=" << alpha << " eV";
         cout << " for sites " << idList << endl;
      }
      cout << alphaBias << endl;
      cout << Ueff << endl;
   }
