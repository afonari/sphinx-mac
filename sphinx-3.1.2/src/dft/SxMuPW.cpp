...
void SxMuPW::addOrbital (int iSpecies, int orbitalType)
{
   ...
   const SxAuxData &aux = mu(iSpecies, orbitalType).auxData;
   int n = aux.n;
   int l = aux.l;
   ...
   for (int ik=0; ik < Gk.nk; ik++)  {
      if (SxLoopMPI::myWork(ik)) {
         for (int m=-l; m <= l; m++)  {
            if (viaRadG)  {
               orbital.auxData.m = char(m);
               muPhiList(ik) << ( Gk(ik) | orbital );
            } else {
               muPhiList(ik) << ( Gk(ik) | mu(iSpecies,-1,n,l,m) );
            }
            SxAuxData &auxData = muPhiList(ik).last ().auxData;
            auxData.is = iSpecies;
            auxData.setOrb (n, l, m);
         }
      }
   }
...
SxVecRef<PrecCoeffG> SxMuPW::operator() (int i, int iSpin, int ik)
{
...
void SxMuPW::finalize ()
{
   ...
   for (int ik=0; ik < gkBasisPtr->nk; ik++)
   {
      ...
      if (SxLoopMPI::myWork(ik))
      {
         // move vectors from SxList<SxVecRef<..> > into SxArray<SxVector<..> >
         muPhi(ik).resize (muPhiList(ik).getSize ());
         SxList<SxVecRef<PrecCoeffG> >::Iterator it = muPhiList(ik).begin ();
         for (int iOrb = 0; iOrb < muPhi(ik).getSize (); iOrb++, it++)
            muPhi(ik)(iOrb) = std::move(*it);
      } else {
         muPhi(ik).resize (0);
      }
   }
...
# ifndef NDEBUG
   ...
   for (int i = 0; i < muPhi(0).getSize () - 1; i++)   {
      SxAuxData &auxDataI = muPhi(0)(i).auxData;
      for (int j = i+1; j < muPhi(0).getSize (); j++)   {
         SxAuxData &auxDataJ = muPhi(0)(j).auxData;
         if ( (auxDataI.is == auxDataJ.is) && (auxDataI.l == auxDataJ.l) 
               && (auxDataI.m == auxDataJ.m))   {
            SX_CHECK (auxDataI.n != auxDataJ.n, (int)auxDataI.n, (int)auxDataJ.n);
         }
      }
   }
# endif
