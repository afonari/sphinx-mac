...
SxVector<double>
SxSpinConstraint::getSpinsDiag (const Eps     &eps0In,
                                      SxFermi &fermiIn,
                                const double  &ektIn,
                                const SxPtr<SxPAWSet> &wavesPtr)
{
   SX_CHECK (wvPtr);
...
#           pragma omp for
            ...
            if (nThreads > 1)  {
               ...
               while (nij & 3) nij++;
               if (nij<128) nij=128; // avoid tiny slices
               ssize_t ijFrom = iThread * nij;
               ssize_t ijTo = min(ijFrom + nij, Hnew.getSize ());

               // --- avoid cache line conflicts
               // leave a space of 8 elements at the beginning of each slice
               // as it could share the cache line with end of preceding slice
               ssize_t ijFromSave = ijFrom + 8;
               for (int jThread = 1; jThread < nThreads; ++jThread)  {
                  const SxVector<SxComplex16> &Hj = extraH(jThread);
#                 pragma omp simd
                  for (ssize_t ij = ijFromSave; ij < ijTo; ++ij)
                     Hnew(ij) += Hj(ij);
               }
               // ... wait for all ...
#              pragma omp barrier
               // ... and now handle the save space
               ijTo=min(ijFromSave, ijTo);
...
#        else
            // --- insane workaround for MKL(?) bug: check & recompute
            //     symptom: in very rare cases the diagonalization was invalid
            //     but ended without LAPACK error. This then ruined the
            //     convergence of the nu optimization loop.
            for (int iTry = 0; iTry < 5; ++iTry)  {
               // diagonalize a copy of Hnew
               eig.compute (SxVector<SxComplex16>(Hnew), All, true);
               // check diagonalization
               SxVector<SxComplex16> Hrot = eig.vecs.overlap (Hnew ^ eig.vecs);
               Hrot.diag () -= eig.vals;
               double error = Hrot.normSqr ();
               if (error>=1e-20 || iTry > 0)
                  cout << "Hrot check: "  << error << endl;
               if (error < 1e-20) break; // buggy cases: error is of order 1e0
               if (iTry == 0)  {
                  FILE *fp = sxfopen (SxString("failedmat-") + error + ".dat", "w");
                  int N = (int)Hrot.getNCols ();
                  fwrite (&N, sizeof(N), 1, fp);
                  fwrite (Hnew.elements, sizeof(SxComplex16), Hnew.getSize (), fp);
                  fclose (fp);
               }
               if (iTry == 4) { SX_EXIT; } // workaround failed
               // --- mysterious fix of diagonalization bug
               // H should be Hermitean [and numerically is so], so enforce it
               // CF 2025-09-18: it seems that numerical noise from this
               // operation is sufficient to not trigger the bug again
               Hnew += Hnew.adjoint (); Hnew *= 0.5;
            }
#        endif
            fermiIn.eps(iSpin,ik) <<= eig.vals;
         }
         ...
         // --- rotate omegann according to new eigenfunctions
         {
            ...
            SX_LOOP(iAtom)
               if (constrainedAtom(iAtom))
                  omega(iAtom)(iSpin, ik) = omegaN(iAtom).real ();
         }
   ...
   for (int iAtom = 0; iAtom < nAtom; iAtom++) {
      ...
      for (int iSpin = 0; iSpin < nSpin; iSpin++)
         for (int ik = 0; ik < nk; ik++)
            if (SxLoopMPI::myWork(ik))
               MSpin(iAtom) += dot(omega(iAtom)(iSpin,ik), fermiIn.focc(iSpin,ik))
                             * fermiIn.kpPtr->weights(ik);
   }
