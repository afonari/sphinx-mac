...
class SX_EXPORT_DFT SxRho : public SxDensity
{
   public:
      ...
      // and the assignment from generic SxDensity ...
      using SxDensity::operator=;
...
//                                 Real8 spinMoment=0.);
      ...
      */
      void displaceHirshfeld (const SxAtomicStructure &toStr,
                              const SxArray<SxVector<PrecCoeffG> > &atomRhoG,
                              SxVector<double> *allAtomRptr = NULL);

      SxArray<double>
      getHirshfeldEffVolumes (const SxArray<SxVector<double> > &atomRhoG,
                              const SxPAWPot & pawPot,
                              double gCut,
                              SxVector<double> *allAtomRptr = NULL);
      SxArray<double>
      getHirshfeldEffCharges (const SxArray<SxVector<double> > &atomRhoG,
                              const SxPAWPot & pawPot,
                              double gCut,
                              SxVector<double> *allAtomRptr = NULL);
