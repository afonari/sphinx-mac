...
SxHamSolver::SxHamSolver()
   : energy(-1.), rhoMixing (1.), foccMixing(1.), deltaT(0.), dPsiConv(0.),
     dEnergy(0.), dEps(0.), dRelEps(0.), dRelR(0.), ekt(0.), ektDefault(0.),
     maxSteps (-1), nSpin(-1), nStates(-1),
     nElectrons(0.), keepRho(false), keepOcc(false)
{
   // empty
}
     ...
     R  (G.fft3d(0).mesh, str.cell)                  // |R>
{
   ...
   SX_OUTPUT_FILE ("energy.dat");
   SX_OUTPUT_FILE ("residue.dat");
   SX_OUTPUT_FILE ("spins.dat");
   ...
   if (potPtr)
      ...
      nElectrons = 3.; // historic setting for k.p (from Al...)
   ...
   if (potPtr)
      ...
      SxAtomicStructure::printCell (str.cell);
}
                         ...
                         bool saveMemory)
{
   ...
   try  {
      ...
      int nComp = SxHamInit::getNComp (&*tablePtr);
      wavesPtr = SxPtr<SxPW>::create(nStates,nSpin,gkPtr, nComp, tmpDir);
                        ...
                        SxConstPtr<SxSymbolTable> tablePtr)
{
   ...
   try  {
      ...
      if (structure.getNAtoms() == 0)  {
         // try to read missing structure from rho file
         if (io.contains ("tau"))
            structure.read (io);
         else
            // otherwise: input file
...
class SxHamSolver::InternalCommand : public SxHamSolver::Command
{
   public:
      /// Pointer to member function
      typedef void (SxHamSolver::*MemFunc)(const SxSymbolTable *, bool);
      /// Constructor
      InternalCommand (MemFunc funcIn, bool saveDataIn = true)
         : func(funcIn), saveData(saveDataIn) {}
      /// Execute the wrapped internal command function
      virtual bool execute (SxHamSolver *solver,
                            const SxSymbolTable *cmd,
                            bool calc) override
      {
         (solver->*(this->func)) (cmd, calc);
         return saveData;
      }
      /// Destructor
   ...
   protected:
      ...
      MemFunc func;
      /// whether to save afterwards
      bool saveData;
};
#define HSFUNC(xx)        SxPtr<InternalCommand>::create \
                          ((InternalCommand::MemFunc)&SxHamSolver::xx, true)
#define HSFUNC_NOSAVE(xx) SxPtr<InternalCommand>::create \
                          (&SxHamSolver::xx, false)
void SxHamSolver::setupCommandTable ()
{
   cmdTable("initialGuess")  = HSFUNC(initialGuess);
   cmdTable("SD")            = HSFUNC(steepestDescent);
   cmdTable("scfDiag")       = HSFUNC(scfDiagonalization);
   cmdTable("subspaceDiag")  = HSFUNC(subspaceDiagonalization);
   cmdTable("CCG")           = HSFUNC(allStateCG);
   cmdTable("bandStructure") = HSFUNC(bandStructureCG);
   cmdTable("hSqrCG")        = HSFUNC(hSqrCG);
   cmdTable("diffEqtn")      = HSFUNC(diffEqtn);
   cmdTable("directDiag")    = HSFUNC(directDiagonalization);
   cmdTable("linGrad")       = HSFUNC_NOSAVE(linGradTest);
   cmdTable("lcao")          = HSFUNC(tightBindingPAW);
}
#undef HSFUNC
#undef HSFUNC_NOSAVE
...
{
   if (!cmd) return false;
   SX_CHECK (cmdTable.hasKey ("initialGuess"));
   SxString str = cmd->getName();
   return (str == "evalForces" || cmdTable.hasKey (str));
}
...
void SxHamSolver::execute (const SxSymbolTable *cmd, bool calc)
{
   ...
   SYMBOLPARSE(cmd) {
      if (cmdTable.hasKey (str)) {
         bool save = cmdTable(str)->execute (this, cmd, calc);
         // --- write data to disc
         if (save && calc) {
            writeData (!SYMBOLGET("noWavesStorage").toBool (),
                       !SYMBOLGET("noRhoStorage").toBool ());
         }
      } else if (str == "evalForces") {
         SxAtomicStructure force = getSymForces (structure, NULL);
         SX_MPI_MASTER_ONLY {
            SxString fname = SYMBOLGET("file") || "forces.sx";
   ...
   if (calc)
      printTiming (/* restart= */true, /* final= */ str != "initialGuess");
}
                                ...
                                const SxGkBasis &gk)
{
   ...
   if (dynamic_cast<SxPAWHamiltonian *>(hamPtr.getPtr ()))   {
      ...
      if (TBInFile.getSize () > 0 && SxFileInfo(TBInFile).exists ())  {
         ...
         // get RadBasis
         radBasisPtr = potPAW.getBasisLCAO ();
         // --- collect LCAO orbitals with initial occupation > 0
         SxArray<SxArray<SxVector<double> > > usedPhiPS(structure.getNSpecies());
         for (int is = 0; is < structure.getNSpecies(); is++)   {
            ...
            for (int iupl = 0; iupl < nUsedProjectors; iupl++)   {
               ...
               usedPhiPS(is)(iupl) = potPAW.getPhiPS (is, ipl);
               cout << is << "\t" << ipl << "\t" << potPAW.lPhi(is)(ipl) << endl;
            }
         }
         TBOrbitals = SxAtomicOrbitals (usedPhiPS, radBasisPtr);
      }
      //SX_MPI_MASTER_ONLY { TBOrbitals.print ("AtomicOrbitals"); }

      // TB orbitals in G+k basis
      aoBasisPtr = SxPtr<SxAOBasis>::create (gk, TBOrbitals.getMuSet (), SPtr);
   } else {
      ...
      for (int ik = 0; ik < wavesPtr->getGkBasis ().getNk (); ik++)  {
         ...
         if (SxLoopMPI::myWork(ik)) {
            ...
            for (int iSpin = 0; iSpin < nSpin; iSpin++)  {
               gk.changeTau (structure);
               SxVector<SxComplex16> aoS = aoBasis.calculateOverlap (ik);
               SxVector<SxComplex16> invS = aoS.inverse ();

               PsiRef psiG = gk | (*wavesPtr)(iSpin, ik);
               PsiG psiAO = invS ^ (aoBasis | psiG);
               psiAO.setBasis (aoBasis);

               psiG -= gk | psiAO;

               cout << "Avg. square residue at ik=" << (ik+1) 
                  << ": " << ((psiG.conj () * (S * psiG)).sum ().re / (double)psiG.getNCols ())
                  << endl;
               ...
               //cout << "Before ortho: " << psiG.overlap (S * psiG) << endl;
                                          ...
                                          const SxSymbolTable *cmd)
{
   ...
   if (dynamic_cast<SxPWHamiltonian *>(hamPtr.getPtr ()))   {
      ...
      if (tauChanged)  {
         ...
         SxArray<SxVector<PrecCoeffG> > atomRho(potPtr->getNSpecies ());
         SxVector<PrecCoeffG> allAtom(0., G);
         SX_LOOP(is)  {
            atomRho(is) = pot.getAtomRhoG (G, (int)is);
            allAtom += atomRho(is) * G.getStructureFactor (is);
         }
         ...
         allAtom.set (0.);
         SX_LOOP(is) allAtom += atomRho(is) * G.getStructureFactor (is);
         allAtomR = R | allAtom;
         SX_LOOP(iSpin)
            H.rho(iSpin).plus_assign_ax(+1./nSpin, allAtomR);
      ...
      if (tauChanged)  {
         ...
         if (Hirshfeld)  {
            SxArray<SxVector<PrecCoeffG> > atomRhoG (structure.getNSpecies ());
            SX_LOOP(is)  {
      ...
      if (muBasisPtr.getPtr())  {
         ...
         for (int ik = 0; ik < nk; ik++)  {
            ...
            for (int iSpin = 0; iSpin < nSpin; iSpin++)  {
               ...
               SxVector<double> epsilon (nWStates);
               for (int iState = 0; iState < nWStates; iState++)  {
                  PsiRef psi = gk(ik) | waves (iState, iSpin, ik);
               ...
               // construct overlap
               SxVector<SxComplex16> Snm (nOrbs, nOrbs);
               for (int iOrbital = 0; iOrbital < nOrbs; iOrbital++)  {
                  const PsiRef &phiI = aoBasis(iOrbital,iSpin,ik);
                  PsiRef SPhiI = S * phiI;
                  for (int jOrbital = iOrbital; jOrbital < nOrbs; jOrbital++)  {
                     const PsiRef &phiJ = aoBasis(jOrbital,iSpin,ik);
               ...
               SxVector<SxComplex16> invS = Snm.inverse ();

               // construct coefficients
               SxVector<SxComplex16> Cnmu(nStates, nOrbs);
               PsiRef SPsi = S * (gk(ik) | waves(iSpin,ik));
               for (int iOrbital = 0; iOrbital < nOrbs; iOrbital++)  {
                  Cnmu.colRef(iOrbital) <<= SPsi.overlap (aoBasis(iOrbital,iSpin,ik));
               }
               Cnmu = (Cnmu ^ invS).conj ();
               ...
               for (int iState = 0; iState < nWStates; iState++)  {
                  SxVector<PrecCoeffG> diff = 1.0 *  (gk(ik) | waves (iState, iSpin, ik));
                  for (int iOrbital = 0; iOrbital < nOrbs; iOrbital++)  {
...
{
   SX_CHECK(!hamPtr);
   if (cmd->containsGroup ("initialGuess"))
      cmd = cmd->getGroup ("initialGuess");
              ...
              || topLvl->containsGroup ("Hamiltonian")) 
   {
      ...
      initializer = SxPtr<SxInitKP>::create (SxInitKP::KdotPgeneral, &R, topLvl);
   ...
   if (topLvl->containsGroup ("spinConstraint")) {
      if (nSpin != 2)  {
         cout << SX_SEPARATOR;
         cout << "ERROR: Cannot use spin constraints for non-spinpolarized "
                 "calculation" << endl;
         cout << "Set 'spinPolarized' flag in the PAWHamiltonian group!" << endl;
         SX_QUIT;
      }
   ...
   if (rhoGroup && rhoGroup->containsGroup ("charged"))  {
      SxVector<PrecCoeffG> extraCharge(0., G);
      SYMBOLPARSE(rhoGroup)  {
         FOREACH_SYMBOLGROUP("charged")  {
            ...
            if (HAVE_SYMBOL("z"))  {
               ...
               cout << "Adding Gaussian charge (Q=" << charge
                    << ") at " << pos << endl;
               SxVecRef<PrecCoeffG> T = G.getPhaseFactors (pos);
               SxVector<double> gauss = exp((-0.5*beta*beta) * G.g2);
   ...
   } else if (dynamic_cast<SxInitKP*>(initializer.getPtr()))  {
      if (calc) initializer->randomize (wavesPtr);
   } else if (!pawHam)  {
...
void SxHamSolver::tightBinding (const SxSymbolTable *cmd, bool calc)
{
   ...
   SxAOBasis initBasis (*gkPtr, TBOrbitals.getMuSet ());
   ...
   SxVector<PrecCoeffG> H(nOrb,nOrb), S;
   
   SxArray<SxVector<PrecCoeffG> > Lk(nk), Hkstat(nk);
   ...
   for (int it = 0; it < maxSteps; it++)  {
    ...
    = hamPW.rsProjector->project (hamPW.phiNl(ik)(iPhi),
                        ...
                        //     with mu(i,iSpin,ik)
                        SxVector<PrecCoeffG>::Iterator pIt
                  ...
                  for (int iOrb = 0; iOrb < nOrb; ++iOrb)  {
                     ...
                     const SxVector<PrecCoeffG> &pI = pPhiMu(is)(iMuRef);
                     for (int jOrb = 0; jOrb < nOrb; ++jOrb)  {
                        ...
                        const SxVector<PrecCoeffG> &pJ = pPhiMu(js)(jMuRef);
                        for (int ka = 0; ka < atPhi.getNAtoms (); ++ka)  {
            ...
            for (int i = 0, ib = 0; i < nOrb; i++, ib++)  {
               ...
               if (computeStatic)  {
                  ...
                  Hkstat(ik).blockRef(0, i-ib, nOrb, nb) = initBasis | Hstat_muI;
               }
               ...
               H.blockRef(0, i-ib, nOrb, nb) = initBasis | H_muI;

               // --- overlap matrix element
               if (computeL)
                  S.blockRef(0, i-ib, nOrb, nb) = initBasis | muI;
            ...
            if (computeL)   {
               ...
               */
               ...
               SxVector<SxComplex16>::Eigensystem Seig;
               Seig = S.eigensystem ();
            ...
            if (computeL)  {
               ...
               if (hamPW.rsProjector)  {
                  ...
                  for (int iMu = 0; iMu < nMu; ++iMu) 
                     muOrbs(iMu).resize(structure.getNAtoms(muRef(iMu).auxData.is));
                  for (int iOrb = 0; iOrb < nOrb; ++iOrb)
                     muOrbs(mu.getRefIdx(iOrb))(mu.psiOrb(iOrb).iAtom) = iOrb;
                  
                  SxAtomicStructure atMu, atNu;
                  SxVector<PrecCoeffG> muNu;
                  SxComplex16 ovlp;
                  // --- loop over orbital types
                  for (int iNu = 0; iNu < nMu; ++iNu)  {
                     atNu = SxSpeciesRef(muRef(iNu).auxData.is)
                            | structure;

                     for (int iMu = iNu; iMu < nMu; ++iMu)  {
                        atMu = SxSpeciesRef(muRef(iMu).auxData.is)
                               | structure;
               ...
               */
               ...
               SxVector<SxComplex16> SInv = S.inverse ();
               S.unref ();
               Lk(ik) = SInv.choleskyDecomposition ().adjoint ();

               // --- test L
               const SxVector<PrecCoeffG> &L = Lk(ik);
               double llMat = ((L.adjoint()^L) - SInv ).normSqr ();
               if (fabs(llMat) > 1e-10)  {
                  sxprintf ("| Warning: L^t*L - S^-1 = %e (should be 0)\n", llMat);
               }
            }
            ...
            const SxVector<PrecCoeffG> &L = Lk(ik);
            if (!H.isHermitian())  {
               sxprintf ("| TB: <mu|H|mu'> is not hermitian\n");
               cout << (H - H.adjoint ()).normSqr () << endl;
            }
            H = (L ^ H ^ L.adjoint());
            if (!H.isHermitian())  {
               sxprintf ("| TB: H is not hermitian\n");
               if ((H - H.adjoint ()).normSqr () < 1e-18 * double (H.getSize ()))  {
                  H += H.adjoint ();
                  H *= 0.5;
               }
            }
            SxSymEigensystem<SxComplex16> eig (H);
            ...
            // --- change to tight-binding basis
            {
               ...
               SxVecRef<PrecCoeffG> cAO = eig.vecs.colsRef (0, nFromTB);
               cAO.setBasis (initBasis);
               if (nPWStates == nFromTB)
                  waves(iSpin,ik) = Gk(ik) | cAO;
               else
                  waves(iSpin,ik).blockRef (0,0, Gk(ik).ng, nFromTB) = Gk(ik) | cAO;
            ...
            */
            ...
            if (hamPW.rsProjector)  {
               const SxArray<PsiG> &muRef = mu.muPhi(ik);
               int nMu = int(muRef.getSize ());
               SxArray<PsiG > eigVecRS(nMu);
               
               // --- resize eigVecRS
               for (int iMu = 0; iMu < nMu; ++iMu)  {
                  int is = muRef(iMu).auxData.is;
...
void SxHamSolver::tightBindingPAW (const SxSymbolTable *cmd, bool calc)
{
   ...
   if (updateXWaves)  {
      ...
      SxAtomicOrbitals xOrbitals(usedPhiPS, potPAW.getBasisLCAO ());
   ...
   for(int iMixSteps = 0; iMixSteps < maxStepsTB || iMixSteps == 0; iMixSteps++)
   {
      ...
      for (int ik = 0; ik < Gk.getNk (); ik++)
      {
         for(int iSpin = 0; iSpin < nSpin; iSpin++)
         {
            ...
            if (SxLoopMPI::myWork(ik))  {
               SxPAWBasis pawBasis (Gk(ik).getThis (), hamPAW.pBasis);
               SxVector<SxComplex16> Hnm(nOrb,nOrb), Snm(nOrb,nOrb);
               // --- construct Hamiltonian Hmn and Overlap Smn
               // H = H.adjoint, S = S.adjoint ()
               PsiRef HPsiJs, SPsiJs;
               ...
               for (int jState = 0, iB = 0; jState < nOrb; jState++, iB++)   {
                  ...
                  if (iB == 0)  {
                     PsiG psiJs (Gk(ik).ng, blockSize);
                     psiJs.auxData.ik = ik;
                     psiJs.auxData.iSpin = char(iSpin);
               ...
               SxVector<SxComplex16> Lnm = Snm.choleskyDecomposition();
               SxVector<SxComplex16> invLnm = Lnm.inverse();
               // transform Hnm
               Hnm = invLnm ^ Hnm ^ invLnm.adjoint();

               // solve standard eigenvalue problem
               SxSymEigensystem<SxComplex16> eig(Hnm);
               ...
               for (int iState = 0; iState < nFromTB; iState += blockSize)   {
                  ...
                  SxVecRef<PrecCoeffG> vecsBlock
                     = eig.vecs.colsRef (iState, blockSize);
               ...
               for (int iState = 0; iState < nFromTB; ++iState)
                  fermi.eps(iState,iSpin, ik) = eig.vals(iState);

               // --- Setup higher waves
               if (nStates > nOrb)   {
                  int nHigh = nStates-nFromTB;
                  PsiG highGk(Gk(ik).ng, nHigh);
                  PsiRef highStates, tbStates;
                  ...
                  highGk.auxData.ik = ik;
                  highGk.auxData.iSpin = char(iSpin);
                  ...
                  SxSymEigensystem<SxComplex16> eigHigh(highStates.overlap(hamPAW * highStates, Gk(ik).ng));
                  highStates.rotate (eigHigh.vecs);
                  for (int i = 0; i < nHigh; ++i)
                     fermi.eps (nFromTB + i, iSpin, ik) = eigHigh.vals (i);
   ...
   if (basisDecomp)  {
      ...
      for (int ik = 0; ik < Gk.getNk (); ik++)  {
         ...
         fprintf(HFilePtr, "\n");
         fprintf(SFilePtr, "\n");
         SxSymEigensystem<SxComplex16> Seig (Snm);
         for (int i = 0; i < nOrb; i++) {
            fprintf(SEigFilePtr, "%f\t",Seig.vals(i));
         ...
         int nEigenStates = (int)eig.vecs.getNCols ();
         eig.vecs = invLnm.adjoint() ^ eig.vecs;

         fprintf(filePtr,"%i kPoint \n\n",ik);
         for(int iState = 0; iState < nEigenStates; iState++)  {
            double sum = eig.vecs.colRef(iState).normSqr();
            for(int iOrb = 0; iOrb < nOrb; iOrb++)  {
               PsiG Psi = initBasis.getAOinG(ik,iOrb);
               int is = Psi.auxData.is;
               int iot = Psi.auxData.n;
               int l = Psi.auxData.l;
      ...
      for (ik=0; ik < nk; ik++)  {
         for (iSpin=0; iSpin < nSpin; iSpin++)  {
            for (i=0; i < nStates; i++)  {
               ...
               PsiRef psiG = waves(i,iSpin,ik);
               SxVector<PrecCoeffG> speed = psiG; // copy

               // TODO: ugly
               psiG.auxData.iSpin = char(iSpin);
               psiG.auxData.ik    = ik;
...
void SxHamSolver::allStateCG (const SxSymbolTable *cmd, bool calc)
{
   ...
   if (spinConstraint)  {
      cout << "Error - direct minimization cannot deal with spin constraints."
           << endl;
      SX_QUIT;
   }
   ...
   SxArray<SxArray<SxVector<SxComplex16> > > dEta, xEta;
   ...
   for (int it=0; it < maxSteps; it++)  {
      ...
      dNElec = 0.; fermidos = 0.;
      if (!dPsiPtr)
         dPsiPtr = SxPtr<SxPW>::create (nStates,nSpin,gkPtr, /* nComp */ 1,
                                        keepWavesOnDisk ? "." : "");
      for (ik = 0; ik < nk; ik++)  {
         SX_MPI_LEVEL("waves-k");
         if (!SxLoopMPI::myWork (ik)) continue;
         ...
         for (iSpin = 0; iSpin < nSpin; iSpin++)  {
            ...
            {
               PsiG psiHpsi = waves(iSpin, ik).overlap (dPsi, gk.ng);
               dPsi -= S * (waves(iSpin, ik) ^ psiHpsi);

               if (isMetal)  {
                  dEta(ik)(iSpin) = std::move (psiHpsi);
                  dEta(ik)(iSpin).diag () -= fermi.eps(iSpin,ik);
                  dEta(ik)(iSpin) *= -1.;
               }
            }
               ...
               for (ik = 0; ik < nk; ik++)  {
                  ...
                  for (iSpin = 0; iSpin < nSpin; iSpin++)  {
                     ...
                     if (isMetal)  {
                        ...
                        SxSymEigensystem<SxComplex16> eig (ham);
            ...
            for (ik = 0; ik < nk; ik++)  {
               ...
               for (iSpin = 0; iSpin < nSpin; iSpin++)  {
                  ...
                  for (i = 0; i < nStates; ++i)
                     ham(i,i) += fermi.eps(i,iSpin,ik);
                  SxSymEigensystem<SxComplex16> eig (ham);
                  fermiTrial.eps(iSpin, ik) = eig.vals;
                  if (keepOccFixed)  {
                     for (i = 0; i < nStates; ++i)  {
      ...
      if (xNorm > 10. * eNorm && lastRestart < it-2)  
      {
         ...
         for (ik = 0; ik < nk; ik++)  {
            ...
            for (iSpin = 0; iSpin < nSpin; iSpin++)  {
               ...
               for (i = 0; i < nStates; ++i)
                  ham(i,i) += fermi.eps(i,iSpin,ik);
               SxSymEigensystem<SxComplex16> eig (std::move (ham));
               fermi.eps(iSpin,ik) = eig.vals;
               waves(iSpin,ik).rotate (eig.vecs);
               xEta(ik)(iSpin).set (0.);
               X(iSpin,ik).set (0.);
      ...
      for (ik = 0; ik < nk; ik++)  {
         ...
         for (iSpin = 0; iSpin < nSpin; iSpin++)  {
            ...
            if (isMetal)  {
               ...
               fermi.eps(iSpin, ik) = std::move (eig.vals);
               waves(iSpin, ik).rotate (eig.vecs);
               X(iSpin, ik).rotate (eig.vecs);
...
void SxHamSolver::scfDiagonalization (const SxSymbolTable *cmd, bool calc)
{
   ...
   for (int it=0; it < maxSteps ; it++)  {
      ...
      if (spinConstraint) 
      {
         ...
         if (keepRho) pawHam.computeRhoTerms ();
      }
      // ############## khr: most of the computing time goes here
...
SxVector<PrecEps> 
SxHamSolver::subspaceDiagonalization (PsiRef *wavesPtrIN,
                                      int subDiagSize,
                                      int subDiagOvlp)
{
   SX_CHECK (wavesPtrIN);
   PsiRef &psiAll = *wavesPtrIN;
   // --- subspace diagonalization ---
   
   SX_CHECK(nStates == psiAll.getNCols (), nStates, psiAll.getNCols ());
   
   SxVector<PrecEps> epsAll(nStates);
   int from = 0;
   ...
   int ng2 = (int)dPsiAll.getNRows ();
   
   int nBlock, nextFrom;
   SxVector<PrecCoeffG> h;
   double eps;
   for (int iSubDiag = 0; /* empty */; iSubDiag++)  {
      ...
      PsiRef psiGI = psiAll.colsRef (from, nBlock);
      PsiRef dPsiGI;
      { 
         SX_CLOCK (Timer::SubspaceMatrix);
         dPsiGI = dPsiAll.colsRef (from, nBlock);
      ...
      // --- diagonalize matrix
      SX_START_TIMER (Timer::SubspaceDiag);
      SxSymEigensystem<SxComplex16> eig(std::move (h));
      SX_STOP_TIMER (Timer::SubspaceDiag);

      // copy eigenvalues
      epsAll(SxIdx(from,to-1)) <<= eig.vals;
      // rotate eigenfunctions
      psiGI.rotate (eig.vecs);
      ...
      // --- update dPsiAll for overlapping states
      {
         ...
         SxVecRef<PrecCoeffG> eigVecOvlp
            = eig.vecs.colsRef (nBlock - overlap, overlap);
         dPsiAll.colsRef (nextFrom, overlap) <<= dPsiGI ^ eigVecOvlp;
      }
...
void SxHamSolver::propagateWaves (int ik, int jk)
{
   ...
   SxArray<PsiG> oldWavesNewBasis(nSpin);
   ...
   for (int iSpin = 0; iSpin < nSpin; ++iSpin)  {
      ...
      oldWavesNewBasis(iSpin).auxData.ik = jk;
   }
...
void SxHamSolver::diffEqtn (const SxSymbolTable *cmd, bool calc)
{
   ...
   // --- print input parameter
   cout << SX_SEPARATOR;
   cout << "| Differential equation solver\n";
   cout << SX_SEPARATOR;
   ...
   for (itDiag=0; /* empty */; itDiag++)  {
      ...
      tr = dot(dPsi, K);
      if (itDiag > 0)  gamma = (tr - dot(dPsiOld, K)) / trOld;
      dPsiOld = dPsi; // copy
      trOld = tr;
      ...
      M(0,0) = eps;
      M(0,1) = dot(X, hPsi);
      M(1,0) = M(0,1).conj();  M(1,1) = dot(X, hX);
...
void SxHamSolver::hSqrCG (const SxSymbolTable *cmd, bool calc)
{
   ...
   int nConvRes;
   int maxItDiag;
   PsiG dPsi, hPsi, hPsi2, hPsiSqr, dPsi0, dPsiOld, K, X, Xold, hX;
   Real8 eps, epsSqr, gamma = 0., theta, epsPrev=0., eTrial;
   PrecCoeffG scp, tr, trOld = 0.;
   SxVector<PrecCoeffG> M(2,2), U(2,2);
         ...
         for (it = 0; it < maxSteps; it++)  {
            ...
            for (i=0; i < nStates; i++)  {
               ...
               for (itDiag=0; /* empty */; itDiag++)  {
                  ...
                  X.normalize ();
                  // TODO: ugly
                  X.auxData.iSpin = char(iSpin);
                  X.auxData.ik    = ik;
                  // --- get M matrix (in ref1: H matrix)
                  hX = (*hamPtr | X) - eTrial * X;
                  M(0,0) = eps * eps - 2. * eps * eTrial + eTrial * eTrial;
                  M(0,1) = dot(hX, hPsi - eTrial * waves(i, iSpin, ik));
                  M(1,0) = M(0,1).conj();  M(1,1) = dot(hX, hX);
            ...
            if (subDiagSize > 0)
            {
               ...
               SxVector<PrecEps>::Iterator epsDiagIt, epsFermiIt;
...
/*            if ( nUnconvDiag == 0 && nConvIt <= nSloppy)  {
            ...
            lastEnergy = eBand;
            hamPtr->printEnergies();
            cout << "eps; " << dot (waves(nStates-1,iSpin,ik), hPsi).re << endl;

            if ( !(it % printSteps) )  {
               fermi.epsSortList(iSpin, ik) = SxVector<int> (fermi.eps(iSpin, ik).getSortIdx ());
               cout << SX_SEPARATOR;
...
void SxHamSolver::bandStructureCG (const SxSymbolTable *cmd, bool calc)
{
   ...
   int nConvRes;
   int maxItDiag;
   PsiG hPsi, dPsi, dPsiOld, K, X, Xold, hX;
   PsiG psiGI, dPsiGI;
   SxVector<PrecCoeffG> h;
   Real8 eps, gamma = 0., theta, epsPrev=0.;
   PrecCoeffG scp, tr, trOld = 0.;
                           ...
                           bool printOnly)
{
   ...
   fermi.eps.synMPI ();
   {
      ...
      SxLoopMPI::sum ({&nConvIt, &nStepsIt, &nCosAcc, &nStepsCos,
                       &nConvAbs, &nStepsAbs, &nConvRel, &nStepsRel,
                       &nConvOcc, &nStepsOcc});
   }
#endif
...
SxVector<PrecCoeffG>
SxHamSolver::solveRmmDiis (const SxVecRef<PrecCoeffG> &h2In,
                           const SxVecRef<PrecCoeffG> &hsshIn,
                           const SxVecRef<PrecCoeffG> &s2In,
                           const SxVecRef<PrecCoeffG> &s,
                           double *epsPtr,
                           int n)
{
   ...
   SxVector<PrecCoeffG> Rij, h2, hssh, s2;
   SxVector<PrecCoeffG> L = SxVector<PrecCoeffG>(s).choleskyDecomposition ().inverse (),
                        U = L.adjoint ();
   h2   = L ^ h2In   ^ U;
   hssh = L ^ hsshIn ^ U;
   s2   = L ^ s2In   ^ U;

   int nPhi = (int)s2.getNCols ();
   SxVector<PrecCoeffG> beta;
   ...
   for (int i = 0; i < n; ++i)  {
      ...
      SxVecRef<PrecCoeffG> alpha;
      for (int it = 0; it < 30; ++it)  {
         Rij = h2 - eps * hssh + (eps*eps)*s2;
         {
            SX_CLOCK (DiagRmm);
            eig.compute (std::move(Rij), All);
         }
         epsInOld = epsIn;
         epsIn = eps;
      ...
      SX_CLOCK (Reduce);
      SxVecRef<PrecCoeffG> T = eig.vecs(SxIdx(nPhi, nPhi*nPhi-1));
      T.reshape (nPhi, nPhi-1);
      SxVector<PrecCoeffG> Ta = T.adjoint ();
      h2   = Ta ^ h2 ^ T;
      hssh = Ta ^ hssh ^ T;
      s2   = Ta ^ s2 ^ T;
      U = U ^ T;
                           ...
                           int maxItDiag, bool verbose, bool sloppyConv)
{
   ...
   hPsi = *hamPtr | psiI;
   eps  = dot (gk | psiI, hPsi).re;
   dPsi = hPsi - eps * (sList = S * psiI);
   fermi.eps(i,iSpin,ik) = eps;
   SxAuxData auxData = dPsi.auxData;
   ...
   for (int itDiag=0; /* empty */; itDiag++)  {
      ...
      SxVector<PrecCoeffG> alpha;
      {
         SxVector<PrecCoeffG> h2, hssh, s2, s;
         s = sList.overlap(gk | phiList);
         if (symEigenvalues (s).minval () < 1e-16)  {
            if (verbose)
               sxprintf ("numerical limit reached (%d of %d)\n", 
                       itDiag+1, maxItDiag);
            nCosAcc++;
                                ...
                                int maxItDiag, bool verbose, bool sloppyConv)
{
   ...
   SxVector<double> epsPrev;
   converged.set (false);

   PsiRef psiI = waves.getBlock(i, blockSize, iSpin,ik);
   SxVecRef<double> eps  = fermi.eps(iSpin,ik)(SxIdx(i,i+blockSize-1));
   if (verbose)  {
   ...
   PsiG phiList, hList, sList;

   // --- compute gradient: |dPsi> = (H-e) |psi>
   hPsi = *hamPtr | psiI;
   PsiRef sPsi = S * psiI;
   SxAuxData auxData = hPsi.auxData;
   {
   ...
   for (int itDiag=0; /* empty */; itDiag++)  {
      ...
      {
         SX_CLOCK (Timer::SearchDir);
         PsiG K (gk.ng, blockSize);
         K.auxData = dPsi.auxData;
         for (int iBlock = 0; iBlock < blockSize; ++iBlock)  {
            K.colRef (iBlock) <<= 
      ...
      SxVector<PrecCoeffG> alpha;
      {
         SxVector<PrecCoeffG> h2, hssh, s2, s;
         s = sList.overlap(gk | phiList);
         SxVector<double> sev = symEigenvalues (s);
         ...
         if (sev.minval () < cut)  {
            ...
            phiList = phiList ^ U;
            phiList.auxData = psiI.auxData;
            hList = hList ^ U;
            sList = sList ^ U;
            hList.auxData = auxData;
            sList.auxData = auxData;
            s = U.adjoint () ^ s ^ U;
         }
      ...
      if (nConvBlock > 0)  {
         ...
         if (nConvBottom > 0 || top < blockSize)  {
            ...
            // --- reformat variables
            dPsi = PsiG (dPsi.colsRef (nConvBottom, nRest));
            X    = PsiG (   X.colsRef (nConvBottom, nRest));
            ...
            resNorm0 = resNorm0(SxIdx(nConvBottom, top - 1));
            resNorm = resNorm(SxIdx(nConvBottom, top - 1));
                                ...
                                bool /* sloppyConv */)
{
   ...
   for (int itDiag = 0; itDiag < maxItDiag; ++itDiag)  {
      ...
      SxVector<PrecCoeffG> Lambda = (gk | psiI).overlap (hPsi);
      SxVector<PrecCoeffG> Residuum = hPsi - (sPsi ^ Lambda);
      SxVector<PrecCoeffG> K = hamPtr->preconditioner (psiI);
      //K.set (1.);
      SxVector<PrecCoeffG> KR, KS, KKR;
      KKR.reformat (gk.ng, blockSize);
      KKR.auxData = hPsi.auxData;
      KR = Residuum;
      KS = sPsi;
      ...
      {
         KKR = waveBasis | (KKR);
         SxVector<SxComplex16> KSKR = KS.overlap (KR);
         // + KR.overlap (KS) ; (* -1)
         SX_LOOP(j)  {
            KSKR(j).re *= -2.;
            KSKR(j).im = 0.;
   ...
   SxVector<PrecCoeffG> H = (gk | psiI).overlap (hPsi);
   SxSymEigensystem<PrecCoeffG> eig (H);
         ...
         if (SxLoopMPI::myWork(ik))    // LoopMPI
         {
            ...
            for (int i=0; i < nStates; i++)  {
               ...
               for (int itDiag=0; /* empty */; itDiag++)  {
                  ...
                  tr = dot(dPsi, gk | K);
                  if (itDiag > 0)
                     gamma = (tr - dot(dPsiOld, (gk | K))) / trOld;
                  //             if (itDiag > 0)  gamma = tr / trOld;
                  dPsiOld = dPsi;
                  trOld = tr;
                                      ...
                                      bool verbose)
{
   ...
   int nStepsEps=0;
   PsiG hPsi, X, Xold;
   SxVector<PrecEps> deltaEps, eps1st(defBlockSize);
...
#  endif
   ...
   for (int ik=0; ik < nk; ik++)
   {
      ...
      for (int iSpin=0; iSpin < nSpin; iSpin++)
      {
         ...
         {
            SX_ALLOC_CACHE;
            SxSymMatrix<PrecCoeffG> subspaceMatrix(nStates);
            ...
            for (int i=0; i < nStates; /* done inside loop */)  {
               ...
               for (int itDiag=0; /* empty */; itDiag++, iter+=1)  {
                  ...
                  if (itDiag % 5 == 0)  {
                     ...
                     // --- compute gradient
                     psiI.auxData.iSpin = char(iSpin);
                     psiI.auxData.ik = ik;
                     hPsi = *hamPtr | psiI;
                     if (itDiag == 0)  {
                        //eps1st = SxVector<PrecEps>(blockSize);
                        double eps;
                        for (int j = 0; j < blockSize; ++j)  {
                  ...
                  PsiRef Spsi = S * psiI;
                  {
                     ...
                     for (int j = 0; j < blockSize; ++j)  {
                        ...
                        //tr += dPsi.normSqr ();
                        double tr_ = 0.;
                        SxVector<double> precond = hamPtr->preconditioner (gk | psiI.colRef(j));
                        ssize_t ng = gk.ng;
#ifdef USE_OPENMP
#pragma omp parallel for reduction(+:tr_)
...

                  {
                     ...
                     zeroX.resize (0);
                     SxSymEigensystem<PrecCoeffG> eig(S.getMatrix (X, X));
                     if (eig.vals(0) > 1e-12 * fabs(eig.vals(blockSize-1)))  {
                        ...
                        // --- orthonormalize (track zero search directions)
                        for (int j = 0; j < blockSize; ++j)  {
                           SxVecRef<SxComplex16> Xj = X.colRef(j);
                           // --- Gram-Schmidt orthogonalization
                           if (j > 0)  {
                              S.setOrthogonal (&Xj, X.colsRef (0, j));
                                                   /* cols 0..(j-1) */
                           }
                  ...
                  {
                     PsiG hX = *hamPtr | X;
                     SxSymMatrix<PrecCoeffG> ham(2 * blockSize);
                     if (blockSize > 1) {
                        SX_CLOCK(Timer::SubspaceMatrix);
                        SxVector<PrecCoeffG> psiHpsi = psiI.overlap (hPsi, gk.ng);
                        SxVector<PrecCoeffG> psiHX   = psiI.overlap (hX, gk.ng);
                        SxVector<PrecCoeffG> XhX     = X.overlap (hX, gk.ng);
                     ...
                     SxSymEigensystem<PrecCoeffG> eig;
                     {
                        SX_CLOCK (Timer::SubspaceDiag);
                        eig.compute (std::move (ham), All);
                     }
                     ...
                     if (blockSize > 1)  {
                        ...
                        SxVector<PrecCoeffG> eigVecI, eigVecX;
                        eigVecI = eig.vecs.blockRef (0,        0,blockSize,blockSize);
                        eigVecX = eig.vecs.blockRef (blockSize,0,blockSize,blockSize);
                        /*
                  ...
                  if (j > 0)  {
                     ...
                     PsiRef psiLow, hPsiConv;
                     psiLow = (gk | waves.getBlock(0, i+j, iSpin, ik));
                     hPsiConv = hPsi.colsRef (0, j);
                     SxVector<PrecCoeffG> psiConvHPsiLow = hPsiConv.overlap (psiLow);
                     int iL, iR;
                     for (iR = 0; iR < i+j; ++iR)
                        for (iL = 0; iL < j; ++iL)
                           if (iR >= i + iL)
            ...
            }
            {
               ...
               {
                  SX_CLOCK(Timer::SubspaceDiag);
                  eig.compute (std::move(subspaceMatrix), All);
               }
#ifdef SX_LOOP_MPI
...
void SxHamSolver::directDiagonalization (const SxSymbolTable *cmd, bool calc)
{
   ...
   SxSymMatrix<PrecCoeffG>  HComplete;
   ...
   for (it = 0; it < maxSteps; it++)  {
      ...
      for (ik = 0; ik < nk; ik++)  {
         ...
         for (iSpin = 0; iSpin < nSpin; iSpin++)  {
            ...
            SX_START_TIMER(Timer::SubspaceDiag);
            SxSymEigensystem<PrecCoeffG> eig (HComplete);
            SX_STOP_TIMER(Timer::SubspaceDiag);

            // --- get energies
            n = (int)fermi.eps(iSpin,ik).getSize ();
            fermi.eps(iSpin,ik) <<= eig.vals( SxIdx(0,n-1) );
            SX_CHECK (waves(iSpin,ik).getNCols () == n,
                      waves(iSpin,ik).getNCols (), n);
...
void SxHamSolver::linGradTest (const SxSymbolTable *cmd, bool calc)
{
   ...
   for (double lambda = lambdaMin; lambda <= lambdaMax; lambda += dLambda)  {
      ...
      eLin = 2. * dot(g , waves(i,iSpin,ik)).re;
...
#endif
   wavesPtr->writeWavesFile(filename, fermi, structure);
}
