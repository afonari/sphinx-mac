...
class SX_EXPORT_DFT SxMuPW : public SxPWSet
{
   ...
   public:
      ...
      virtual SxVecRef<PrecCoeffG> operator() (int i, int iSpin, int ik);
      ...
      /** \brief computes \f$ \langle \bf{G+k} | \mu_i\rangle\f$
          ...
          \todo Caching for mu(i,iSpin,ik) -> <Psi|H|Psi> */
      virtual const SxVecRef<PrecCoeffG> operator() (int i, int iSpin, int ik) const
      {
         return const_cast<SxMuPW*>(this)->operator ()(i, iSpin, ik);
      }
...
// protected:
      ...
      SxArray<SxArray<PsiG> >  muPhi; //:ik,:iOrb
   ...
   private:
      ...
      SxArray<SxList<SxVecRef<PrecCoeffG> > >  muPhiList;
