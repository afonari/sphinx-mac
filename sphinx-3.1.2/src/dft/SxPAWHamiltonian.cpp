...
void SxPAWHamiltonian::checkDistances () const
{
   SX_CHECK (potPtr);
   SX_CHECK (structure.getNAtoms () > 0);
   // --- for neighbor search
   SxGrid grid (structure, 10);
   SxNeighbors nn;
   int neighborParams = SxNeighbors::StoreAbs
                      | SxNeighbors::StoreDistSqr
                      | SxNeighbors::IncludeZeroDistance;
   bool crash = false;
   SX_LOOP2(is, ia)  {
      // compute neighbors within rcut
      nn.compute (grid, structure, structure.getAtom(is,ia),
                  0.5 * potPtr->rc(is), neighborParams);
      if (nn.getSize () > 1)  {
         const SxArray<SxString> &chemName = potPtr->chemName;
         if (!crash)
            cout << endl << SX_SEPARATOR
                 << "| Critical interatomic distances detected!" << endl
                 << "| Atoms are too close. Please check your input structure." << endl
                 << SX_SEPARATOR;
         crash = true;
         const Coord &pos = structure.constRef(is, ia);
         SX_LOOP(in)  {
            int ja, js = structure.getISpecies (nn.idx (in), &ja);
            if (js == is && ja == ia) continue;
            if (nn.idx (in) < structure.getIAtom (is, ia)
                && nn.distSqr(in) < sqr(0.5 * potPtr->rc(js))) continue;
            cout << chemName(is) << (ia + 1) << " @["
                 << pos(0) << ", " << pos(1) << ", " << pos(2) << "] <-> ";
            if ((nn.absPositions(in) - structure.constRef(js, ja)).normSqr () > 1e-12)
               cout << "replica of ";
            const Coord &pos2 = nn.absPositions(in);
            cout << chemName(js) << (ja + 1) << " @["
                 << pos2(0) << ", " << pos2(1) << ", " << pos2(2) << "]: d="
                 << sqrt(nn.distSqr(in)) << " bohr" << endl;
         }
      }
   }
   if (crash)  { SX_QUIT; }
...
void SxPAWHamiltonian::compute (const SxFermi &, bool, bool)
{
...
void SxPAWHamiltonian::writePotentials () const
{
   if (hubbardU) hubbardU->writeOccupations ("hubbardOcc.dat");
}
...
void SxPAWHamiltonian::read (const SxSymbolTable *table)
{
   ...
   if (hamGroup->containsGroup("vExt"))  {
      ...
      const SxVecRef<double> &vExtSym = rPtr->symmetrize(vExt);
...
{
   SX_CHECK(hubbardU);
   SxArray<SxPtr<SxHubbardXO> > &moSite = hubbardU->moSite;
   pawRho.blockAO.resize (moSite.getSize ());
   for (int iMoSite = 0; iMoSite < moSite.getSize (); iMoSite++)  {
      // setup projectors
      moSite(iMoSite)->setupProjGk (gk, pBasis);
      ...
      SX_LOOP2(iSpin,iSite)  {
         int nAO = moSite(iMoSite)->getNOrbSite (iSite);
         int nAOHam = moSite(iMoSite)->getNOrbHam (iSite);
         blockRho(iSpin,iSite).reformat (nAO, nAO);
         blockHam(iSpin,iSite).reformat (nAOHam, nAOHam);
         // ... and set to zero
         blockRho(iSpin,iSite).set (0.);
         blockHam(iSpin,iSite).set (0.);
         // ... and set spin index
         blockRho(iSpin,iSite).auxData.iSpin = char(iSpin);
         blockHam(iSpin,iSite).auxData.iSpin = char(iSpin);
      }
   }
}
   ...
   for (int ik = 0; ik < waves.getNk (); ++ik) {
      for (int iSpin = 0; iSpin < nSpin; ++iSpin) {
         ...
         {  // LoopMPI
            ...
            SxVector<SxComplex16> p = pBasis | waves(iSpin,ik);
            int offset = 0;
            for (int is = 0; is < structure.getNSpecies (); ++is)  {
               int nProjLocal = pot.getNProj (is);

               // --- set up i^l
               SxVector<SxComplex16> il(nProjLocal); // i^(l)  (:ip)
               ...
               for (int ia = 0; ia < structure.getNAtoms(is); ++ia, offset+=nProjLocal)  {
                  SxVector<double> &D = res(iSpin, is, ia);
...
void SxPAWHamiltonian::computeRhoTerms ()
{
   ...
   SxMeshG rhoPSG = gBasis | ((nSpin == 1) ? rhoPSR(0)
                                        : rhoPSR(0) + rhoPSR(1));
   ...
   SxMeshG nHatG(0., gBasis), nHatpG(0., gBasis);

   // |G|^l * Ylm(G)
   SX_START_TIMER (TimeYlmGl);
   int lMaxRho = pawpot.lMaxRho.maxval ();
   SxVector<double> YlmGl(gBasis.ng, sqr(lMaxRho + 1));
   YlmGl.setBasis (gBasis);
   for (int lm = 0, l = 0; l <= lMaxRho; ++l)  {
      SxVector<double> gl = pow (gBasis.g2, 0.5 * l);
   ...
   SxMeshG vRhoPSG = FOUR_PI / gBasis.g2(noZero) * rhoPSG(noZero);
   ...
   for (int is = 0; is < structure.getNSpecies (); ++is)
   {
      ...
      SxVector<double> gaussHard = exp ( -(0.25 * rc2 ) * gBasis.g2),
      ...
      // ---
      /*
      SxVector<double> VncAE
         = -(SQRT_4PI * pawpot.nuclearCharge(is))/rad.radFunc(is)
         + radial.getHartreePotential (pawpot.rhoCoreAE(is));
      SxVector<double> VncPS;
      {
         SxVector<double> dRhoCore = pawpot.rhoCorePS(is).getCopy ();
      ...
      */
      ...
      SxVector<double> omegaij(pawPot.getNProj(is),pawPot.getNProj(is));
      omegaij.set (0.);
      SX_LOOP2(ipt, jpt)  {
      ...
      for (int ia = 0; ia < structure.getNAtoms (is); ++ia)
      {
         ...
         if (SxLoopMPI::myWork(iTlAtom)) // SxParallelHierarchy
         {
            ...
            Q.resize(rhoPAWcorr.meshData.getNCols ());
            for (int l = 0; l <= rhoPAWcorr.lmax; ++l)  {
               SxVector<double> rl = pow(rad.radFunc(is),double(l+3));
               for (int m = -l; m <= l; ++m)  {
            ...
            dEdQrl(is)(ia).resize(rhoPAWcorr.meshData.getNCols ());
            if (hContrib & CalcHartreeRad)  {
               for (int l = 0; l <= rhoPAWcorr.lmax; ++l)  {
                  for (int m = -l; m <= l; ++m)  {
            ...
            if (hContrib & CalcNHatG)  {
               SX_CLOCK (NHatG);
               SxVecRef<PrecCoeffG> T = gBasis.getPhaseFactors (is,ia);
               int nlm = sqr(lMax+1);
               // --- compute prefactors
...
//   sxprintf ("eVnc = %.12f\n", eVnc);
   ...
   if (SxLoopMPI::nr () > 1)
   {
      ...
      // synchronize Hubbard occupations
      if (hubbardU)
         hubbardU->syncMPI ();
   }
#endif
   ...
   SxMeshG rhoHartreeHard = rhoPSG + nHatG,
   ...
   SxMeshG vBarTotal(0., gBasis);
   if (hContrib & CalcBar)  {
      for (int is = 0; is < structure.getNSpecies (); ++is)
         vBarTotal += vBarG(is) * gBasis.getStructureFactor (is);
   ...
   SxMeshR vPSR = rBasis | (vHartreeHard + vBarTotal);
...
#endif
      SxMeshR rhoHartreeR = rBasis | rhoHartreeSoft;
      dipoleCorr->update (rBasis | rhoPSG, rhoHartreeR);
      dipoleCorr->correctPotential (&vDipole);
      // correction to potential
      vPSR += vDipole;

      // correction to energy
      eDipole =  0.5 * tr((rBasis | rhoHartreeSoft) * vDipole);
      eDoubleCounting -= eDipole;
      eDoubleCounting += tr((rBasis | rhoCoreG) * vDipole);
   ...
   if (hContrib & CalcNHatGSoft)
   {
      ...
      SxVector<double> gaussSoft = exp ( -(0.25 * rcp2) * gBasis.g2);
      for (int is = 0; is < structure.getNSpecies (); ++is) {
         int lMax = pawpot.lMaxRho (is);
         int nlm = sqr(lMax+1);
         for (int ia = 0; ia < structure.getNAtoms (is); ++ia) {
...
#pragma omp for
      ...
      for (int ia = 0; ia < structure.getNAtoms (is); ++ia) {
         if (SxLoopMPI::myWork(structure.getIAtom (is,ia))) { // SxParallelHierarchy
            ...
            for (int lm = 0, l = 0; l <= lMax; ++l)  {
               SxVector<double> rl = pow(rad.radFunc(is), double(l));
            ...
            SxVector<double> vR0ij =   pawpot.getVMatEl (vR0, pawpot.phiAE(is))
                                     - pawpot.getVMatEl (vR0, pawpot.phiPS(is));
            for (int iSpin = 0; iSpin < nSpin; ++iSpin)   {
               SxVector<double> &VijR = Vij(iSpin,is,ia);
               // add VR0 term
               VijR += vR0ij;
               // --- add kinetic Energy
               if (hContrib & CalcKinRad)  {
   ...
   if (vExt.getSize () > 0)  {
      // --- external potential
      eExt = tr ((rBasis | rhoHartreeSoft) * vExt);
      eDoubleCounting += tr((rBasis | rhoCoreG) * vExt);
      vPSR += vExt;
   } else {
      eExt = 0.;
   }
      ...
      if (pawpot.kjXcShape.getSize () > 0)  {
         ...
         for (int is = 0; is < structure.getNSpecies (); ++is)  {
            if (pawpot.kjXcShape(is).getSize () > 0)  {
               ...
               SxMeshG kjxcG = gBasis | pawpot.kjXcShape(is);
               for (int ia = 0; ia < structure.getNAtoms(is); ++ia)  {
   ...
   if (applyVDWCorrection && vdwCorrection.needEffVolumes ()) {
      ...
      if (refRho.getSize () != structure.getNSpecies ())  {
         ...
         SxRadialBasis radR(0.,rMax,nrRad,true,SxRadialBasis::Linear);
         double gMax = sqrt(G.g2(G.ng-1))*1.05;
         int ngRad = 1000; // ad hoc
         SxRadialBasis radG(0., gMax, ngRad,false, SxRadialBasis::Linear);
         SX_LOOP(is)  {
            refRho(is) = potPtr->getAtomRhoG(rPtr->getGBasis (), (int)is);
            SxVector<double> filter(potPtr->rhoInit(is).getSize ());
            ...
            SxVector<double> rhoAtomFree = radR | (potPtr->rhoInit(is) * filter);
            writePlot ("rhoAtomFree.dat", radR.getRadFunc (), rhoAtomFree);
            SxVector<double> rhoAtomR3 = rhoAtomFree * radR.getRadFunc ().cub ();
            writePlot ("rhoAtomFreeR3.dat", radR.getRadFunc (), rhoAtomR3);
      ...
      SxMeshG allAtomsG(0., G);
      SX_LOOP(is) allAtomsG += refRho(is) * G.getStructureFactor (is);
      // --- get total density
      SxVecRef<double> rhoTot;
      if (pawRho.getNSpin () == 1)
         rhoTot = pawRho.pwRho(0);
      else
         rhoTot = pawRho.pwRho(0) + pawRho.pwRho(1);
      // get rho / refRho in G space
      SxMeshG rhoByRef = G | (rhoTot / sqrt((*rPtr | allAtomsG).real ().sqr () + 1e-10));
      SxMeshG rhoByRefR = rhoTot / sqrt((*rPtr | allAtomsG).absSqr () + 1e-10);
      effectiveVolumes.resize (structure.nTlAtoms);
      SX_LOOP(is)  {
         SxMeshG rhoRefR3 = rhoByRef * refRhoR3(is);
         SX_LOOP(ia)  {
         ...
         for (int iSpin = 0; iSpin < nSpin; ++iSpin)  {
            ...
            if (SxLoopMPI::myWork(ik)) {
               ...
               for (int iState = 0; iState < waves.getNStates(ik); ++iState)  {
                  ...
                  PsiRef psiG = gk | waves(iState,iSpin,ik);
                  eKin += 0.5 * weight * focc * gk.laplacian (psiG);
               }
            } // LoopMPI
         }
      }
         ...
         if(SxLoopMPI::myWork(structure.getIAtom (is, ia)))
         {
            ...
            for (int js=0; js < structure.getNSpecies (); js++)  { // --- species j
               ...
               for (int ja = 0; ja < neighbors.relPositions.getNAtoms(js); ++ja)
               {
                  ...
                  SxVecRef<double> delta = gaussIJ.compute (lmaxI, lmaxJ, cg);
...
PsiG SxPAWHamiltonian::apply (const PsiRef &psi) const
{
   SX_CLOCK (Timer::PAW_H_psi);
   int iSpin = psi.auxData.iSpin;
   SX_CHECK (psi.getNCols () > 0, psi.getNCols ());
   ...
   for (int iState = 0; iState < nStates; ++iState)  {
      PsiRef psiG = gk | psi.colRef(iState);
      if (hContrib & CalcKinPS)  {
...
SxVector<double> SxPAWHamiltonian::preconditioner (const PsiRef &dPsi) const
{
   ...
   double kin = Gk->laplacian (*Gk | dPsi);
   /* --- beautiful and slow
   SxVector<PrecCoeffG::TReal> x, x2, x3, x4, n, K;
   x  = Gk->g2 / kin;
   x2 = x.sqr();
   x3 = x.cub();
   x4 = x2.sqr();
   ...
   */
   ...
   double kinInv = 1. / kin;
   SxVector<double> K(Gk->ng); // should not have a basis!
#ifdef USE_OPENMP
...
SxPAWHamiltonian::getHnm (const PsiRef &waveBlock) const
{
   ssize_t nStates = waveBlock.getNCols ();
   int bSize = 64;
   if (waveBlock.getSize () < 1000000)  {
      PsiG HPsi = apply(waveBlock);
      return waveBlock.overlap (HPsi, HPsi.getNRows ());
   }
   SxVector<SxComplex16> Hnm(nStates, nStates);
   ssize_t ngPAW = waveBlock.getNRows ();
   for (ssize_t i = 0; i < nStates; i += bSize)  {
      ssize_t ni = i + bSize < nStates ? bSize : (nStates - i);
      SxVecRef<SxComplex16> blockI
         = waveBlock.getRef<Compact> (i * ngPAW, ngPAW, ni);
      blockI.auxData = waveBlock.auxData;
      const PsiG &HPsi = apply(blockI);
      Hnm.getRef (nStates * ni, nStates * i)
         <<=  waveBlock.overlap (HPsi, HPsi.getNRows ());
   }
   return Hnm;
}
...
void SxPAWHamiltonian::computeForces (const SxPWSet &waves, SxFermi &fermi)
{
   ...
   SxIdx noZero(1,gBasis.ng - 1);
   const SxVector<double> &G  = gBasis.gVec;
   SxVector<PrecCoeffG> castG  = G; // different type!
   const SxVector<double> &G2 = gBasis.g2;
   ...
   if (dipoleCorr && (hContrib & CalcDipoleC))  {
      ...
      SxMeshR vDipole(0., *rPtr);
      dipoleCorr->correctPotential (&vDipole);
      vHartreeSoft += gBasis | vDipole;
   }

   if (vExt.getSize () > 0)  {
      // add external field to potential
      vHartreeSoft += gBasis | vExt;
   }
   ...
   SX_LOOP2(is,ia) {
      SxMeshG rhoHatRPG = computeRhoHatG ((int)is,(int)ia,YlmGl, rcp2);
      SxVecRef<PrecCoeffG> T = gBasis.getPhaseFactors ((int)is,(int)ia);
      // --- Ref.1 Eq (50) 1st term,
      {  SX_CLOCK (Timer::F11);
         f11.ref(is,ia) -= ((I * vHartreeSoft.conj() * rhoHatRPG) ^ castG)
                           .real ().toVector3 ();
      }
      // --- Ref.1 Eq (50) 2nd term,
      {  SX_CLOCK (Timer::F12);
         SxMeshG rhoHatRG(gBasis), v(gBasis);
         ...
         f12.ref(is,ia) -= ((-I * rhoPSG * v.conj()) ^ castG).real ().toVector3 ();
      }
      // Ref.1 Eq (50) 3rd term, contribution due to pseudopot and core density
      {  SX_CLOCK (Timer::F13);
         f13.ref(is,ia) += 1./nSpin * ((rhoCorePSG(is) * T * vPSG.conj()) ^ castG)
                                      .imag ().toVector3 ();
      }
   }
   ...
   if (hubbardU && hubbardU->moSite.getSize () > 0)  {
      ...
      SX_LOOP2(iMO, iSpin)  {
         ...
         SX_LOOP2(iDir,iSite)  {
            int nAO = hubbardU->moSite(iMO)->getNOrbSite (iSite);
   ...
   for (int ik = 0; ik < waves.getNk (); ++ik)  {
      ...
      for (int iSpin = 0; iSpin < waves.getNSpin (); ++iSpin) {
         SxVecRef<SxComplex16> P = *pBasis | waves(iSpin,ik);
         SxArray<SxVector<SxComplex16> > gradP
         = pBasis->projectors->gradProject (gk(ik) | waves(iSpin,ik));
         ...
         if (hubbardU)  {
            for (int iMO = 0; iMO < hubbardU->moSite.getSize (); ++iMO)  {
               const SxHubbardXO &mo = *hubbardU->moSite(iMO);
               // get <p|psi>
               SxVector<SxComplex16> PMo = (*mo.aoProj) | waves(iSpin, ik);
               // get d/dR <p|psi>
               SxArray<SxVector<SxComplex16> > gradPMo
...
SxMeshG
SxPAWHamiltonian::computeRhoHatG (int is, int ia, 
                                  const SxVecRef<double> &YlmGl,
                                  double rc2) const
{  
   double rNorm = FOUR_PI / sqrt(structure.cell.volume);
   SX_CHECK_NUM (rNorm);
   ...
   SxMeshG nHatG (gBasis);
   nHatG.set(0.0);

   int lMax = pawpot.lMaxRho (is);
   SxVector<double> gauss = exp ( -(0.25 * rc2) * gBasis.g2);
   ...
   for (int l = 0; l <= lMax; l++, dFac *= 2 * l + 1)   {
      for (int m = -l; m <= l; m++)   {
         ...
         SxMeshG grl = (YlmGl.colRef(lm) * gauss) * T;
                                                          ...
                                                          SxArray<SxVector<SxComplex16> > &gradP)
{
   ...
   for (int iDir = 0; iDir < 3; iDir++)   {
      ...
      for (int is = 0; is < structure.getNSpecies (); ++is)  {
         ...
         // --- set up i^l 
         SxVector<SxComplex16> il(nProjLocal); // i^(-l)  (:ipl)
         for (int ipt = 0, ipl = 0; ipt < pot.lPhi(is).getSize (); ++ipt)  {
            int l = pot.lPhi(is)(ipt);
            SxComplex16 c = (l & 1) ? I : SxComplex16(1.);
            if (!(l & 2)) c=-c;
         ...
         for (int ia = 0; ia < structure.getNAtoms(is); ++ia, offset+=nProjLocal)  {
            SxVector<double> &D = res(iDir)(0,is,ia);
            for (int ipl = 0; ipl < nProjLocal; ++ipl)  {
               for (int jpl = 0; jpl < nProjLocal; ++jpl)  {
                  SxComplex16 phase = il(ipl) * il(jpl).conj ();
                  for (int iState = 0; iState < nStates; ++iState)  {
...
SxPAWHamiltonian::pawGradDot (int iSpecies, int iAtom, int i, int j,
                              ...
                              const SxVecRef<SxComplex16> &gradP) const
{
   ...
   const SxVector<double> &dS = potPtr->deltaS(iSpecies);
   SxComplex16 res = 0.;
   int offsetA = 0;
   for (int js = 0; js < iSpecies; ++js)  {
      offsetA += int(pBasis->projectors->refOrbMap(js).getSize ())
                 * structure.getNAtoms (js);
   }
   offsetA += int(pBasis->projectors->refOrbMap(iSpecies).getSize ()) * iAtom;
         ...
         for (int ia = 0; ia < structure.getNAtoms (is); ++ia)  { 
            ...
            for (int ipt = 0; ipt < nProjTypes; ++ipt)  {
               ...
               for (int ipt2 = 0; ipt2 < nProjTypes; ++ipt2)  {
                  ...
                  SxArray<SxVector<double> > sym1 (3);
                  SxVector<double> rotGDijDir (nm1,nm2);
                  for(int iDir = 0; iDir < 3; iDir++)   {
                  ...
                  for (int iSym = 0; iSym < nSyms; ++iSym)  {
                     ...
                     for (int iDir = 0; iDir < 3 ; iDir++)   {
                        ...
                        SxVector<double> &GDijDir = GDij(iDir)(iSpin,is,symAtomIdx(iSym));
                        const SxVector<double> &Dl1 = ylmRot(iSym)(l1);
                        const SxVector<double> &Dl2 = ylmRot(iSym)(l2);
...
{
   return SxPtr<SxPAWOverlap>::create (pBasis);
}
...
SxMeshR SxPAWHamiltonian::computeCorePS () const
{
   SX_CHECK (rPtr);
   const SxGBasis &gBasis = rPtr->getGBasis ();
   SxMeshG coresG(0., gBasis);
   for (int is = 0; is < structure.getNSpecies (); ++is)
      coresG += rhoCorePSG(is) * gBasis.getStructureFactor (is);
   SX_VALIDATE_VECTOR (coresG);
   SxMeshR res = *rPtr | coresG;
