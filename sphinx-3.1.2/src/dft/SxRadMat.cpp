   ...
   for (int iSpin = 0; iSpin < getNSpin (); ++iSpin)  {
      for (int is = 0, iSite = 0; is < nSpecies; ++is)  {
         ...
         for (int ia = 0; ia < nAtoms; ++ia, iSite++)  {
            ...
            rho(iSpin, iSite).auxData.iSpin = char(iSpin);
         }
      }
   }
}
...
void SxRadMat::resize (const SxRadMat &in)
{
   ...
   for (int iSpin = 0; iSpin < getNSpin (); ++iSpin)  {
      for (int iSite = 0; iSite < getNSite (); ++iSite)  {
         ssize_t nOrb = in.rho(iSpin, iSite).getNRows ();
         SX_CHECK (in.rho(iSpin, iSite).getNCols () == nOrb,
                   in.rho(iSpin, iSite).getNCols (), nOrb);
         rho(iSpin, iSite).reformat (nOrb, nOrb);
      }
   }
}
         ...
         for (int iSpin = 0; iSpin < getNSpin (); ++iSpin)  {
            ...
            for (int ipt = 0; ipt < pot.getNProjType (is); ++ipt)  {
               ...
               for (int ipt2 = 0; ipt2 < pot.getNProjType (is); ++ipt2)  {
                  ...
                  SxVector<double> sym (nm, nm2), rotRho(nm,nm2);
                  sym.set (0.);
                  for (int iSym = 0; iSym < nSym; ++iSym)  {
                     // --- collect rotRho
                     SxVector<double> &rotD
...
SxRadMat::operator^ (const SxVecRef<SxComplex16> &p) const
{
   ...
   SxVector<SxComplex16> res(p.getNRows (), p.getNCols ());
   res.set (0.);
   res.setBasis (&pBasis);
   res.auxData = p.auxData;
   ...
   for (int is = 0; is < getNSpecies (); ++is)  {
      ...
      for (int ia = 0; ia < getNAtoms(is); ++ia, offset+=nProjLocal)  {
         ...
         const SxVector<double>& Hloc = (*this)(iSpin,is,ia);
      ...
      for (int iSpin = 0; iSpin < nSpin; ++iSpin)  {
         ...
         for (int is = 0; is < nSpecies; ++is)  {
            ...
            for (int ia = 0; ia < info->nAtoms(is); ++ia)  {
               SxVector<double> &Dij = (*this)(iSpin, is, ia);
               Dij.reformat (N, N);
               Dij.auxData.iSpin = char(iSpin);
...
void SxRadMat::writeRho (SxBinIO &io) const
{
   ...
   // --- get dimensions
   for (int is = 0; is < getNSpecies (); ++is) {
      npl(is) = int( (*this)(0,is,0).getNCols () );
