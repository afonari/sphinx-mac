...
class SX_EXPORT_DFT SxPAWPot : public SxSpeciesData
{
   ...
   public:
      ...
      */
      ...
      /** \brief read PAW potentials in xml format

          @param fileName file name
          @param is       species id
          @param nPts     number of radial points to use (-1 for automatic)
      */
      void readXml (const SxString &filename, int iSpecies, int nPts = -1);
   protected:
      ...
      /** \brief read PAW potentials POTCAR from VASP-code
          ...
          @param fileName   file name
          @param is species id
          @param table      symbol table for further options
      */
      void readVasp (const SxString &fileName, int is, const SxSymbolTable *table);

      /// Recompute phiPS tails and write to phiLCAO
      void recomputeTail (int is, const SxVecRef<double> &V, int  irMatch);
   public:
      ...
      /** \brief Initialization density (:is) / Y00
        ...
        */
      SxArray<SxVector<double> > rhoInit;
      ...
      /// Basis set for LCAO waves
      SxPtr<SxRadBasis> radBasisLCAO;

      SxConstPtr<SxRadBasis> getBasisLCAO () const
      {
         if (radBasisLCAO) return radBasisLCAO; else return radBasisPtr;
      }

      /// Partial waves (pseudo) (:is,:iphi)
      SxArray<SxVector<double> > phiPS,
      /// Partial waves (pseudo) for LCAO (:is,:iphi)
                                   phiLCAO,
      ...
      SxArray<SxArray<SxVector<double> > > getPhiPS ();

      /// Get a single pseudo wavefunction
      SxVector<double> getPhiPS (int is, int ip);
      ...
      /** \brief Kinetic energy correction matrix
        ...
        */
      /// (:is)(:ipt,:jpt)
      SxArray<SxVector<double> > deltaKin,
...
pawPot.getNProjType ();
      ...
      SxArray<SxVector<double> > DijInit;
   ...
   public:
      ...
      */
      inline static 
      SxVector<double> nij(const SxVecRef<double> &phi,
                             ssize_t ipt, ssize_t jpt)
      {
         SxVector<double> res = phi.colRef(ipt) * phi.colRef(jpt);
         res.auxData.is = phi.auxData.is;
      ...
      /** Get normalized generalized Gaussians
        ...
        */
      SxVector<double> getGrl (int is, int l) const;

      /** \brief Get matrix elements of potential V
        */
      SxVector<double> getVMatEl (const SxRadialMesh &V,
                                  const SxVecRef<double> &phi) const;
      //@}
      ...
      */
      ...
      /** \brief AE norm within PAW radius (is)(ipt,jpt)
        ...
        setupQijL
        */
      SxArray<SxVector<double> > omegaPAW;
      ...
      SxArray<SxVector<double> > kjXcShape;
...
void writePlot (const SxString &fileName,
                ...
                const SxVecRef<double> &y2 = SxVecRef<double> (),
                const SxVecRef<double> &y3 = SxVecRef<double> () );
