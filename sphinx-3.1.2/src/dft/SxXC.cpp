   ...
   if (pot.nlcc)  {
      ...
      for (int is=0; is < nSpecies; is++)  {
         ...
         if (pot.radCore(is)(0).getSize() > 0)  {
            rad      = SxVector<double> (pot.radCore(is)(0));
            radCore  = SxVector<double> (pot.radCore(is)(1));
            logDr    = pot.logDr(is);
   ...
   if (nlcc)  {
      ...
      for (int is=0; is < nSpecies; is++)  {
         if (phiCore(is).getSize () > 0)  {
            ...
            rhoCoreG += pwBasis->getStructureFactor (is) * phiCore(is);
         }
      }
                     ...
                     const SxFermi *)
{
   ...
   for (iSpin=0; iSpin < nSpin; iSpin++)  {
      ...
      //else       rhoXcR(iSpin) = rhoR(iSpin);
         ...
         rhoXcR(iSpin).unref ();
         rhoXcR(iSpin).SxVecRef<PrecRhoR>::operator=
            (const_cast<SxVector<PrecRhoR>&>(rhoR(iSpin)));
      }
   }
   ...
   if (xcBasisPtr && (xcBasisPtr.getPtr () != rBasisPtr))  {
      ...
      for (iSpin=0; iSpin < nSpin; iSpin++)  {
         ...
         rhoXcR(iSpin) = *xcBasisPtr | (G | rhoXcR(iSpin));
         vXc(iSpin) = SxVector<double> (*xcBasisPtr);
      }
   ...
   if (xcBasisPtr && (xcBasisPtr.getPtr () != rBasisPtr))  {
      ...
      for (iSpin=0; iSpin < nSpin; iSpin++)  {
         // Fourier interpolation
         vXc(iSpin) = *rBasisPtr | (G | vXc(iSpin));
      }
   }

   for (iSpin=0; iSpin < nSpin; iSpin++) rhoXcR(iSpin).unref ();
}
...
//   {
      ...
      for (int d=0; d < 3; d++)
      {
         gradR(d) = R | ( (I * gBasis.gVec.colRef(d)) * ( gBasis | meshR) );
      }
//   }
//#endif
...
{
   SX_CHECK (!SxLoopMPI::getLevel (), SxLoopMPI::getLevel ()->getName ());
...
#pragma omp parallel for firstprivate(xc) reduction(+:myExc)
      ...
      for (int i=i0; i < i1; i++)
      {
         SxVector3<PrecRhoR> gradRhoUp, gradRhoDown, gradRhoTl;
         // --- collect gradients
   ...
   {
      SxArray<SxMeshG > vXcTmp(nSpin);
      for (int iSpin = 0; iSpin < nSpin; ++iSpin) {
         vXcTmp(iSpin) = SxMeshG(0., G);
         for (int iDir = 0; iDir < 3; ++iDir) {
            if (SxLoopMPI::myWork (iDir + 3 * iSpin)) // LoopMPI
            {
               SxMeshR &fxc_gradR = gradients(iSpin)(iDir);
               SxMeshG fxc_gradG = G | fxc_gradR;
