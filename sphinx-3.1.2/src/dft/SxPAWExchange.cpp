...
void SxPAWExchange::prepareRotData(int iq, Coord &kVec) const
{
   ...
   if (pwProjComp) {
      ...
      ssize_t nQlm = ((potPtr->lMaxRho + 1).sqr ()
                   * structure.atomInfo->nAtoms).sum ();
      pwProjComp->orbitalMap.resize (nQlm);
      for (int is = 0, io = 0; is < nSpecies; ++is)
         for (int ia = 0; ia < structure.getNAtoms (is); ++ia)
            for (int l = 0, lm = 0; l <= potPtr->lMaxRho(is); ++l)
               for (int m = -l; m <= l; ++m, ++lm)
                  pwProjComp->orbitalMap (io++).set (is, ia, lm);
   ...
   for (int iSym = 0; iSym < nSym2; ++iSym)  {
      ...
      bool useFolding = false;
      {
         ...
         SxVector<double> &vkqRot = vkq(iSym);
         vkqRot.resize (G.ng);
      ...
      SxVector<double> gkq2 = getGkq2 (dkq);
      
      // --- |G+k-q|^l * Ylm(G+k-q)
      {
         SxVector<double> gkqAbs = sqrt(gkq2);
         SxVector<double> gkqL = gkqAbs;
      ...
      gHard(iSym).reformat (G.ng, nSpecies);
      {
         if (pwProjComp) pwProjComp->refOrbitals(iSym).resize (nSpecies);
         for (int is = 0; is < nSpecies; ++is)  {
            ...
            SxVector<double> gauss = exp( -rc2 * gkq2);
                               ...
                               const SxRadMat &Dij)
{
   ...
   SX_CHECK (wavesIn);
   if (   wavesPtr.getPtr () != NULL
       && (&wavesPtr->getGkBasis () != &wavesIn->getGkBasis ()))
   {
      cout<<"WARNING: SxPAWExchange::compute changes pointer to wave functions!"
          << endl;
   }
   wavesPtr = wavesIn;
   focc = foccIn;
      ...
      for (int iq = 0; iq < nk; ++iq)  { // nk:1
         ...
         for (int iSpin = 0; iSpin < nSpin; ++iSpin)  {  // 1
            ...
            for (ssize_t j0 = 0; j0 < waves.getNStates (ik); j0 += jBlockSize) {
               ...
               for ( ; jBlock > 0
                       && focc(j0 + jBlock - 1, iSpin, ik) < FOCC_MIN ;
                    jBlock--) {/* empty */}
               if (jBlock <= 0)  {
                  // no more occupied states in current block
                  // -> run up with j0, probably until j0=nStates
               ...
               PsiRef pPsiJ;
               pPsiJ = pPsi.getRef<Compact> (pPsi.getNRows () * j0,
                                             pPsi.getNRows (), jBlock);
               ...
               for (int i = 0; i < waves.getNStates (iq); ++i)  {
                  ...
                  for (int iSym = 0; iSym < nSym2; ++iSym)  { // Si8:96
                     ...
                     SymMat S = structure.cell.symGroupPtr
                                ->getSymmorphic (iSym % nSym);
                     Coord qVec = S ^ gkBasis.getK (iq);
                     if (iSym >= nSym) qVec *= -1.;
                     Coord dkq = kVec - qVec;
                     ...
                     SxArray<SxVector<PrecCoeffG> > nHatPhase;
                     if (xContrib & CompTerms)  {
                        nHatPhase = getNHatUnshaped (qlm,iSym,dkq);
                     } else {
                        nHatPhase.resize (structure.getNSpecies ());
                        SxVector<PrecCoeffG> zero (G.ng, jBlock);
                        zero.setBasis (G);
                        zero.set (0.);
                        zero.auxData.ik = iSym; // cf. getNHatUnshaped
                        for (int is = 0; is < nHatPhase.getSize (); ++is)  {
                           nHatPhase(is) = zero;
                        }
                     }

                     SxVector<PrecCoeffG> vGSoft (G.ng, jBlock);
                     vGSoft.setBasis (G);
                     vGSoft.auxData.ik = iSym; // needed for pwProjComp
                     for (int j1 = 0; j1 < jBlock; ++j1)  {
                        // n*(r') = u*(r') psi(r') in G-space
                        // Ref. 1, Eq. 8 ; also Ref 2, Eq. (31)
                        SxVector<PrecCoeffG> nG;
                        if (xContrib & PSTerms)  {
                           SX_CLOCK (Get_nG);
                           nG = (G | (uR.conj () * psiR(j1)));
                        } else {
                           nG = SxVector<PrecCoeffG> (0., G);
                        }

                        SxVector<PrecCoeffG> nHatHard = getNHat (nHatPhase, j1);
                        SxVector<PrecCoeffG> nHatSoft = getNHatSoft (nHatPhase,j1);

                        // two-orbital exchange potential - Ref. 1, Eq. (25)
                        SX_START_TIMER (vG);
                        SxVector<PrecCoeffG> vGHard = vkq(iSym) * (nG + nHatHard);
                        SX_STOP_TIMER (vG);
                        ...
                        if (screened && (xContrib & OnSiteLR))  {
                           SxComplex16 M0;
                           SxVector<PrecCoeffG> deltaN = computePAWCorrG (pXrot, pPsiJ.colRef(j1),
                                                          iSym, dkq, &M0);
                           SxVector<PrecCoeffG> vDeltaN = deltaN * smallV(iSym);
                           double dE = 0.;
                           SX_LOOP(ig)  {
                     ...
                     if (xContrib & HardSoft)  {
                        // Ref. 2, Eq. (40c)
                        SxVecRef<PrecFocc> foccJ
                           = focc(iSpin,ik).getRef (jBlock, j0);
...
PsiG
SxPAWExchange::apply (const PsiRef &psi) const
{
   ...
   if (nStates > jBlockSize)  {
      PsiG res (gBasisRes->ng, nStates);
      res.setBasis (gBasisRes);
      for (int j0 = 0; j0 < nStates; j0 += jBlockSize)  {
         int block = min(jBlockSize, nStates - j0);
         int nRowPsi = (int)psi.getNRows ();
         const PsiRef &psiBlock
            = psi.getRef<Compact> (j0 * nRowPsi, nRowPsi, block);
         res.getRef (block * resG.ng, j0 * resG.ng)  <<= apply(psiBlock);
      }
      return res;
   }
   const SxPWSet &waves = *wavesPtr;
      ...
      for (int i = 0; i < waves.getNStates (iq); ++i)  {
         ...
         for (int iSym = 0; iSym < nSym2; ++iSym)  { // Si8 : nSym2==96
            ...
            if (xContrib & CompTerms)  {
               ...
               nHatPhase.resize (structure.getNSpecies ());
               SxVector<PrecCoeffG> zero (G.ng, nStates);
               zero.setBasis (G);
               zero.set (0.);
               zero.auxData.ik = iSym; // cf. getNHatUnshaped
               for (int is = 0; is < nHatPhase.getSize (); ++is)  {
            ...
            SxVector<PrecCoeffG> vGSoft (G.ng, nStates);
            vGSoft.setBasis (G);
            vGSoft.auxData.ik = iSym; // needed for pwProjComp
            ...
            for (int j = 0; j < nStates; ++j)  {
               ...
               // two-orbital exchange potential - Ref. 1, Eq. (25)
               SxVector<PrecCoeffG> nHatHard = nG + getNHat    (nHatPhase, j);
               SxVector<PrecCoeffG> nHatSoft = nG + getNHatSoft(nHatPhase, j);
               SxVector<PrecCoeffG> vGHard = vkq(iSym) * nHatHard;
   ...
   SX_VALIDATE_VECTOR (Xin);
#ifndef NDEBUG
   SX_LOOP(j) SX_VALIDATE_VECTOR (xPsi(j));
#endif
...
SxPAWExchange::SxQlm SxPAWExchange::gRlExchange (const SxVecRef<PrecCoeffG> &vG,
                                                 const Coord &dkq,
                                                 const SxQlm &qlm) const
{
   SX_CLOCK (Timer::GRlExchange);
   SX_CHECK (potPtr);
   const SxAtomicStructure &str = gPtr->getTau ();
   int nSpecies = str.getNSpecies ();
   ssize_t nStates = vG.getNCols ();
   
   SxQlm res(nSpecies);
   // Ref 2, Eq. (43a)
   SxVector<PrecCoeffG> gRlV;
   if (xContrib & CompTerms)  {
      ...
      for (int ia = 0; ia < str.getNAtoms(is); ++ia, iTlAtom++)  {
         ...
         for (int js=0, jTlAtom = 0; js < str.getNSpecies (); js++)  {
            ...
            for (int ja = 0; ja < neighbors.relPositions.getNAtoms(js); ++ja)
            {
               ...
               SxVector<double> delta = gaussIJ.compute (lmaxI, lmaxJ, cg);
...
SxPAWExchange::qRlExchange (const Coord &dkq,
                            const SxQlm &qlm,
                            SxQlm *grl,
                            const SxVecRef<PrecFocc> &foccJ) const
{
   SX_CLOCK (Timer::Qrl);
   SX_CHECK (potPtr);
   const SxAtomicStructure &str = gPtr->getTau ();
      ...
      for (int iTlAtom = 0; iTlAtom < str.getNAtoms (); ++iTlAtom)  {
         ...
         for (int js=0; js < nSpecies; js++)  { // --- species j
            for (int ja = 0; ja < neighborDist(iTlAtom).getNAtoms(js); ++ja)
            {
               ...
               const SxVector<double> &delta = kernelU(iTlAtom)(jTlAtom);
               ...
               if (grl)  {
                        SxVector<SxComplex16> &gRL  = (*grl)(is)(ia);
                  const SxVector<SxComplex16> &QLMj = qlm(js)(jRefAtom);
                  ssize_t nStates = QLMj.getNCols ();
                  for (ssize_t iState = 0; iState < nStates; ++iState)  {
                     for (int ilm = 0; ilm < delta.getNRows (); ++ilm)  {
                        SxComplex16 sum = 0.;
                        for (int jlm = 0; jlm < delta.getNCols (); ++jlm)
                           sum += delta(ilm, jlm) * QLMj(jlm, iState);
                        gRL(ilm, iState) += kPhase * sum;
                     }
                  }
               } else {
                  const SxVector<SxComplex16> &QLMi = qlm(is)(ia),
...
#ifndef NDEBUG
         if (grl) { SX_VALIDATE_VECTOR ((*grl)(is)(ia)); }
#endif
      }
...
SxVector<PrecCoeffG> 
SxPAWExchange::compensationExchange (const SxVecRef<PrecCoeffG> &vG,
                                     const Coord &dkq,
                                     const SxQlm &qlm,
                                     const PsiRef &pX,
                                     const SxArray<SxComplex16> &n0) const
{
   ...
   SX_CHECK (pX.getNCols () <= 1, pX.getNCols ());
   const SxAtomicStructure &str = gPtr->getTau ();
   SX_CHECK (potPtr);
   const SxPAWPot &pot = *potPtr;
   ssize_t nStates = vG.getNCols ();
   ...
   SxVector<PrecCoeffG> res (pX.getSize (), nStates);
   res.set (0.);
   // TODO: ref
   int offset = 0;
   for (int is = 0; is < str.getNSpecies (); ++is)  {
      ...
      // --- set up i^l 
      SxVector<SxComplex16> il(npt); // i^l
...
SxPAWExchange::SxQlm
SxPAWExchange::getQlm (const PsiRef &pA,
                       const PsiRef &pB) const
{
   SX_CLOCK (Timer::Qlm);
   SX_CHECK (potPtr);
   const SxAtomicStructure &str = gPtr->getTau ();
   ...
   for (int is = 0; is < nSpecies; ++is)  {
      ...
      for (int ia = 0; ia < nAtoms; ++ia)  {
         SxVector<SxComplex16> &qRlm = qlm(is)(ia);
                                ...
                                const Coord &dkq) const
{
   ...
   const SxAtomicStructure &structure = G.getTau ();
   int nSpecies = structure.getNSpecies ();
   SxArray<SxVector<PrecCoeffG> > res(nSpecies);

   /*
   const SxVector<double> &Ylmgl = YlmGl(iSym);
   for (int is = 0; is < nSpecies; ++is)  {
      int nlm = sqr(potPtr->lMaxRho(is) + 1);
      SxVector<SxComplex16> weights(nlm);
      res(is) = SxVector<PrecCoeffG> (0., G);
      res(is).auxData.ik = iSym;
      for (int ia = 0; ia < structure.getNAtoms (is); ++ia)  {
         // e^{iGr}
         SxVecRef<PrecCoeffG> T = G.getPhaseFactors (is,ia);
         //SxVector<PrecCoeffG> sum(G.ng);
         //sum.set (0.);
         // e^{-i(k-q)r}
         ...
         SxVector<PrecCoeffG> sum = Ylmgl ^ weights;
         {
            // --- variant 1
            // res(is) += sum * T;

            // --- variant 2 (should be 25% faster than variant 1)
            SxVector<PrecCoeffG> &resIs = res(is);
#ifdef USE_OPENMP
#pragma omp parallel for
   ...
   */
   ...
   for (int is = 0; is < nSpecies; ++is)  {
      ...
      int nPairs = (nAtoms > 0) ? (int)qlm(is)(0).getNCols () : 0;
      SxVector<SxComplex16> qlms (nlm * nAtoms, nPairs);
      ...
      SX_VALIDATE_VECTOR (qlms);

      // --- set up fast projector
      SxVecRef<SxComplex16> myYlm = YlmGl(iSym)(SxIdx(0, nlm * G.ng-1));
...
SxVector<double> SxPAWExchange::getGkq2(const Coord &dkq) const
{
   SX_CLOCK (Timer::Gkq2);
   SxVector<double> gkq2(0., *gPtr);
   for (int i = 0; i < 3; ++i)
...
SxVector<PrecCoeffG>
SxPAWExchange::getNHat (const SxArray<SxVector<PrecCoeffG> > &nHatUnshaped,
                        ssize_t j) const
{
   SX_CLOCK (Timer::NHat);
   int iSym = nHatUnshaped(0).auxData.ik;
   SX_CHECK (gPtr);
   SxVector<PrecCoeffG> res(0., *gPtr);
...
SxVector<PrecCoeffG>
SxPAWExchange::getNHatSoft (const SxArray<SxVector<PrecCoeffG> > &nHatUnshaped,
                            ssize_t j) const
{
   SX_CLOCK (Timer::NHat);
   int iSym = nHatUnshaped(0).auxData.ik;
...
SxVector<PrecCoeffG> 
SxPAWExchange::getRotated (const PsiRef &proj,
                           const Coord &kVec, int iSym) const
{
   ...
   SxVector<PrecCoeffG> res (proj.getSize ());
   // --- symmetry operation
   // <p(lm,tau)|psi_S(k)>
...
void SxPAWExchange::computeXij (const SxRadMat &Dij)
{
   ...
   for (int iSpin = 0; iSpin < nSpin; ++iSpin)  {
      for (int is = 0; is < nSpecies; ++is)  {
         ...
         for (int ia = 0; ia < nAtoms; ++ia)  {
            SxVector<double> &XijR = Xij(iSpin, is, ia);
            const SxVector<double> &DijR = Dij(iSpin, is, ia);
...
SxVector<PrecCoeffG>
SxPAWExchange::computePAWCorrG (const PsiRef &pa,
                                const PsiRef &pb,
                                int iSym,
                                const Coord &dkq,
                                SxComplex16 *Mab0)
{
   *Mab0 = 0.;
   SxVector<PrecCoeffG> res(smallG(iSym).getSize ());
   res.set (0.);
   const SxPAWPot &pot = *potPtr;
   ssize_t offset = 0;
   ...
   SX_LOOP2(is,ia)  {
      ...
      SxComplex16 kPhase = exp(I * (dkq ^ gPtr->getTau ().getAtom (is, ia)));
      // Ref. 2, Eq. (67), sum over LM and m = 2,4,... 
      const SxVecRef<PrecPhase> &phFac = gPtr->getPhaseFactors(is,ia);
      SX_LOOP(ig)  {
         ssize_t idxg = smallG(iSym)(ig);
         SxComplex16 phase = phFac(idxg) * kPhase;
