...
Real8 SxFermi::getFermiEnergy (Real8 ekt, Real8 nEl, int spin)
{
   ...
   if (fail)  {
      ...
      cout << "ekt=" << ekt << " beta=" << beta << endl;
      cout << "Can't find root of fermi function!" << endl;
      writeSpectrum ("eps-failed", SxString("dat-") + SxLoopMPI::me ());
      SX_EXIT;
   }
...
void SxFermi::resize (int nStates)
{
   ...
   for (ik=0; ik < nk; ik++)
      for (iSpin=0; iSpin < nSpin; iSpin++)
         epsSortList(iSpin,ik) = SxVector<int> (eps(iSpin,ik).getSortIdx());
...
//      if (!io.contains ("nPerK") || (io.ncMode == SxBinIO::WRITE_DATA))
      ...
      for (ik=0; ik < nk; ik++)
      {
         ...
         for (iSpin=0; iSpin < nSpin; iSpin++)
         {
            ...
            {
               io.writeVec ("epsSortIdx", epsSortList(iSpin,ik), nElemName, offset);
               io.writeVec ("focc", focc(iSpin,ik), nElemName, offset);
               io.writeVec ("eps", eps(iSpin,ik), nElemName, offset);
   ...
   try  {
      ...
      for (ik=0; ik < nk; ik++)  {
         ...
         for (iSpin=0; iSpin < nSpin; iSpin++)  {
            ...
            if (nStates <= nPerK(ik))  {
               io.readVec ("focc", &focc(iSpin,ik), nStates, offset);
               io.readVec ("eps", &eps(iSpin,ik),  nStates, offset);
            } else {
               io.readVec ("focc", &focc(iSpin,ik), nPerK(ik), offset);
               io.readVec ("eps", &eps(iSpin,ik),  nPerK(ik), offset);
