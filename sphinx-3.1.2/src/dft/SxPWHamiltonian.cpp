                               ...
                               bool tauChanged, bool /*rhoChanged*/)
{
   ...
   if (tauChanged)  {
      ...
      for (int iSpecies=0; iSpecies < structure.getNSpecies (); iSpecies++)
         vLocG += G.getStructureFactor (iSpecies)
                * phiLocPseudo(iSpecies);
      vLocR = R.symmetrize (R | vLocG);
...
void SxPWHamiltonian::update (const SxFermi &fermi)
{
   ...
   SxMeshR rhoRTotal (0., *rho(0).getBasisPtr());
            ...
            if (contrib & CALC_KIN) {
               ...
               for (int i=0; i < nStates; i++)  {
                  eKin += 0.5 * Gk.weights(ik) * fermi.focc(i,iSpin,ik)
                        * Gk(ik).laplacian (waves(i,iSpin,ik));
               }
            }
   ...
   if (contrib & CALC_HARTREE)  {
      ...
      if (dipoleCorrection)  {
         ...
         SX_MPI_TARGET(TopLevel, TaskGroupAll);
         charge = SxLoopMPI::bcast (charge, 0);
#endif
         SxDipoleCorrZ dipCorr (rhoRTotal, R | rhoHartreeG, charge);
      ...
      eHartree     = 0.5 * tr((R | rhoHartreeG) * vHartreeR)
                   + eField;
      eHartreeElec = TWO_PI
   ...
   if (contrib & CALC_EXT)  {
      ...
      if (!(   xcPtr->xcFunctional == SxXC::EXX
         ...
         if (vExtActsOnNuclei)  {
            ...
            double eExtNuc = -tr(vExtR(0) * (R | rhoGaussG).real ());
            eExt += eExtNuc;
   ...
   if (contrib & (CALC_X | CALC_C))  {
      ...
      if (xcMeshDensity >= 2) {
         ...
         double sp;
         SxComplex16 phase;
         SxVector<SxComplex16> phaseVec(G.ng);
         SxVector3<double> Rvec, Gvec;
         SxArray<SxMeshR> vXc1(nSpin);
         SxArray<SxMeshG> vXc1G(nSpin);
         SxArray<SxMeshG> vXcG(nSpin);
         SxArray<SxMeshG> myrhoG(nSpin);
         SxArray<SxMeshG> rhoGorig(nSpin);
         ...
         for (int iSpin = 0; iSpin < nSpin; iSpin++) {
            rhoGorig(iSpin) = G | rho.rhoR(iSpin);
         }
            ...
            for (int y = 0; y < xcMeshDensity; y++) {
               for (int z = 0; z < xcMeshDensity; z++) {
                  ...
                  for (iG = 0; iG < G.ng; iG++) {
                     ...
                     Gvec(1) =  G.gVec(iG,1);
                     Gvec(2) =  G.gVec(iG,2);
                     phaseVec(iG) = SxComplex16::phase(Gvec ^ Rvec);
                  }


                  for (int iSpin = 0; iSpin < nSpin; iSpin++) {
                     vXc1G(iSpin) = G | xcPtr->vXc(iSpin);
                     for (int i = 0; i < vXc1G(iSpin).getSize (); i++) {
                        vXc1G(iSpin)(i) = vXc1G(iSpin)(i)*phaseVec(i);
                     }
                     xcPtr->vXc(iSpin) = R | vXc1G(iSpin);
                  }
         ...
         for (int iSpin = 0; iSpin < waves.getNSpin (); iSpin++) {
            rho.rhoR(iSpin) = R | rhoGorig(iSpin);
         }
         cout << "...ready" << endl;
      }
      else {
   ...
   for (int iSpin=0; iSpin < nSpin; iSpin++)  {
      ...
      // synchronize effective potential across MPI tasks (to avoid issues with numerical noise)
      SX_MPI_SOURCE(TopLevel, TaskGroupMaster);
      SX_MPI_TARGET(TopLevel, TaskGroupAll);
      SxLoopMPI::bcast (vEffR(iSpin), 0);
   }
   ...
   if (calcForces)  {
      ...
      if (SxLoopMPI::nr () > 1)  {
         ...
         SxLoopMPI::bcast (fTotal.coords, 0);
      }
#endif
   }
}
...
PsiG SxPWHamiltonian::operator* (const PsiRef &psiG) const
{
   SX_CLOCK (Timer::hPsi);

   // --- TODO: ugly!!!
   if (psiG.getNCols () == 1)
      return H_PsiG  (psiG); // apply H to only single state
   else
      return H_PsiGI (psiG); // apply H to all states at once
}

PsiG SxPWHamiltonian::H_PsiG (const PsiRef &psiG) const
{
   ...
   // --- TODO: ugly!!!
   int iSpin   =  psiG.auxData.iSpin;
   int ik      =  psiG.auxData.ik;

   PsiG dPsiG (0., Gk);

   // --- TODO: ugly!!!
   dPsiG.auxData.iSpin = char(iSpin);
   dPsiG.auxData.ik    = ik;
   ...
   if (contrib & CALC_NL && phiOrbNl.getSize () > 0 && !rsProj2)    {
      ...
      SxVector<PrecCoeffG> pPsi = *nlProj | psiG;
...
PsiG SxPWHamiltonian::H_PsiGI (const PsiRef &psiGI) const
{
   SX_CHECK (dynamic_cast<const SxGBasis *>(psiGI.getBasisPtr()));
   SX_CHECK (rho.rBasisPtr);

   int i=0, nStates = (int)psiGI.getNCols ();
   const SxGBasis &Gk = *dynamic_cast<const SxGBasis *>(psiGI.getBasisPtr());
   const SxRBasis &R  = *rho.rBasisPtr;

   // --- TODO: ugly!!!
   int iSpin   =  psiGI.auxData.iSpin;
   int ik      =  psiGI.auxData.ik;

   PsiG dPsiGI (psiGI.getNRows (), psiGI.getNCols ());
   dPsiGI.auxData = psiGI.auxData;
   dPsiGI.set (0.);
#ifdef USE_FFT2d1d
   ...
   for (i=0; i < nStates; i++)  {
      PsiRef dPsiG = dPsiGI.colRef(i);
      const PsiRef &psiG  = psiGI.colRef(i);

      // --- TODO: ugly!!!
      //psiG.auxData.i     = i;
      //psiG.auxData.iSpin = iSpin;
      //psiG.auxData.ik    = ik;
   ...
   if ((contrib & CALC_NL) && phiOrbNl.getSize () > 0 && !rsProj2)  {
      SX_CLOCK (Timer::vNlPsi);
      SxVector<PrecCoeffG> pPsi = *nlProj | psiGI;
...
SxVector<double> 
SxPWHamiltonian::preconditioner (const PsiRef &dPsi,
...
void SxPWHamiltonian::computePhiGauss (const SxGBasis &G)
{
   ...
   for (int iSpecies = 0; iSpecies < structure.getNSpecies (); iSpecies++)  {
      ...
      rad    = SxVector<double> (psPot.rad(iSpecies));
...
{
   rhoGaussG = SxMeshG (0., G);
   for (int iSpecies=0; iSpecies < structure.getNSpecies (); iSpecies++)  {
      rhoGaussG += G.getStructureFactor (iSpecies)
                 * phiGaussG(iSpecies);
   }
   const SxRBasis &R = G.getRBasis ();
...
void SxPWHamiltonian::computePhiLocPseudo (const SxGBasis &G)
{
   ...
   for (int iSpecies=0; iSpecies < structure.getNSpecies (); iSpecies++)  {
      ...
      rad    = SxVector<double> (psPot.rad(iSpecies));
      pot    = SxVector<double> (psPot.pseudoPot(iSpecies)(l));

      func   = pot + zv / rad * erf (rad / rGauss);
...
void SxPWHamiltonian::computePhiNonLocal (const SxGkBasis &Gk, bool rsProjNew)
{
   ...
   double diag, lDr;
   SxPtr<SxRadBasis> rPtr = SxPtr<SxRadBasis>::create(psPot.rad, psPot.logDr);
   SxRadBasis &r = *rPtr;
   SxVector<double>         psi_pot, rad, psi, pot;           // :r
   ...
   SxArray<SxArray<SxVector<double> > > projR(nSpecies);
   ...
   for (iSpecies=0; iSpecies < psPot.getNSpecies (); iSpecies++)  {
      ...
      for (l = SxQuantumNumbers::s; l <= psPot.lMax(iSpecies); l++)  {
         if ( l != psPot.lLoc(iSpecies) )  {
            ...
            psi_pot = psi * pot;
            psi_pot.setBasis (&r);
            psi_pot.auxData.is = iSpecies;
            psi_pot.auxData.l  = char(l);
            if (!psPot.realSpace(iSpecies))
   ...
   cout << SX_SEPARATOR;
   nlProj = SxPtr<SxAOBasis>::create (Gk, projR);
}
...
void SxPWHamiltonian::computeVNl (int iSpin, int ik, const Focc &focc)
{
   ...
   SxVector<PrecCoeffG> allPotPsiFac;
   double nlFac;
   ...
   if (! calcForces)  {
      ...
      if (nNlOrb > 0 && !rsProj2)  {
         ...
         for (int iNlOrb=0; iNlOrb < nNlOrb; iNlOrb++)  {  //:is,ia,n,l,m
            ...
            eNl += nlFac * (allPotPsiFac.rowRef(iNlOrb).absSqr ()
                            * focc(iSpin, ik)                 ).sum ();
         }
      }
      ...
      if (nNlOrb > 0 && !rsProj2)  {
         ...
         if (allStatesAvailable)  {
            const SxPW &wavesRef = (const SxPW &)waves;
            SxVector<PrecCoeffG> pPsi = *nlProj | wavesRef(iSpin, ik);
            SxArray<SxVector<PrecCoeffG> > pPsiGrad
            ...
            for (int iState = 0; iState < nStates; ++iState)  {
               // --- inefficient
               const PsiRef &psi = waves(iState, iSpin, ik);
               SxVector<PrecCoeffG> pPsi = *nlProj | psi;
               SxArray<SxVector<PrecCoeffG> > pPsiGrad
         ...
         int d;
         SxVecRef<PrecCoeffG> potPsiFac;
         for (int iNlOrb=0; iNlOrb < nNlOrb; iNlOrb++)  {  //:is,ia,n,l,m

            // get <mu | Vps | Psi_n> for current mu and all n
            potPsiFac = allPotPsiFac.rowRef(4 * iNlOrb);
            ...
            for (d=0; d < 3; d++)  {  // {:xyz}
               fNl.ref(orb.iSpecies,orb.iAtom)(d) -= 2.
                  * nlFac * dot(allPotPsiFac.rowRef(4 * iNlOrb + d + 1),
                                potPsiFac                              ).im;
            }
         }
      }
            ...
            if (psPot.realSpace(is))  {
               ...
               for (i = 0; i < nStates; ++i)  {
                  ...
                  preFac = Gk.weights(ik) / eKB (iPhi)
                         * focc(i, iSpin, ik);
                  eNl += preFac * phiPsi.rowRef(0).normSqr ();
                  int ia, iDir;
   ...
   for (is=0; is < str.getNSpecies (); is++)  {  // --- species i
      ...
      for (ia=0; ia < str.getNAtoms(is); ia++)  {  // --- atom i
         ...
         for (js=0; js < str.getNSpecies (); js++)  { // --- species j
            ...
            dEsr    = (0.5 * zvI * zvJ) * erfc (arg) / dist; // for all atoms j
            eEwald += dEsr.sum ();
                            ...
                            int iSpin, int ik, int nG_)
{
   ...
   for (iG = 0; iG < nG; iG++)  {
      for (iGp = iG; iGp < nG; iGp++)  {
         ...
         H(iG, iGp) = (idxG == -1) ? (PrecCoeffG)0. : VeffG(idxG);
      }  // :iGp
   }  // :iG

   // --- kinetic part
   SX_LOOP(ig) H(ig,ig) += 0.5 * GkPtr->g2(ig);
   ...
   SX_LOOP(iNlOrb)  {
      ...
      for (iG = 0; iG < nG; iG++)  {
         for (iGp = iG; iGp < nG; iGp++)  {
            H(iG,iGp) += phiNlGk(iG) * phiNlGk(iGp).conj() / diag;
         }  // :iGp
      }  // iG
   }  // :iNlOrb
...
//       ekt = hamiltonian->get("ekt")->toReal();
   ...
   if (hamiltonian->containsGroup ("rsProj"))  {
      ...
      SxArray<SxArray<SxVector<double> > > proj(nSpecies);
      for (int is = 0; is < nSpecies; ++is)  {
         ...
         for (int ipt = 0, ip = 0; ipt < npt; ipt++)  {
            ...
            proj(is)(ip).auxData.is = is;
            proj(is)(ip).auxData.l = char(ipt);
...
bool SxPWHamiltonian::rereadTable (const SxSymbolTable *table)
{
   ...
   if (table->contains ("dipoleCorrection"))  {
      ...
      if (dipole != dipoleCorrection)  {
         ...
         SX_CHECK (rho.rBasisPtr);
         const SxRBasis &R    = *rho.rBasisPtr;
         SxMeshR rhoRTotal (0., R);
         for (int iSpin=0; iSpin < rho.rhoR.getSize (); iSpin++)
            rhoRTotal += rho(iSpin);
         SxMeshR rhoHartree = rhoRTotal - (R | rhoGaussG);
...
void SxPWHamiltonian::writePotentials () const
{
   if (!((contrib & CALC_HARTREE) && (contrib & CALC_LOC))) return;
   SX_MPI_MASTER_ONLY {
      cout << "|   Electrostatic potential ...    "; cout.flush ();
      SxRho((vLocR + vHartreeR)*HA2EV).writeRho ("vElStat-eV.sxb");
      cout << "done\n";
   }
}
                              ...
                              const SxAtomicStructure &str) const
{
   ...
   SxVector<PrecCoeffG> res(nOrb, nOrb);
   const SxArray<PsiG > &muRef = mu.muPhi(ik);
   res.set (0.);

   int nMu = (int)muRef.getSize ();
   SxArray<SxVector<PrecCoeffG> > pPhiMu(nMu);
   ...
   int ia, ka;
   for (int iPhi = 0; iPhi < phiNl(ik).getSize (); ++iPhi)  {
      atPhi = SxSpeciesRef(phiNl(ik)(iPhi).auxData.is) | str;

      // --- set up <phi @ ka|mu @ ia>
      for (int iMu = 0; iMu < nMu; ++iMu)  {
         atMu = SxSpeciesRef(muRef(iMu).auxData.is) | str;
         ...
         {
            SxVector<PrecCoeffG>::Iterator pIt = pPhiMu(iMu).begin ();
            SxComplex16 phase;
            for (ia = 0; ia < atMu.getNAtoms (); ++ia)  {
