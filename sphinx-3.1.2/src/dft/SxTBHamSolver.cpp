...
void SxTBHamSolver::execute (const SxSymbolTable *cmd, bool calc)
{
   ...
   if      (str == "tbInitialGuess")   initialGuess (cmd, calc);
...
void SxTBHamSolver::initialGuess (const SxSymbolTable *cmd, bool calc)
{
...
void SxTBHamSolver::diagonalizeGamma ()
{
   ...
   SxVector<double>           HG(lOrbMax, lOrbMax);
...
SxSymEigensystem<double>
SxTBHamSolver::diag (const SxVector<double> &HIn, const SxVector<double> &SIn) const
{
   SX_CLOCK (Timer::diagonalization);

   // --- Diagonalization det(H - S*Ei) = 0, this is done via 
   // --- det ((L^dagger . H . L) - Ei * 1) = 0, where S^-1 = L . L^dagger
   SxVector<double>    HInNew, SInInv, L;
   SInInv = SIn.inverse();
   L      = SInInv.choleskyDecomposition();//S^-1 = L . L^dagger
   HInNew = (L.transpose () ^ HIn ^ L);
   
   SxSymEigensystem<double> eigIn(std::move(HInNew));
...
void SxTBHamSolver::setFermi (const SxVector<double> &eigenvalues) 
{
   for (int iOrb = 0; iOrb < lOrbMax; iOrb++) {
...
void SxTBHamSolver::diagonalizeCmplx ()
{
   ...
   SxArray<SxVector<SxComplex16> > H(fermi.getNk());
...
void SxTBHamSolver::calcForcesGamma ()
{
   ...
   SxVector<double>  ham1, ham2, ovl1, ovl2;
   SxVector<double>  subPDens, subPergDens, subHG1Part1;
   SxVector<double>  hamDiff, ovlDiff;
   double            gradient;

   SxVector<double> pDens    = getPDensMat();
   SxVector<double> pErgDens = getPErgDensMat();
...
void SxTBHamSolver::calcForcesCmplx ()
{
   ...
   SxVector<SxComplex16>  ham1, ham2, ovl1, ovl2;
   SxVector<SxComplex16>  subPDens, subPergDens, subH1Part1;
   SxVector<SxComplex16>  hamDiff, ovlDiff;
   SxVector<SxComplex16>  pDens, pErgDens;
   double               gradient, arg;
   SxComplex16          phase;
   SxNeighbors          neighbors;
...
SxArray<SxVector3<double> > SxTBHamSolver::getDRhoForcesNonPeriodic ()
{
   ...
   SxVector<double>  dGammaMatrix = getDGammaMatNonPeriodic ();
...
SxArray<SxVector3<double> > SxTBHamSolver::getDRhoForcesPeriodic ()
{
   ...
   // --- calculate the derivative of gamma (analytical)
   SxArray<SxVector<double> >  dGammaMatrix = getDGammaMatPeriodic();
...
SxSymEigensystem<SxComplex16>
SxTBHamSolver::diag (const SxVector<SxComplex16> &HIn,
                     const SxVector<SxComplex16> &SIn) const
{
   ...
   // --- Diagonalization det(H - S*Ei) = 0, this is done via 
   // --- det ((L^dagger . H . L) - Ei * 1) = 0, where S^-1 = L . L^dagger
   SxVector<SxComplex16>    HInNew, SInInv, L;
   SInInv = SIn.inverse();
   L      = SInInv.choleskyDecomposition();//S^-1 = L . L^dagger
   HInNew = (L.adjoint() ^ HIn ^ L);
   
   SxSymEigensystem<SxComplex16> eigIn (HInNew);
...
SxArray<SxVector<double> > SxTBHamSolver::getMullikenRhoG () //:iAtom, iOrb
{
   ...
   SxVector<double> densityMat = getPDensMat ();

   // --- Mulliken charges are the diagonal elements of PSMat
   SxVector<double> PSMat = densityMat ^ SG;
...
SxArray<SxVector<double> > SxTBHamSolver::getMullikenRho () //:iAtom, iOrb
{
   ...
   SxArray<SxVector<double> >     mullikenRhoIn(nTlAtoms);
   SxArray<SxVector<SxComplex16> >  densityMat(fermi.getNk ());
   SxArray<SxVector<SxComplex16> >  PSMat(fermi.getNk ());
...
SxVector<double> SxTBHamSolver::getPDensMat () const
{
   const SxVector<double> &focc = fermi.focc(0,0);// iSpin=0, ik=0
   return eigG.vecs ^ SxDiagMat<double>(focc) ^ eigG.vecs.transpose();
}

SxVector<double> SxTBHamSolver::getPErgDensMat () const
{
   const SxVector<double> &focc = fermi.focc(0,0);// iSpin=0, ik=0
   return eigG.vecs ^ SxDiagMat<double>(focc*eigG.vals) ^ eigG.vecs.transpose();
}

SxVector<SxComplex16> SxTBHamSolver::getPDensMat (int ik) const
{
   const SxVector<double> &focc = fermi.focc(0,ik);// iSpin=0
   return eig(ik).vecs ^ SxDiagMat<double>(focc) ^ eig(ik).vecs.adjoint();
}

SxVector<SxComplex16> SxTBHamSolver::getPErgDensMat (int ik) const
{
   const SxVector<double> &focc = fermi.focc(0,ik);// iSpin=0
   SxDiagMat<double> diag(focc * eig(ik).vals);
   return eig(ik).vecs ^ diag ^ eig(ik).vecs.adjoint();
}
...
SxVector<double> SxTBHamSolver::getGammaMatNonPeriodic () const
{
   SX_CLOCK (Timer::gammaMatrix);

   SxVector<double>  gammaMat(lOrbMax,lOrbMax);
   SxVector3<double> distVec;
...
SxVector<double> SxTBHamSolver::getGammaMatPeriodic () const
{
   SX_CLOCK (Timer::gammaMatrix);
      ...
      for (n2 = -nC2; n2 <= nC2; n2++)  {
         for (n3 = -nC3; n3 <= nC3; n3++)  {
            ...
            if (latVec2  < rEwald2 ) // don't exclude R = 0  
               rLat.append (SxVector3<Real8> (latVec));
         }
      }
   }
      ...
      for (n2 = -nC2; n2 <= nC2; n2++)  {
         for (n3 = -nC3; n3 <= nC3; n3++)  {
            ...
            if ( g2 < gEwald2 && g2 > 1e-10) // excluding G = 0 
               gLat.append (SxVector3<Real8> (gLatVec));
         }
      }
   }
...
SxVector<double> SxTBHamSolver::getDGammaMatNonPeriodic () const
{
   SxVector<double>  dGammaMat(lOrbMax,lOrbMax);
   SxVector3<double> distVec;
   int iA, jA, iS, jS, nOrb1, nOrb2, iOrb, jOrb;
...
SxArray<SxVector<double> > SxTBHamSolver::getDGammaMatPeriodic () const
{
   SxArray<SxVector<double> > dGammaMat(3);
   SxVector<double> matX(lOrbMax,lOrbMax);
   SxVector<double> matY(lOrbMax,lOrbMax);
   SxVector<double> matZ(lOrbMax,lOrbMax);
   ...
   dGammaMat(0) = SxVector<double> (matX);
   dGammaMat(1) = SxVector<double> (matY);
   dGammaMat(2) = SxVector<double> (matZ);
...
bool SxTBHamSolver::isHermitian (const SxVector<SxComplex16> &matIn) const
{
   int iC, iR;
   bool isHerm = true;

   for (iR = 0 ; iR < matIn.getNRows (); iR++)  {
      for (iC = 0 ; iC < matIn.getNCols (); iC++)  {
