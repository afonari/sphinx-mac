...
SxHubbardMO::SxHubbardMO (int siteOffsetIn)
   : SxHubbardXO (siteOffsetIn),
     distFrom (-1.), dDist (0.), nAoPerSite (0), nInterpolate (100)
{
   //empty
}
                           ...
                           double eCut)
{
   ...
   if (setupStructure.getNAtoms () > 0)  {
      ...
      for (int ipt = 0, ipl = 0; ipt < npt; ipt++) {
         int lProj = pawPotPtr->lPhi(iSpecies)(ipt);
         for (int m = -lProj; m <= lProj; m++, ipl++)  {
            proj->refOrbMap(iSpecies)(ipl).set (ipt, lProj, m);
         }
      }
...
static
SxVecRef<double> readAo (SxSimpleParser &mySimpleParser,
                         ...
                         int is)
{
   ...
   SYMBOLGROUP ("orbital")  {
      ...
      if (fromPot)  {
         ...
         checkIot (iot, aoQuamol.getNOrbTypes (isFile), quamolFile, isFile);
                        ...
                        const SxPtr<SxPAWPot> &potPtr)
{
   ...
   SYMBOLPARSE(table)  {
      SYMBOLGROUP ("MO") {
         ...
         ao = readAo (mySimpleParser, aoQuamol, potPtr, is);
         mMO = SYMBOLGET("mMO");
         if (mMO > ao.auxData.l)  {
...
void SxHubbardMO::setupProjGk (const SxGkBasis &gk,
                               const SxPtr<SxPartialWaveBasis>& /*unused*/)
{
   ...
   for (int io = 0, ia = 0; ia < atomIdx.getSize (); ++ia)  {
      ...
      for (int m = 0; m < nm; ++m, ++io)  {
         aoProj->orbitalMap(io).set (iSpecies, iAtom, m);
      }
   }
}
...
class SxHubbardMO::SymMap : public SxHubbardXO::SymMap
{
   private:
      const SxHubbardMO &parent;
      const SxAtomicStructure &structure;
      SxArray<int> mapAtoms;
   ...
   public:
      SymMap (const SxHubbardMO &hubMO, const SxAtomicStructure &structureIn);
      virtual int rotateSite (int iSite, int iSym) override;
      virtual int rotatedType (int iSite, int iOrbType, int iSym) override;
      virtual int getNOrbType (int iSite) override
      {
         return nAtomsPerMolecule;
      }
      virtual int getRefSite (int iSite) override;
      virtual int getOffset (int iSite, int iOrbType) override;
      virtual int getL (int iSite, int iOrbType) override;
};

SxHubbardMO::SymMap::SymMap (const SxHubbardMO &hubMO,
                             const SxAtomicStructure &structureIn)
   : parent(hubMO),
     structure(structureIn),
     mapAtoms(structure.getNAtoms ()),
     grid (structure, 10),
     currentSite(-1)
{
   // map iTlAtom -> atomIdx
   mapAtoms.set (-1);
   SX_LOOP(i) mapAtoms(parent.atomIdx(i))= (int)i;

   SX_CHECK (structure.cell.symGroupPtr);
   int nSym = structure.cell.symGroupPtr->getNSymmorphic ();
   nAtomsPerMolecule = (parent.latShift.getSize () > 0) ? 2 : 1;
   symAtomIdx.reformat(nAtomsPerMolecule,nSym);
}

int SxHubbardMO::SymMap::rotateSite (int iSite, int iSym)
{
   if (currentSite != iSite) setSite (iSite);
   return symAtomIdx(0, iSym) / nAtomsPerMolecule;
}


void SxHubbardMO::SymMap::setSite (int iSite)
{
   int idx = nAtomsPerMolecule * iSite;
   ...
   for (int iSym = 0; iSym < nSym; ++iSym)  {
      ...
      for (int ia = 0; ia < nAtomsPerMolecule; ++ia)  {
         Coord pos = structure.getAtom(parent.atomIdx(idx+ia));
...
int SxHubbardMO::SymMap::rotatedType (int iSite, int iOrbType, int iSym)
{
   if (currentSite != iSite) setSite (iSite);
   return symAtomIdx(iOrbType, iSym) & 1;
}

int SxHubbardMO::SymMap::getOffset (int iSite, int iOrbType)
{
   return iOrbType * parent.getNOrbSite (iSite) / nAtomsPerMolecule;
}

int SxHubbardMO::SymMap::getRefSite (int iSite)
{
   if (currentSite != iSite) setSite (iSite);
   return symAtomIdx.minval () / nAtomsPerMolecule;
}

int SxHubbardMO::SymMap::getL (int iSite, int iOrbType)
{
   return parent.l;
}

SxPtr<SxHubbardXO::SymMap>
SxHubbardMO::getSymMap (const SxAtomicStructure &structure) const
{
   return SxPtr<SxHubbardMO::SymMap>::create (*this, structure);
}
