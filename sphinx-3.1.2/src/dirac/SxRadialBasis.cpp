      ...
      const SxVecRef<double> &vec) const
{
   ...
   if (!realSpace && basisPtr->realSpace)
      return fourier (basisPtr, vec);
   // r->r or g->g interpolation
   // No extrapolation!
   SX_CHECK (basisPtr->getRadMin () <= getRadMin (),
             basisPtr->getRadMin (), getRadMin ());
   SX_CHECK (basisPtr->getRadMax () >= getRadMax (),
             basisPtr->getRadMax (), getRadMax ());
   bool isSpline = vec.getNRows () == 4 * getNElements ();
   SxVector<double> splined;
   if (!isSpline) splined = toSpline(vec);
   SxCubicSpline spline(basisPtr->getRadFunc (), isSpline ? vec : splined);
   SxVecRef<SxRadialBasis::CoeffType> res = spline.getY(rad);
                                            ...
                                            const SxVecRef<double> &vec) const
{
   ...
   if (!realSpace)  {
      ...
      result.auxData.setAtom (iSpecies, iAtom);
      result.auxData.setOrb (n, l, m);
...
SxVecRef<double> SxRadialBasis::projectTo (const SxBasis *basis,
                                           const SxVecRef<double> &in) const
{
   // identity?
   if (basis == this) return const_cast<SxVecRef<double>&>(in);
   // other SxRadialBasis
   if (dynamic_cast<const SxRadialBasis*> (basis))
      return radToRad(dynamic_cast<const SxRadialBasis*>(basis), in);
   // SxRadBasis
   if (const SxRadBasis* radBasis = dynamic_cast<const SxRadBasis*>(basis))
      return toRadBasis(radBasis, in);
   // --- some double basis (e.g. PAW)?
   //const SxExtraBasis<double>* other
   //   = dynamic_cast<const SxExtraBasis<double>*> (basis);
   //if (other) return other->projectFrom (this, in);

   SX_EXIT; return SxVecRef<double>();
}
...
SxVector<double> SxRadialBasis::toVec (const SxVecRef<double> &vec) const
{
   ...
   // set Basis and auxData
   result.setBasis(this);
   result.auxData.setAtom (vec.auxData);
   result.auxData.setOrb (vec.auxData.n, vec.auxData.l, vec.auxData.m);

   return result;
}

double SxRadialBasis::getOverlap(int l1, int m1, int l2, int m2,
                                 const SxVecRef<double> &matElem,
                                 const SxVecRef<double> &Ylm,
                                 const SxYlm::SxClebschTable &clebsch)
{
   int lm1 = SxYlm::combineLm(l1, m1),
       lm2 = SxYlm::combineLm(l2, m2);
   double res = 0.;
   for (int L = abs(l1 - l2); L <= l1 + l2; L +=2)  {
      double integral = matElem(L);
      for (int M = -L; M <= L; ++M)  {
         int LM = SxYlm::combineLm(L,M);
         res += clebsch(lm2, LM, lm1) * Ylm(LM) * integral;
      }
   }
   res *= FOUR_PI;
   return res;
...
SxVector<double>
SxRadialBasis::computeMatElem(const SxVecRef<double> &f1,
                              const SxVecRef<double> &f2,
                              const SxArray<SxVector<double> > &jsb)
{
   int l1 = f1.auxData.l,
       l2 = f2.auxData.l;
   SX_CHECK (dynamic_cast<const SxRadialBasis*>(f1.getBasisPtr ()));
   SX_CHECK (dynamic_cast<const SxRadialBasis*>(f2.getBasisPtr ()));
   SX_CHECK (f1.getBasisPtr () == f2.getBasisPtr ());

   SxVector<double> res(l1 + l2 + 1);
   //res.set (0.);
   for (int L = abs(l1 - l2); L <= l1 + l2; L +=2)
      res(L) = ::tr (f1 * f2 * jsb (L));
   return res;
}

SxArray<SxVector<double> >
SxRadialBasis::computeJsb (int lmax, double dist)
{
   SxVector<double> gr = dist * getRadFunc ();
   SxArray<SxVector<double> > res(lmax + 1);
   for (int l = 0; l <= lmax; ++l)
      res(l) = SxRadialBasis::jsb (l, gr);
   return res;
}
