...
/** \brief | \b G> basis
  ...
  \author  Sixten Boeck
  */
class SX_EXPORT_DIRAC SxGBasis : public SxBasis
{
   public:

      typedef PrecCoeffG           CoeffType;
      typedef SxVector<PrecCoeffG> TPsi;
      ...
      /** \brief list of \f$ |\mathbf{G}|^2 \f$, {:ig}
           ...
           \f] */
      SxVector<PrecG>                    g2;
      /** \brief Get structure factors for one species
          @param is species id
       */
      const SxVector<PrecPhase> &getStructureFactor (ssize_t is) const
      {
         // compute structure factors on demand
         if (structureFactors(is).getSize () == 0)
            computeStructureFactor (is);
         return structureFactors(is);
      }
      /** \brief Get phase factors for one atom
        ...
        \sa phaseFactors
       */
      SxVecRef<PrecPhase> getPhaseFactors(ssize_t is, ssize_t ia) const;
      /** \brief Get phase factors for one atom

        \param is  species id
        \param ia  atom id
        \param res preallocated vector for result
        ...
        \sa phaseFactors
       */
      void getPhaseFactors(ssize_t is, ssize_t ia,
                           SxVecRef<PrecPhase> &res) const;
   ...
   protected:
      /** \brief phase factors, {:is,:ia,:ig}
           ...
           stored permanently.
       */
      // 2024-10-04 CF: leave atoms as single vectors because they cannot
      //                be used in a single block.
      mutable SxArray<SxArray<SxVector<PrecPhase> > > phaseFactors;
      /** \brief structure factors, {:is,:ig}
           ...
           \sa phaseFactors
       */
      mutable SxArray<SxVector<PrecPhase> >      structureFactors;  // :is,:ig
      ...
      /// If necessary, set up 1D phFac(is) and packedGrel
      void setupPhase1D (ssize_t is) const;
      /// Set up packed 1D phase factors for a given coordinate
      SxVector<SxComplex16> setupPhase1D (const Coord &tau) const;
      /// Setup mapping from ig to packed 1D phase factors
      void setupPackedG () const;

      /// Compute structure factor for one species
      void computeStructureFactor (ssize_t is) const;
   public:
      /** The index array to build up the fft mesh :iFFT:ig.
        ...
        \author Sixten Boeck
       */
      mutable SxArray<SxVector<PrecFFTIdx> > n123;
      ...
      SxVecRef<double> getYlm(int l, int m) const;
   ...
   protected:
      ...
      /// The cashed Ylm
      mutable SxVector<double> realYlm;
   ...
   public:
      ...
      /// Convolute
      PsiG convolute (const PsiRef &allPsi, const SxVecRef<double> &V) const;

      /// 2+1D FFT algorithm for density calculation
      void addToRho (const PsiRef &allPsi,
                     const SxVecRef<double> &focc,
                     SxVecRef<double> *rho) const;

      /// Returns number of G vectors
      virtual ssize_t getNElements () const { return ng; }
      ...
      SxGBasis (const SxVector3<int> &mesh, const SxAtomicStructure &,
                PrecEnergy gCut,
                const SxVector3<PrecG> &dG = SxVector3<PrecG>(0.,0.,0.),
   ...
   public:
      ...
      */
      SxVector<SxComplex16> getPhaseFactors(const Coord &tau) const
      {
      ...
      composePhase (const SxVecRef<SxComplex16> &phFac) const;

      /** Compose 3D phase factors from packed 1D phase factors
          @param phFac 1D phase factors
          @param res   preallocated target vector
        */
      void composePhase (const SxVecRef<SxComplex16> &phFac,
                               SxVecRef<SxComplex16> &res) const;
      ...
      /** Multiply with 3D phase factors from packed 1D phase factors
        ...
        */
      void applyComposedPhase (const SxVecRef<SxComplex16> &phFac,
      ...
      SxVector3<PrecG> getG (ssize_t ig) const
      {
         return SxVector3<PrecG> (gVec(ig,0), gVec(ig,1), gVec(ig,2));
      }

      SxVector3<PrecG> getK () const;

      REGISTER_PROJECTOR (SxGBasis, identity);
      REGISTER_PROJECTOR (SxRBasis, toRealSpace);
      REGISTER_PROJECTOR (SxAOBasis, toAO);

      PROJECT_TOEXTRA_VIA_FROM(SxExtraBasis<SxComplex16>, float);
      PROJECT_TOEXTRA_VIA_FROM(SxExtraBasis<SxComplex16>, double);
      PROJECT_TOEXTRA_VIA_FROM(SxExtraBasis<SxComplex16>, SxComplex8);
      PROJECT_TOEXTRA_VIA_FROM(SxExtraBasis<SxComplex16>, SxComplex16);
      ...
      /** \brief Identity projector from \b G to \b G
          ...
          \sa \ref page_dirac */
      SxVecRef<CoeffType> identity (const SxGBasis *,
      ...
      /** \brief Projector from \b G space to realspace
          ...
          \sa \ref page_dirac */
      SxVector<PrecCoeffG> toRealSpace ( const SxRBasis *,
                                           const SxVecRef<PrecCoeffG> &
                                         ) const;
      /** \brief Projector from \b G space to radial Space
          ...
          \sa /ref page_dirac */
      /*
      SxVector<PrecCoeffG> toRadialSpace ( const SxRadBasis *,
                                             const SxVecRef<PrecCoeffG> &
                                           ) const;
      */
      ...
      projectTo (const SxBasis *, const SxVecRef<SxComplex16> &) const;
      ...
      /** \brief Laplacian operator in \b G space
          ...
          \f]
          */
      virtual Real8 laplacian (const SxVecRef<PrecCoeffG> &) const;

      /**
        \author Abdullah Al-Sharif
        \deprecated see list of add-ons
        */
      PsiR GtoR111 (const PsiRef &, int nr, int shift=0) const;
      ...
      /**
        ...
        */
      PsiR GtoR111 (const PsiRef&, int nr, int shift, const SxCell &cell) const;
      ...
      // use also the "common basis" scalarProduct from SxBasis
      using SxBasis::scalarProduct;
      ...
      /** \brief shifts a function \f$ f_{\mathbf{k}} \f$ given on this
                 ...
                 \author            Matthias Wahn
       */
      SxVector<SxComplex16> shiftK (const SxVecRef<SxComplex16> &f,
       ...
       */
      SxVector<SxComplex16>
         shiftK (const SxVecRef<SxComplex16> &f, RelVec &deltaGRel);
       ...
       */
      PsiG transferWaves (const PsiRef &in) const;
      ...
      PsiG mapToFFT (const PsiRef &, const SxVecRef<PrecFFTIdx> &,
