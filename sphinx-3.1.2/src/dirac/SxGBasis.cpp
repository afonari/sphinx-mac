...
void SxGBasis::changeTau (const SxAtomicStructure &tauList)
{
   ...
   if (memMode == SaveTime)  {
      for (int is=0; is < nSpecies; is++)  {
         ...
         structureFactors(is).auxData.is = is;
         phaseFactors(is).resize (tauList.getNAtoms(is));
         for (int ia=0; ia < tauList.getNAtoms(is); ia++)  {
            phaseFactors(is)(ia).resize (ng);
            phaseFactors(is)(ia).auxData.setAtom (is, ia);
            SxVector3<PrecTauR> tau = tauList(is,ia);
            SxVector<PrecCoeffG>::Iterator phFacIt, strFacIt;
            strFacIt = structureFactors(is).begin ();
            phFacIt = phaseFactors(is)(ia).begin ();
            for (ssize_t ig=0; ig < ng; ++ig, ++phFacIt, ++strFacIt)  {
               double phase = -(getG(ig) ^ tau);
               PrecCoeffG c     = PrecCoeffG (cos(phase), sin(phase));
...
SxVecRef<PrecPhase> SxGBasis::getPhaseFactors(ssize_t is, ssize_t ia) const
{
   ...
   SX_CHECK (ia >= 0 && ia < getTau ().getNAtoms ((int)is),
             ia, getTau ().getNAtoms ((int)is) );
   if (memMode == SaveTime)
      return phaseFactors(is)(ia);

   // --- calculate phase factors
   if (phaseFactors(is).getSize () == 0) setupPhase1D (is);

   SxVector<PrecPhase> res = composePhase (phaseFactors(is)(ia));
   res.auxData.setAtom ((int)is, (int)ia);
   return res;
}

void SxGBasis::getPhaseFactors(ssize_t is, ssize_t ia,
                               SxVecRef<PrecPhase> &res) const
{
   ...
   if (memMode == SaveTime)  {
      res <<= phaseFactors(is)(ia);
   } else {
      // calculate phase factors
      if (phaseFactors(is).getSize () == 0) setupPhase1D (is);
      composePhase (phaseFactors(is)(ia), res);
   }
}
...
SxGBasis::composePhase (const SxVecRef<SxComplex16> &phFac) const
{
   ...
   composePhase (phFac, res);
   res.setBasis (this);
   return res;
}

void SxGBasis::composePhase (const SxVecRef<SxComplex16> &phFac,
                                   SxVecRef<SxComplex16> &res) const
{
   SX_CHECK (res.getSize () == ng, res.getSize ());
...
void SxGBasis::computeStructureFactor (ssize_t is) const
{
   SxVector<PrecPhase> &strFac = structureFactors(is);
   strFac.resize (ng);
   strFac.set (0.);
   strFac.setBasis (this);
   strFac.auxData.is = (int)is;
   const SxAtomicStructure &str = getTau ();
   bool noPhase = phaseFactors(is).getSize () == 0;
   for (int ia = 0; ia < str.getNAtoms ((int)is); ++ia)
      strFac += getPhaseFactors(is, ia);
   if (noPhase) phaseFactors(is).resize (0);
}
...
void SxGBasis::setupPhase1D (ssize_t is) const
{
   SX_CHECK (memMode == SaveMemory);
   const SxAtomicStructure &str = getTau ();
   SX_CHECK (is >= 0 && is < str.getNSpecies (), is, str.getNSpecies ());

   int nAtoms = str.getNAtoms ((int)is);
   phaseFactors(is).resize (nAtoms);
   for (int ia = 0; ia < nAtoms; ++ia)
      phaseFactors(is)(ia) = setupPhase1D(str.constRef(is,ia));
   if (packedGrel.getSize () != 3*ng) setupPackedG ();
}
...
{
   for (int is = 0; is < phaseFactors.getSize (); ++is)  {
      phaseFactors(is).resize (0);
      structureFactors(is).resize (0);
   }
}
