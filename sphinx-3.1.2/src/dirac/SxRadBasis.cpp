...
SxVecRef<double> SxRadBasis::projectTo (const SxBasis *basis,
                                        const SxVecRef<double> &in) const
{
   // identity ?
   if (basis == this) return const_cast<SxVecRef<double>&>(in);
   // other SxRadBasis
   if (const SxRadBasis* rBasis = dynamic_cast<const SxRadBasis*> (basis))
      return changeRadBasis(rBasis, in);
   // SxRadialBasis
   if (const SxRadialBasis* rBasis = dynamic_cast<const SxRadialBasis*>(basis))
      return toRadialBasis(rBasis, in);

   // --- some other double basis?
