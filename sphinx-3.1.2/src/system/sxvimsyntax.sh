...
| grep -v '^\s*[A-Za-z]\s*$' | xargs | grep . | fold -w55 -s \
...
echo "Getting SX_EXPORT_... macros"
sed -ne '/# *define *SX_EXPORT\w*\s*$/{s/.*define *//p;}' */Sx*.h ${sxbase}/*/Sx*.h \
  | xargs | fold -w55 -s \
  | sed -e's/^/syn keyword sxExportMacro /' >> $syntaxfile

# things that aren't detected by automatic commands:
# - SX_UNUSED in SxMacroLib.h
# - SX_VALIDATE_VECTOR in SxVector.h
# - ssize_t isn't standard
cat <<END_STD_PART >> $syntaxfile

syn keyword sxMacro SX_VALIDATE_VECTOR SX_NO_MPI SX_UNUSED
