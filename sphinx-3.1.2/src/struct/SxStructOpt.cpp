...
namespace {
   ...
   template<class T>
   void requirePtr(const SxPtr<T> &ptr, FILE *extRes)
   {
      if (!ptr)  {
...
void SxStructOpt::extControl (const SxSymbolTable *cmd, bool calc)
{
   ...
   SX_MPI_SLAVE_ONLY {
      ...
      while(true /* exit is inside */)
      {
         ...
         if (minimId == WriteWaves)  {
            int size = SxLoopMPI::bcast(-1, 0);
            SX_CHECK (size > 0, size);
            SxString fileName;
            fileName.resize (size);
            SxLoopMPI::bcast(const_cast<char*>(fileName.ascii ()), size, 0);
            hamSolver->write (fileName);
            continue;
         }
         if (minimId == ReadRho)  {
   ...
   while (!feof(extCtl))  {
      ...
      // --- set spin constraint
         ...
         requirePtr(hamSolver->spinConstraint, extRes);
      ...
      // --- get number of hubbard sites
         ...
         requirePtr(pawHam, extRes);
         requirePtr(pawHam->hubbardU, extRes);
      ...
      // --- write waves
      else if (cmdMatch(extCmd, "write waves"))  {
         SxString fileName(extCmd + 11);
         fileName = fileName.trim ();
         if (fileName == "-")  {
            char buffer[4096], *res;
            buffer[0] = 0;
         ...
         if (fileName.getSize () == 0) fileName="waves.sxb";
         cout << "Writing waves and fermi to '" << fileName << "'." << endl;
         SxLoopMPI::bcast (WriteWaves, 0);
         int size = (int)fileName.getSize ();
         SxLoopMPI::bcast (size, 0);
         SxLoopMPI::bcast (const_cast<char*>(fileName.ascii ()), size, 0);
         hamSolver->write (fileName);
