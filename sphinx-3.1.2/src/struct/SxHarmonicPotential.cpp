   ...
   : beta (1.0), energy (-1.)
{
   ...
   SxStack<SxVector<double> > matSymStack, matSymQStack;
   genQ.resize (nAtoms);
   genQ.set (-1);
...
SxVector<double> SxHarmonicPotential::getParamSpace () const
{
   int nFreeParam = 0;
   for (int i = 0; i < matSyms.getSize (); ++i)  {
      nFreeParam += (int)matSyms(i).getNCols ();
   }
   SxVector<double> res(9 * matSyms.getSize (), nFreeParam);
   res.set (0.);
   for (int i = 0, iFree = 0; i < matSyms.getSize (); ++i)  {
      for (int j = 0; j < matSyms(i).getNCols (); ++j, ++iFree)  {
         // fill in matrix symmetries (9 elements) @ (9*i [+0..8], iFree)
         res.getRef (9, res.getIdx(9*i, iFree)) <<= matSyms(i).colRef (j);
      }
   }
   return res;
}
                                  ...
                                  SxArray<int> genId)
{
   ...
   // --- setup weights
   SxVector<double> wik(nAniso, nAniso);
   wik.set (0.);
   for (int iAniso = 0; iAniso < nAniso; ++iAniso)  {
      int ia = idAniso(iAniso);
   ...
   SxVector<double> changeQ = -wik.transpose ();
                                          ...
                                          bool symmetric) const
{
   ...
   for (int iSymM = 0; iSymM < nSymM; ++iSymM)  {
      ...
      SxSymEigensystem<double> eig(U.transpose () ^ M ^ U);
      eig.vecs = U ^ eig.vecs;
      int nDof = 0;
      for (int i = 0; i < eig.vecs.getNCols (); ++i)
         if (fabs (eig.vals(i)) < 1e-9)
   ...
   if (nSymM > 1)
      cout << "Computing symmetry constraints from " << nSymM << " symmetries: "
           << U.getNCols () << " free parameters" << endl;
   ...
   */
   ...
   for (int is = 0, iTlAtom = 0; is < getNSpecies (); ++is)  {
      for (int ia = 0; ia < refStr.getNAtoms (is); ++ia, ++iTlAtom)
         Qab(iTlAtom) = SxMatrix3<double> (valenceCharge(is));
...
SxHarmonicPotential::getElForces (const SxAtomicStructure &tau)
{
   ...
   if (nAniso >= 0)  {
      ...
      // --- set up rho
      SxVector<PrecCoeffG> rho(G), V(G), muG, gradV;
      rho.set (0.);
      muG = SxVector<SxComplex16> (G.ng, 3);
      muG.setBasis (G);
      muG.set (0.);
      for (int is = 0, iTl = 0; is < tau.getNSpecies (); ++is)  {
         for (int ia = 0; ia < tau.getNAtoms (is); ++ia, ++iTl)  {
            ...
            const SxVecRef<PrecCoeffG> &T = G.getPhaseFactors(is,ia);
      ...
      SxVector<double> shape = exp(-G.g2*(betaSoft*betaSoft/4.))
      ...
      gradV = SxVector<SxComplex16> (G.ng, 3);
      gradV.setBasis (G);
      for (int i = 0; i < 3; ++i)  {
      ...
      for (int is = 0, iTl = 0; is < tau.getNSpecies (); ++is)  {
         for (int ia = 0; ia < tau.getNAtoms (is); ++ia, ++iTl)  {
            ...
            SxVector<PrecCoeffG> Ts = G.getPhaseFactors(is,ia) * shape;
            Coord dEdMu(0., 0., 0.);
            ...
            for (int ja = 0; ja < nn.relPositions.getNAtoms(); ++ja)  {
               ...
               SxVector<double> hIJ = gaussIJ.compute (1, 1, cg);
            ...
            if (iAniso >= 0)  {
               // - mu(R,i) [d phi_R(G)/ d R] shape(G) gradV(G,i)
               SxVector<PrecCoeffG> muGradV = mu(iAniso)(0) * gradV.colRef(0)
   ...
   SxGBasis &G = *gBasis;
   G.changeTau (tau);
   SxVector<PrecCoeffG> rho(G), V(G);
   rho.set (0.);
   for (int is = 0; is < getNSpecies (); ++is)  {
      rho.plus_assign_ax (valenceCharge(is), G.getStructureFactor (is));
   }
   if (fabs(rho(0).re) > 1e-8)  {
      cout << "Warning: net charge=" << rho(0).re << endl;
   }
   rho(0) = 0.;
   SxVector<double> shape = exp(-G.g2*(beta*beta/4.)) / sqrt(tau.cell.volume);
   rho *= shape;
...
SxHarmonicPotential::paramGrad (const SxAtomicStructure &tau)
{
   ...
   SxVector<double> res (3 * tau.getNAtoms (), nParam);
   res.set (0.);
...
void SxHarmonicPotential::setHesse (const SxVecRef<double> &param)
{
                                 ...
                                 const SxAtomicStructure &tau)
{
   ...
   SxVector<SxComplex16> res(nDof, nDof);
   res.set (0.);
   ...
   SxVector<double> shape = exp(-G.g2*(betaSoft*betaSoft/2.))
                            / tau.cell.volume;
   rho *= shape;
   for (int i = 0; i < 3; ++i)
      muG.colRef(i) *= shape;
   ...
   for (int iBlock = 0; iBlock * iBlockSize < tau.getNAtoms (); ++iBlock)  {
      ...
      SxVector<PrecCoeffG> allTi (G.ng, iNext - iFrom);
      ...
      for (int jBlock = 0; jBlock * jBlockSize < tau.getNAtoms (); ++jBlock)  {
         ...
         SxVector<PrecCoeffG> allWj (G.ng, 6 * (jNext - jFrom));
         for (int jTl = jFrom; jTl < jNext; ++jTl)  {
            int ja, js = tau.getISpecies (jTl, &ja);
            SxVecRef<PrecCoeffG> Tj = G.getPhaseFactors(js,ja);
...
#pragma omp parallel for
         ...
         SxVector<PrecCoeffG> allWabJ = allTi.overlap (allWj);
...
// <<<<<<<< shift indent to the left, now come the actual i/j loops
   ...
   for (int js = 0, jTl = 0; js < tau.getNSpecies (); ++js)  {
      for (int ja = 0; ja < tau.getNAtoms (js); ++ja, ++jTl)  {
         ...
         SxVecRef<PrecCoeffG> Wj = allWj.colsRef (6 * (jTl - jFrom), 6);
         int nr = int(allWabJ.getNRows ());
         SxVecRef<PrecCoeffG> allWab = allWabJ(SxIdx (nr * 6 * (jTl - jFrom),
                                      nr * (6 * (jTl - jFrom + 1)) - 1) );
         allWab.reshape (nr, 6);
         ...
         for (int is = 0, iTl = 0; is < tau.getNSpecies (); ++is)  {
            for (int ia = 0; ia < tau.getNAtoms (is); ++ia, ++iTl)  {
               ...
               //SxVecRef<PrecCoeffG> Ti = G.getPhaseFactors(is,ia);
               SxVecRef<PrecCoeffG> Ti = allTi.colRef (iTl - iFrom);

               //SxVector<PrecCoeffG> Wab = Ti.adjoint () ^ Wj;
               SxVector<PrecCoeffG> Wab = allWab.rowRef (iTl - iFrom);
               // d^2 E/d R^k d mu^n
               SxVector<PrecCoeffG> d2EdRdMu(6); d2EdRdMu.set (0.);
         ...
         // QV term of diagonal
         SX_LOOP2(a,b)  {
            SxVector<PrecCoeffG> Vab = G.gVec.colRef(a) * G.gVec.colRef (b) * V;
         ...
         if (muJ.normSqr () > 1e-12)  {
            SxVector<PrecCoeffG> muE(G);
            muE.set (0.);
            SX_LOOP(iDir) muE += muJ(iDir) * gradV.colRef(iDir);
            SX_LOOP2(a,b)  {
               SxVector<PrecCoeffG> muEab
                  = G.gVec.colRef(a) * G.gVec.colRef (b) * muE;
         ...
         for (int ia = 0; ia < nn.relPositions.getNAtoms(); ++ia)  {
            ...
            if (jAniso >= 0)  {
               ...
               SxVector<PrecCoeffG> d2EdRdMuPacked(6);
               SX_LOOP2(a,b)
...
void SxHarmonicPotential::addH4 (int iTl, int jTl,
                                 SxVector<SxComplex16> *resPtr,
                                 const SxMatrix3<SxComplex16> &d2EdMu2,
                                 const Coord &kVec)
{
   // i=m, j=n in writeup
   SX_CHECK (resPtr);
   SxVector<SxComplex16> &res = *resPtr;
   int iAniso = mapAniso(iTl);
   int jAniso = mapAniso(jTl);
   SxMatrix3<SxComplex16> QQW
      = SxMatrix3<SxComplex16>(QFrame(iAniso)).transpose ()
      ^ d2EdMu2 
      ^ SxMatrix3<SxComplex16>(QFrame(jAniso));
...
void SxHarmonicPotential::addH23 (int iTl, int jTl,
                                  SxVector<SxComplex16> *resPtr,
                                  const SxVector<PrecCoeffG> &d2EdRdMu,
                                  const Coord &kVec)
{
   SX_CHECK (resPtr);
   SxVector<SxComplex16> &res = *resPtr;
   int jAniso = mapAniso(jTl);
   SxMatrix3<SxComplex16> QW; QW.set (0.);
   SX_LOOP3(a,b,b2)
...
SxVector<SxComplex16> SxHarmonicPotential::getHesse (const Coord &kVec,
                                                   const SxAtomicStructure &tau)
{
   SX_CLOCK (HesseTotal);
   SxVector<SxComplex16> res = getHesseEl (kVec, tau);
